...
PHPySelectBottom::PHPySelectBottom(const std::string &name): SubsysReco(name)
{
  phpythia = 0;		// array of pythia particles
}

PHPySelectBottom::~PHPySelectBottom()
{
}

int PHPySelectBottom::Init(PHCompositeNode *topNode)
{
  return EVENT_OK;
}

int PHPySelectBottom::End(PHCompositeNode *topNode)
{
  return EVENT_OK;
}

int PHPySelectBottom::process_event(PHCompositeNode *topNode)
{
  ...
  for ( int ipart=0; ipart<npart; ipart++ ) {
    ...
    if ( pt<0.5 || fabs(rapidity)>0.5 ) continue;
    TMCParticle *parent;
    if ( phpythia->hasAncestor(part,5)!=0 ) {
      parent = phpythia->hasAncestor(part,5);
    } else if ( phpythia->hasAncestor(part,-5)!=0 ) {
      parent = phpythia->hasAncestor(part,-5);
    } else {
      continue;
    }
...
int PHPySelectBottom::ParticleCut( TMCParticle *part )
{
  TMCParticle *parent = phpythia->getParticle(m_bottom_id);
  /// for bottom
  if ( part==parent ) return 1;
  if ( phpythia->hasAncestor(part,5)==parent ) return 1;
  if ( phpythia->hasAncestor(part,-5)==parent ) return 1;

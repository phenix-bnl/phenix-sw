C     Modification for PHENIX - Richard Seto - Sep 12, 2011
c      HIPR1(50) and above are free to use
C       use HIPR1(51) for parp(67) in pythia to limit initial state radiation
C
c     Version 1.37
c
...
C
C****************************************************************
      SUBROUTINE HIJING_GEN(FRAME,BMIN0,BMAX0,BIMPACT)
CSCJ	SUBROUTINE HIJING(FRAME,BMIN0,BMAX0)
	CHARACTER FRAME*8
...
C
C BAC 12-Aug-2002 Use Hulthen wave function.
C
           RSEP = HULTHENMC(NSEED)

c          Only sample costheta from 0 to 1 since we have to
c            keep track of which nucleon is at larger z.
c
           COSTHETA = RANF(NSEED)
           SINTHETA = SQRT(1.0 - COSTHETA*COSTHETA)
           PHI = RANF(NSEED)*2.0*HIPR1(40)

C
C  Make sure we preserve the ordering of the nucleons, larger z
C    is the first one in the list.
C
           RPERP = RSEP*SINTHETA/2.0
           RPARA = RSEP*COSTHETA/2.0

           YP(1, 1) = RPERP*COS(PHI)
           YP(2, 1) = RPERP*SIN(PHI)
           YP(3, 1) = RPARA
...
C             HIPR1(29)
 ...
 12         CONTINUE
        ...
        YPAVGZ = 0
        DO I = 1, IHNT2(1)
          YPAVGX = YPAVGX + YP(1, I)
          YPAVGY = YPAVGY + YP(2, I)
          YPAVGZ = YPAVGZ + YP(3, I)
        END DO
C
C******************************
14	YT(1,1)=0.0
...
C
C  Make sure we preserve the ordering of the nucleons, larger z
C    is the first one in the list. Sample only positive cos(theta)
C
           COSTHETA = RANF(NSEED)
           SINTHETA = SQRT(1.0 - COSTHETA*COSTHETA)
           PHI = RANF(NSEED)*2.0*HIPR1(40)
           ...
           YT(1, 1) = RPERP*COS(PHI)
           YT(2, 1) = RPERP*SIN(PHI)
           YT(3, 1) = RPARA
...
C             HIPR1(29)
 ...
 22       CONTINUE
        ...
        DO I = 1, IHNT2(3)
          YTAVGX = YTAVGX + YT(1, I)
          YTAVGY = YTAVGY + YT(2, I)
          YTAVGZ = YTAVGZ + YT(3, I)

          YTAVGXSQ = YTAVGXSQ + YT(1, I)*YT(1, I)
          YTAVGYSQ = YTAVGYSQ + YT(2, I)*YT(2, I)
          YTAVGZSQ = YTAVGZSQ + YT(3, I)*YT(3, I)
        END DO

        YTAVGX = YTAVGX/FLOAT(IHNT2(3))
        YTAVGY = YTAVGY/FLOAT(IHNT2(3))
        YTAVGZ = YTAVGZ/FLOAT(IHNT2(3))

        YTAVGXSQ = YTAVGXSQ/FLOAT(IHNT2(3))
        YTAVGYSQ = YTAVGYSQ/FLOAT(IHNT2(3))
        YTAVGZSQ = YTAVGZSQ/FLOAT(IHNT2(3))

        YTMSQX = YTAVGXSQ - YTAVGX*YTAVGX
        YTMSQY = YTAVGYSQ - YTAVGY*YTAVGY
        YTMSQZ = YTAVGZSQ - YTAVGZ*YTAVGZ

        RTRMS = sqrt(YTMSQX + YTMSQY + YTMSQZ)
        RSCALE = HINT1(71)/RTRMS

        DO I = 1, IHNT2(3)
          YT(1, I) = (YT(1, I) - YTAVGX)*RSCALE
          YT(2, I) = (YT(2, I) - YTAVGY)*RSCALE
          YT(3, I) = (YT(3, I) - YTAVGZ)*RSCALE
        END DO

c        print *, HINT1(71), RTRMS, RSCALE
c
c        print *, ytavgx, ytavgy, ytavgz, ytrmsx, ytrmsy, ytrmsz, 
c     &   rtrms
...
C
	BB=SQRT(BMIN**2+RANF(NSEED)*(BMAX**2-BMIN**2))
        BIMPACT=BB
	PHI=2.0*HIPR1(40)*RANF(NSEED)
	BBX=BB*COS(PHI)
	BBY=BB*SIN(PHI)
	HINT1(19)=BB
	HINT1(20)=PHI
...
C
	IF(IHPR2(3).NE.0) THEN
	   NHARD=1+INT(RANF(NSEED)*(NCOLT-1)+0.5)
...
C
	IF(IHPR2(9).EQ.1) THEN
		NMINI=1+INT(RANF(NSEED)*(NCOLT-1)+0.5)
...
C				already been produced here
	   XR1=-ALOG(EXP(-TTRIG)+RANF(NSEED)*(1.0-EXP(-TTRIG)))
 106	   NJET=NJET+1
	   XR1=XR1-ALOG(max(RANF(NSEED),1.0e-20))
	   IF(XR1.LT.TTRIG) GO TO 106
	   XR=0.0
 107	   NJET=NJET+1
	   XR=XR-ALOG(max(RANF(NSEED),1.0e-20))
...
110	XR=-ALOG(EXP(-TT)+RANF(NSEED)*(1.0-EXP(-TT)))
111	NJET=NJET+1
	XR=XR-ALOG(max(RANF(NSEED),1.0e-20))
...
C     DLW: don't turn off B's and D's decays, since GEANT can't do them
C	   CALL LUGIVE('MDCY(C411,1)=0;MDCY(C-411,1)=0')
C	   CALL LUGIVE('MDCY(C421,1)=0;MDCY(C-421,1)=0')
C	   CALL LUGIVE('MDCY(C431,1)=0;MDCY(C-431,1)=0')
C	   CALL LUGIVE('MDCY(C511,1)=0;MDCY(C-511,1)=0')
C	   CALL LUGIVE('MDCY(C521,1)=0;MDCY(C-521,1)=0')
C	   CALL LUGIVE('MDCY(C531,1)=0;MDCY(C-531,1)=0')
...
C			nucleon collisions.
	IF(IHNT2(1).GT.2) THEN
          CALL HIJWDS(IHNT2(1),1,RMAX)

C			********set up Wood-Sax distr for proj.
        ELSE IF (IHNT2(1) .EQ. 2) THEN
C
C         Return an arbitrary large value for deuteron
C
          RMAX = 20.0
	ENDIF
        HIPR1(34)=RMAX

	IF(IHNT2(3).GT.2) THEN
          CALL HIJWDS(IHNT2(3),2,RMAX)
C			********set up Wood-Sax distr for  targ.
        ELSE IF (IHNT2(3) .EQ. 2) THEN
C
C         Return an arbitrary large value for deuteron
C
          RMAX = 20.0
...
580		CONTINUE
		N=N+NTJ(JTP)
	ENDIF
	IF(IHPR2(1).GT.0.AND.RANF(NSEED).LE.HIDAT(3)) THEN
	     HIDAT20=HIDAT(2)
	     HIPR150=HIPR1(5)
...
1	CALL AR3JET(S,X1,X3,JL)
	...
	IF(HIDAT(2).GT.0.0) THEN
	   ...
	   IF(PTG.GT.HIDAT(2)) THEN
	      ...
	      IF(RANF(NSEED).GT.FMFACT) GO TO 1
	   ENDIF
	ENDIF
...
1     	IF(NTRY.EQ.5000) THEN
		...
		RETURN
      	...
      	XT2=A*(XT2M/A)**(RANF(NSEED)**(1./D))
     
      	YMAX=ALOG(.5/SQRT(XT2)+SQRT(.25/XT2-1.))
      	Y=(2.*RANF(NSEED)-1.)*YMAX
      	X1=1.-SQRT(XT2)*EXP(Y)
      	X3=1.-SQRT(XT2)*EXP(-Y)
      	X2=2.-X1-X3
      	NEG=0
...
C
	...
	I_INIRAD=0
	IF(RANF(NSEED).LE.PINIRAD) I_INIRAD=1
...
c 
c   --- modification by R. Seto Sep 12, 2011 to limit initial state radation
c    note that this is a multiplicative factor
c	WRITE(6,*) ' &&&&&&&&&&  parp(67) before ',PARP(67),' HIPR1(51)=',HIPR1(51),' &&&&&&&&&&&&'
        PARP(67)=HIPR1(51)
c	WRITE(6,*) ' &&&&&&&&&&  parp(67) after ',PARP(67),' &&&&&&&&&&&&'
c
c
        CKIN(9)=-10.0
        CKIN(10)=10.0
...
C		********mb=0.1*fm, YP is in fm,HIPR1(31) is in mb
     ...
     &			*ROMG(0.0))**2
		IF(RANF(NSEED).GT.GS/GS0) GO TO 40
		DO 30 K=1,5
			PSC1(K)=PP(JP,K)
			PSC2(K)=PP(I,K)
30		CONTINUE
...
C		********mb=0.1*fm, YP is in fm,HIPR1(31) is in mb
     ...
     &			*ROMG(R2)))**2
		GS0=(1.0-EXP(-(HIPR1(30)+HINT1(11))/HIPR1(31)/2.0
     &			*ROMG(0.0)))**2
		IF(RANF(NSEED).GT.GS/GS0) GO TO 70
		DO 60 K=1,5
			PSC1(K)=PT(JT,K)
			PSC2(K)=PT(I,K)
60		CONTINUE
...
C			the pt kick is the pt from hard-collision.
	I_SNG=0
	IF(IHPR2(13).NE.0 .AND. RANF(NSEED).LE.HIDAT(4)) I_SNG=1
...
C               ********decite whether to have single-diffractive
	IF(IHPR2(5).EQ.0) THEN
		PKC=HIPR1(2)*SQRT(-ALOG(1.0-RANF(NSEED)
	...
	IF(PKC.GT.HIPR1(20)) 
     ...
     &	       -RANF(NSEED)*(EXP(-HIPR1(20)**2/HIPR1(2)**2)-
     &	       EXP(-PKCMX**2/HIPR1(2)**2))))
C
	IF(I_SNG.EQ.1) PKC=0.65*SQRT(
     &		-ALOG(1.0-RANF(NSEED)*(1.0-EXP(-PKCMX**2/0.65**2))))
C			********select PT kick
30	PHI0=2.0*HIPR1(40)*RANF(NSEED)
	PKC11=PKC*SIN(PHI0)
	PKC12=PKC*COS(PHI0)
	PKC21=-PKC11
	PKC22=-PKC12
...
C			single-diffr 
	      IF(RANF(NSEED).GT.0.5.OR.(NFT(JT,5).GT.2.OR.
     &		      NTJ(JT).NE.0.OR.NFT(JT,10).NE.0)) THEN
		 D1=DPN
		 D2=DTX
		 NFP3=NFP(JP,3)
...
C		********have single diffractive collision
	...
	ELSE IF(SW.GT.MAX(SPDTX,SPXTD)+0.001 .AND.
     ...
     &         RANF(NSEED).GT.0.5).OR.(NPJ(JP).EQ.0
...
c*** this is identical to the statement following the next ELSE IF
     ...
     &       .AND.RANF(NSEED).GT.0.5).OR.(NPJ(JP).EQ.0
     &        .AND.NTJ(JT).NE.0)).AND.NFP(JP,5).LE.2) THEN
	      D1=DPN
	      D2=DTX
	      NFP3=NFP(JP,3)
...
C       so that pt=p1+p2
	...
	IF(ABS(NFT(JT,1)*NFT(JT,2)).GT.1000000.OR.
     &			ABS(NFT(JT,1)*NFT(JT,2)).LT.100) THEN
		KICKDIT=0
	ENDIF
	IF((KICKDIP.EQ.0.AND.RANF(NSEED).LT.0.5)
     &     .OR.(KICKDIP.NE.0.AND.RANF(NSEED)
...
C				********pt kick for proj
	PT(JT,1)=PT11-PKC21
	PT(JT,2)=PT12-PKC22
	IF((KICKDIT.EQ.0.AND.RANF(NSEED).LT.0.5)
     &     .OR.(KICKDIT.NE.0.AND.RANF(NSEED)
...
c
c BAC ONly swap the "current" entries in NFP/NFT and PP
c
          DO 620 JSB=1,15
            if (JSB .LT. 10 .OR. JSB .GT. 12) THEN
              PSB=PP(JP,JSB)
              PP(JP,JSB)=PT(JT,JSB)
              PT(JT,JSB)=PSB
            end if

            if (JSB .LT. 4 .or. JSB .EQ. 6 .or. JSB .EQ. 10) THEN
              NSB=NFP(JP,JSB)
...
C
	...
	IF(IOPT.EQ.2) THEN
     ...
     &			-RANF(NSEED)*(EXP(-HIPR1(8)**2/HIPR1(2)**2)-
     &			EXP(-PTMAX**2/HIPR1(2)**2))))

	ELSE
		PT=HIPR1(2)*SQRT(-ALOG(1.0-RANF(NSEED)*
...
C     			********FNORM is the normalize factor
        ...
        AVGRSQ = GAUSS1(R4WDSAX, XLOW, XHIGH, 0.001)
...
10	IF(JU-JL.GT.1) THEN
	...
	IF(J.GE.201) J=200
c
c BAC Fix problem with taking average of adjacent values 
c     instead of interpolating
c
...
C	This generate random number between XMIN and XMAX
	...
	RX=RR(I,JMIN)+(RR(I,JMAX)-RR(I,JMIN))*RANF(NSEED)
	JL=0
	JU=202
10	IF(JU-JL.GT.1) THEN
	   JM=(JU+JL)/2
...
c	HIRND2=(XX(I,J)+XX(I,J+1))/2.0
	RETURN
	END	
C
C
c     ================================================================
      Real Function HulthenMC(NSEED)
c     ================================================================
c
      Implicit None
      Integer NSEED
...
C BAC 12-Aug-2002
C
C         psi(r) = sqrt(a*b*(a+b)/2pi)/(b-a) [e^(-ar) - e^(-br)]/r
C
C         Note that when we sample the wave function we are actually
C         sampling 4pi r^2 psi^2(r)
C
C     Effective energies for Hulthen wave function parameters
C     
C      ka = sqrt(2 m_p Ea)
C      kb = sqrt(2 m_p Eb)
C
C      a = ka/hbar c = ka/hbar c
C
C      The below energies are in MeV. hbarc = 200 MeV fm
C
      Real EA, EB, RMNUCL, HULTHA, HULTHB, HULTHENNORM

      Parameter (EA = 2.2245)
      Parameter (EB = 59.8)
      Parameter (RMNUCL = 940.)
       
      Parameter (HULTHA = 1.0/4.38)
      Parameter (HULTHB = 1.0/0.85)
...
c
c     The trick to this sampling is that 4 pi r^2 psi^2(r) is
c
c     exp(-2ar) + exp(-2br) -2exp(-(a+b)r)
c
c     up to multiplicative constants.
c
c     Such a distribution results from convoluting three exponential
...
c     P3(r) = exp(-(a+b)r)
c
c     Since the convolution of the three probability distributions
c     gives 4 pi r^2 psi^2(r), we simply sample from each distribution
c     add the resulting r values and voila! Hulthen distributed r values.
c
c     Don't be fooled by the fact that each of the above probability 
c     distributions looks like one of the terms in 4 pi r^2 psi^2(r),
c     in general if you have p(x) = f(x) + g(x), the convolution of
c     f and g (f*g) is NOT the same as p(x). It just works out this
c     way (almost) for exponential distributions.
c
      ...
      r1 = -(0.5/HULTHA)*log(Pr1)
      r2 = -(0.5/HULTHB)*log(Pr2)
      r3 = -1.0/(HULTHA+HULTHB)*log(Pr3)

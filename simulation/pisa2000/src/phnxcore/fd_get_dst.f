*CMZ :  2.04/00 16/12/94  10.46.08  by  Charles F. Maguire
*CMZ :  2.01/00 05/10/92  11.19.41  by  Charles F. Maguire
*-- Author :
*-- Author :
      SUBROUTINE FD_GET_DST(READNXT,NUH,IUHEAD)
C  Inner device data strucure reading routine
      IMPLICIT NONE
      LOGICAL READNXT
...
C  accept the bank ('A')
        CALL FZIN(LUN_DINP,IXDIV_FE,LFD_PARA,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
     ...
     &     'Top bank name     ',LQF(LFD_PARA + 4)

      ELSE IF(IUHEAD(4) .EQ. U_CTOH4('PARU'))THEN
        CALL FZIN(LUN_DINP,IXDIV_FE,LFD_PARU,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
        CALL IQUEST_CHK(READNXT)
        WRITE(6,60)
     &     '# log. rec. read  ',IQUEST(2),

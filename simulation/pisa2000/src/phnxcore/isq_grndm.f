      INTEGER FUNCTION ISQ_GRNDM(NRNDM1, NRNDM2)

      implicit none
c
c     Function to return GEANT random number sequence value (1 to 215)
c
c     Original Author: C.F. Maguire (Vanderbilt), based on GRNDMQ routine
c     Creation date:   Feb. 17, 1998
c
c     Called by E_PUT_DST in PISA at start of run
c     Searches over 215 values of GEANT starting random number pairs
c     Will return -1 if starting pair is not found
c
c     Calling variables
c
      integer nrndm1     ! first random number seed
      integer nrndm2     ! second random number seed
c
c     Local variables
c
      integer loopSeq    ! loop variable over MAXSEQ

      integer maxseq
      PARAMETER  (MAXSEQ=215)

      INTEGER LSEQS(2,MAXSEQ)
...
c
      do loopSeq = 1, maxseq
         if (nrndm1.eq.lseqs(1,loopSeq).and.
     +       nrndm2.eq.lseqs(2,loopSeq))then
             isq_grndm = loopSeq
             return
         endif
      enddo

      isq_grndm = -1    ! starting pair was not found

*CMZ :  2.04/00 16/12/94  10.47.36  by  Charles F. Maguire
*CMZ :  2.01/00 05/10/92  11.19.41  by  Charles F. Maguire
*-- Author :
*-- Author :
      SUBROUTINE FN_GET_DST(READNXT,NUH,IUHEAD)
C  MUON ID
      IMPLICIT NONE
      LOGICAL READNXT
...
C  accept the bank ('A')
        CALL FZIN(LUN_DINP,IXDIV_FE,LFN_PARA,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
     ...
     &     'Top bank name     ',LQF(LFN_PARA + 4)
      ELSE IF(IUHEAD(4) .EQ. U_CTOH4('PARU'))THEN
        CALL FZIN(LUN_DINP,IXDIV_FE,LFN_PARU,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
        CALL IQUEST_CHK(READNXT)
        WRITE(6,60)
     &     '# log. rec. read  ',IQUEST(2),

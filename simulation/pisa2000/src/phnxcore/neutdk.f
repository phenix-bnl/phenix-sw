      SUBROUTINE NEUTDK(PTOT,AMASS,P1,PD1,PD2)
      IMPLICIT NONE
...
C    Purpose: DECAYS THE NEUTRAL MASS INTO TWO PHOTONS
...
c     begin execution
      ...
      CALL GDECA2(AMASS,PHOTMASS,PHOTMASS,PCM)
...
c     pair mass check for decay photons
         ...
         py1 = pd1(2)
         pz1 = pd1(3)
         et1 = sqrt(px1*px1 + py1*py1 + pz1*pz1)  ! assume photon, no mass
         px2 = pd2(1)
         py2 = pd2(2)
         pz2 = pd2(3)
         et2 = sqrt(px2*px2 + py2*py2 + pz2*pz2)  ! assume photon, no mass
         pxsum = px1 + px2
         pysum = py1 + py2
         pzsum = pz1 + pz2

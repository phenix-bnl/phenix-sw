c     $Id: rqmd_evt.f,v 1.3 2008/05/21 08:22:13 hpereira Exp $
      integer function rqmd_evt()
      implicit none

c     Descriptions:
c     Routine to read a complete RQMD event from a file and
...
c     it appears that read_rqmd doesn't return a
...
c     Read in new event from RQMD data file
      call event_filter(ireturn)
      if (ireturn.eq.-1)then
         status = .true.
      else
         write(*, '(a, i4)') 'rqmd_evt - ireturn = ',ireturn
         status = .false.
         rqmd_evt = 0
         go to 500
      endif
...
c     Copy RQMD arrays into the arrays that GUEVGEN expects (viz. PPTOT, 
C     IDTOT)
...
C     case -- JPS 24 Jan 96.
 ...
 501  format ( ' rqmd_evt - ievent,xyz vertex', i5,3f10.4)
      rqmd_evt = -1
 500  continue
       
      if (.not.status) write(6,*) ' rqmd_evt - reading encountered ',
     &  'eof or error condition'
      chevt_name = 'rqmd source'
      return
      end

c     $Id: venus_evt.f,v 1.3 2008/05/21 08:22:14 hpereira Exp $
      integer function venus_evt ()
      implicit none
      
c     Descriptions:
c     Routine to read a complete VENUS event from a file and
...
c     Read in new event from VENUS data file
      call event_filter(ireturn)
      if (ireturn.eq.-1)then
         status = .true.
      else
         write(*, '(a, i4)') 'venus_evt - ireturn = ',ireturn
         status = .false.
         VENUS_evt = 0
         Go to 500
      endif
...
c     Copy VENUS arrays into the arrays that GUEVGEN expects (viz. PPTOT, 
C     IDTOT)
c     =======================================================================
     ...
     &    ' venus_evt - keeping ', mxtot, ' particles',
     &    ' of true venus event ', numevt
              
        write (6,'(/,1x,a,i7,a)')
     &    ' venus_evt - there were actually ',nptls,
...
C     case -- JPS 24 Jan 96.
 ...
 501  format (' venus_evt - ievent,xyz vertex', i5,3f10.4)
      venus_evt = -1
 500  continue
 
      if (.not.status) write(6,*) ' venus_evt - reading encountered ',
     &  'eof or error condition'
      chevt_name = 'venus source'

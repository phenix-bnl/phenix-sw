*CMZ :  2.04/00 23/11/94  14.14.04  by  Charles F. Maguire
*CMZ :  2.01/00 05/10/92  11.19.41  by  Charles F. Maguire
*-- Author : S.R. Tonse
      SUBROUTINE FV_GET_DST(READNXT,NUH,IUHEAD)
C  VERTEX
      IMPLICIT NONE
      LOGICAL READNXT
...
C  accept the bank ('A')
        CALL FZIN(LUN_DINP,IXDIV_FE,LFV_PARA,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
     ...
     &     'Top bank name     ',LQF(LFV_PARA + 4)
      ELSE IF(IUHEAD(4) .EQ. U_CTOH4('PARU'))THEN
        CALL FZIN(LUN_DINP,IXDIV_FE,LFV_PARU,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
        CALL IQUEST_CHK(READNXT)
        WRITE(6,60)
     &     '# log. rec. read  ',IQUEST(2),

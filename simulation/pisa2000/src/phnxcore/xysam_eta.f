...
*-- Author :
*-- Author :
        subroutine xysam_eta
        implicit none
c
c       subroutine to initialize eta Dalitz decay
c
        real xs, ys, elemas, etamas, xsam, xmin, delx
...
c	
c
        xmin=2.0*elemas/etamas
        delx=(1.0-xmin)/10000.0	          ! more than 1.0e+04  
        ymax=sqrt(1.0-4.0*(elemas/etamas)*(elemas/etamas))
        do ix=1,10000                     ! more than 1.0e+04  
          xs=xmin+(float(ix)-0.5)*delx    ! x/mu 
          xsam=xs*etamas                  ! virtual photon mass

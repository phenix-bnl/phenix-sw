*CMZU:  2.04/00 27/07/93  23.23.43  by  Charles F. Maguire
*CMZ :  2.02/00 05/06/93  05.26.54  by  Charles F. Maguire
*-- Author :    Charles F. Maguire   13/12/92
      SUBROUTINE RV_JPSI2(PTOT,IEOM,IJOP,P1,PD1,PD2)
      IMPLICIT NONE
C
C    DECAYS THE J/PSI MASS INTO TWO ELECTRONS, RETURNS DAUGHTER MOMENTA PD1,PD2
C    DECAYS THE PSI' MASS INTO TWO ELECTRONS, RETURNS DAUGHTER MOMENTA PD1,PD2
C
c
c    Based on RV_PHI2 subroutine
c
c     Subroutine arguments:
c          PTOT: total momentum of parent (input)
c          IEOM: switch for muon decay (=0) or electron decay (=1)  (input)
c          IJOP: switch for J/PSI (=1) or PSI-PRIME (=2)   (input)
...
c
      ...
      REAL PD2(3)
      REAL AMASS1
      PARAMETER (AMASS1 = 3.096)
      REAL AMASS2
      PARAMETER (AMASS2 = 3.685)
      REAL AMASS
      REAL BETA(4)
      REAL PCM(4,3)
      REAL GKIN(4,2)
...
c
      ...
      ELSE
         AMASS = AMASS2
      ENDIF
      CALL GDECA2(AMASS,DMASS,DMASS,PCM)

...
*-- Author :
*-- Author :
      SUBROUTINE FE_GET_DST(READNXT,NUH,IUHEAD)
C  EM calo data strucure reading routine
      IMPLICIT NONE
      LOGICAL READNXT
...
C  accept the bank ('A')
        CALL FZIN(LUN_DINP,IXDIV_FE,LFE_PARA,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
     ...
     &     'Top bank name     ',LQF(LFE_PARA + 4)
      ELSE IF(IUHEAD(4) .EQ. U_CTOH4('PARU'))THEN
        CALL FZIN(LUN_DINP,IXDIV_FE,LFE_PARU,1,'A',NUH,IUHEAD)
C  check that FZIN read went OK. Return READNXT false if not OK
        CALL IQUEST_CHK(READNXT)
        WRITE(6,60)
     &     '# log. rec. read  ',IQUEST(2),

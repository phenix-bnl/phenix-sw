        subroutine pdalitz(pizmom, phomom, elemom, posmom)
...
c       PIZMOM  Lab four momentum vector for the PI Zero (input)
...
c       POSMOM  Lab four momentum vector for the Positron (output)
        ...
        real pizmom(4), phomom(4), elemom(4), posmom(4), ptemp

#include "gckine.inc"
#include "gcflag.inc"
#include "subevt.inc"

        real xsam, ysam, pizmas, virtm, evirt, pvirt
        real costh, sinth, cosph, sinph, phi, elemas
        real pcmv(4), pcmp(4), eele, epos, pele, ppos
        real pcmele(4), pcmpos(4), beta(4), ppiz, epiz, xsm2, ysm2
...
c       calculate the first decay product in the center-of-mass system
        ...
        pvirt=pizmas*(1.0-xsam*xsam)/2.0
...
c       first take frame along  the pizero lab momentum
c       then rotate to the true lab frame

        ppiz=sqrt(pizmom(1)*pizmom(1)+pizmom(2)*pizmom(2)+
     1                  pizmom(3)*pizmom(3))
        epiz=sqrt(ppiz*ppiz+pizmas*pizmas)
        beta(1)=0.0
        beta(2)=0.0
        if(epiz.eq.0.0.or.ppiz.eq.0.0)then
                write(6,635) ntent,epiz,ppiz
635     format('pdalitz - ntent, epiz, ppiz ',i5,2e14.5)
        else
                beta(3)=-ppiz/epiz
                costh=pizmom(3)/ppiz
        endif
        ...
        if(sinth.ne.0.0.and.ppiz.ne.0.0)then
                cosph=pizmom(1)/(sinth*ppiz)
                sinph=pizmom(2)/(sinth*ppiz)
        else
                sinph=0.0
                cosph=1.0
        endif

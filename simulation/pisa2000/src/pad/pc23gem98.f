c $Id: pc23gem98.f,v 1.6 2008/05/21 08:22:03 hpereira Exp $

      SUBROUTINE PC23GEM98(NMPD23)
*       ============================
...
c    C.F. Maguire   May  11, 2002   Add parameter to increase the Z = 0 gap
c                                   of PC2 and PC3.  The changes in 1998
c                                   were done before PC2/PC3 were built.  The
c                                   as-built version had a gap 3.3 cm larger
c                                   than the one used in the 1998 simulation
c                                   model.  It is not clear which of the
c                                   frame parts of PC2/PC3 need adjusting.

c                                   Decide to increase IBAR2DZ and TBAR2DZ by
c                                   0.4 cm each in order to get extra 3.2 cm

c                                   The active extent of PC2 and PC3 are also
c                                   slightly larger in the as-built version.
c                                   Therefore the hardwired PC2DZGAS and PC3DZGAS
c                                   are moved to the phnx.par file from the
c                                   source code.

c                                   These changes are supposed to get the active
c                                   gas volume of PC2 and PC3 close to the as-built
c                                   gas volume which is used in the real PRDFs analysis

c                                   Some of the passive volumes may be inaccurate
...
*             || || PIXEL/CHEVRON   ||   |
*             || ||     (1.50")     ||   |
*             |__||_________________||___   493.2632 cm
*             |--------------------------:  493.2124 cm
*             |  ||  . GAS GAP (12 MM). . .    492.6124 cm  <- wire plane
*             |__||______________________|  492.0124 cm
*             |---------------------||---|  491.9362 cm
*             |  ||                 ||
*             |  ||     GROUND      ||   :
*             |  ||   CATHODE(1.50")||  
*             |__||_________________||___|  488.1262 cm 
*             |---------------------|       488.0500 cm
*              \                    . '  
*               \               . `   |
*                \          . `       |
...
* pc2dzext = 336.62,  ! Maximum Z extent of PC2 (change from 333.32 by CFM, May 2002)
* pc3dzext = 389.39,  ! Maximum Z extent of PC3 (change from 383.086 by CFM, May 2002)
...
C.. Namelist parameters:
      ...
      INTEGER PC23NCHMB
      integer pc2arm0, pc2arm1, pc3arm0, pc3arm1
      integer pc2active, pc3active
      REAL PC23TETGT
...
C..  Local Variables:
      ...
      CHARACTER*4 VPC2Z0SP(9), VPC3Z0SP(9)
...
CAAR  volume names for central support which are in the phnx.par file
      ...
      logical ibeamtf
      real endsupdx /.4318/, endsuptdx /1.524/
      real endsuptdy /.7366/, endsupdy
      real redangle /21.92/      !extent of coverage in x. Calculated
                                 !from radius and angular coverage

      real pc2dzgas /314.07/      ! CFM May 2002, change of the active extent of PC2 (had been 304.95)
      real pc3dzgas /366.057/     ! CFM May 2002, change of the active extent of PC3 (had been 357.38)
...
C     Central support parameters
      ...
      real GZCNTR  ! changed from integer to real (CFM, 12/15/99)
CAAR

C.. Read PC23 parameters through Namelist /PC23_PAR/

      NAMELIST / PC23_PAR/ pc2arm0, pc2arm1, pc3arm0, pc3arm1,
     + PC23NCHMB, PC23TETGT, PC2DZEXT, PC3DZEXT,
...
c     now store the PC2/PC3 parameters for later use in PISORP
      ...
      iqf(lfp_pd23 + 2) = 19981225        ! date key for PC23 geometry


C.. Read the geometry file segment
      
      write( *,* ) 'pc23gem98 - reading parameter from common interface'
      rewind(itf_lun)
...
C     Reset tan to give some space in between layers
      
      TANG  = TAN( redangle*DEGRAD/2. )  ! tangent of 11.25 degrees
...
caar Temporary Area
      ...
      gap = (tbar2dz+ibar2dz+.1)*4.-.2 ! adding in extra 3.2 cm from change in tbar2dz and ibar2dz
...
C.. 3a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBCLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 3b) Left side bar (air, the inside of the tubing)
      ...
      DX = (PC23SBAR-2.*PC23SBTH)/(2.)
      DY = (PC2CTHEX-2.*PC23SBTH)/(2.)
      DZ = PC2DZEXT/2.
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBCLI(L),'BOX ', 16, Par, 3, IST)
...
cAAR
     ...
     >       PC23CBAR/2.)/2.
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC2CTHXL(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
cAAR
     ...
     >       PC23CBAR/2.)/2.
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC2CTHXR(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
C.. 3g) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBCRO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 3h) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBCRI(L),'BOX ', 16, Par, 3, IST)
...
C.. 5a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBWLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 5b) Left side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBWLI(L),'BOX ', 16, Par, 3, IST)
...
c     CFM: revision for 699 medium number for PC3
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC2GASGP(L),'BOX ',MED_PC_GAS,
     +                Par, 3, IST)
...
C.. 5d) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBWRO(L), 'BOX ', MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 5e) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBWRI(L),'BOX ', 16, Par, 3, IST)
...
C.. 7a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBGLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 7b) Left side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBGLI(L),'BOX ', 16, Par, 3, IST)
...
caar
      dz=(PC2DZEXT-gap2)/4.
      DX = ((PC2RINST)*TANG-PC23SBAR-PC23CBAR/2.)/2.
      DY = PC2GRHEX/2.
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC2GRHXL(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
caar
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC2GRHXR(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
C.. 7g) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBGRO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 7h) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC2SBGRI(L),'BOX ', 16, Par, 3, IST)
...
c         parameters come from 
      ...
      dz   =  sbardz/2.
      DX   =  ((PC2RINST)*TANG-PC23SBAR-PC23CBAR/2.)
      DY   =  sbardy/2.
      ...
      call gsvolu (VPC2Z0SP(1),'BOX ',med_s2, Par, 3 , IST)
      ...
      call gsvolu (VPC2Z0SP(8),'BOX ',med_s2, Par, 3 , IST)
...
c     9b) Hollow part of pipes
      ...
      call gsvolu(VPC2Z0SP(2),'BOX ',16, Par, 3 , IST)
...
c     9c) End Bar
      ...
      call gsvolu(VPC2Z0SP(3),'BOX ',med_s2, Par, 3 , IST)
...
c                   tall bar
      ...
      call gsvolu(VPC2Z0SP(5),'BOX ',med_s2, Par, 3 , IST)
...
c                 cross bar
      ...
      call gsvolu(VPC2Z0SP(4),'BOX ',med_s2, Par, 3 , IST)
...
c           'cross' pieces
      ...
      call gsvolu(VPC2Z0SP(6),'BOX ',med_alu, Par, 3 , IST)
...
c     central dividing beam
      ...
      call gsvolu(VPC2Z0SP(7),'BOX ',med_alu, Par, 3 , IST)
...
c     9f)  mounting frame
      ...
      call gsvolu(VPC2Z0SP(9),'BOX ', med_alu, Par, 3, ist)
      call gsatt(VPC2Z0SP(9),'SEEN',1)      
      call gsatt(VPC2Z0SP(9),'COLO',col_hexcell)   
...
C.. 2a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBGLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 2b) Left side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBGLI(L),'BOX ', 16, Par, 3, IST)
...
caar      DZ = PC3DZEXT/2. 
      DZ = (PC3DZEXT-gap2)/4. 
      DX = ((PC3RINST)*TANG-PC23SBAR-PC23CBAR/2.)/2.
      DY = PC3GRHEX/2.
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC3GRHXL(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
caar      DZ = PC3DZEXT/2. 
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC3GRHXR(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
C.. 2g) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBGRO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 2h) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBGRI(L),'BOX ', 16, Par, 3, IST)
...
C.. 4a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBWLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 4b) Left side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBWLI(L),'BOX ', 16, Par, 3, IST)
...
caar      DZ = PC3DZGAS/2. 
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC3GASGP(L),'BOX ',MED_PC_GAS + 5,
     +                Par, 3, IST)
...
C.. 4d) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBWRO(L), 'BOX ', MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 4e) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBWRI(L),'BOX ', 16, Par, 3, IST)
...
C.. 6a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBCLO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 6b) Left side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBCLI(L),'BOX ', 16, Par, 3, IST)
...
caar      DZ = PC3DZEXT/2. 
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC3CTHXL(L),'BOX ',MED_HEXCELL,Par,3,IST)
...
caar      DZ = PC3DZEXT/2. 
      ...
      DO L = 1, PC23NCHMB
         CALL GSVOLU (VPC3CTHXR(L),'BOX ',MED_HEXCELL, Par, 3, IST)
...
C.. 6g) Right side bar (The walls of carbon-fiber parallelepiped tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBCRO(L),'BOX ',MED_CARBON_EPOXY, Par, 3, IST)
...
C.. 6h) Right side bar (air, the inside of the tubing)
      ...
      DO L = 1, PC23NCHMB
       CALL GSVOLU (VPC3SBCRI(L),'BOX ', 16, Par, 3, IST)
...
c      TL1  = BL1+PC2GRHEX*TANG/2.
      dx  = ((PC3RINST)*TANG-PC23SBAR-PC23CBAR/2.)

      Par( 1) = DX
      Par( 2) = DY
      Par( 3) = DZ

      call gsvolu (VPC3Z0SP(1),'BOX ',med_s2, Par, 3 , IST)
...
c      TL1  = BL1+PC2GRHEX*TANG/2.
      ...
      call gsvolu (VPC3Z0SP(8),'BOX ',med_s2, Par, 3 , IST)
      call gsatt (VPC3Z0SP(8),'SEEN',1)

c     9b) Hollow part of pipes

      dz=sbardz/2-hollow/2.
      dy  = sbardy/2-hollow+.38
      dx  = ((PC3RINST)*TANG-PC23SBAR-PC23CBAR/2.)
      ...
      call gsvolu(VPC3Z0SP(2),'BOX ',16, Par, 3 , IST)
...
c     9c) End Bar
      ...
      dz=enbdz/2
      dy  = sbardy/2+.38
      dx =  ((PC3RINST)*TANG-PC23SBAR-PC23CBAR/2.)  !use pre BL1 and TL1

      Par( 1) = DX
      Par( 2) = DY
      Par( 3) = DZ

      call gsvolu(VPC3Z0SP(3),'BOX ',med_s2, Par, 3 , IST)
...
c                   tall bar
      ...
      call gsvolu(VPC3Z0SP(5),'BOX ',med_s2, Par, 3 , IST)
...
c                 cross bar
      ...
      call gsvolu(VPC3Z0SP(4),'BOX ',med_s2, Par, 3 , IST)
...
c           'cross' pieces
      ...
      call gsvolu(VPC3Z0SP(6),'BOX ',med_alu, Par, 3 , IST)
...
c     central dividing beam
      ...
      call gsvolu(VPC3Z0SP(7),'BOX ',med_alu, Par, 3 , IST)
...
c     9f)  mounting frame
      ...
      call gsvolu(VPC3Z0SP(9),'BOX ', med_alu, Par, 3, ist)
      call gsatt(VPC3Z0SP(9),'SEEN',1)      
      call gsatt(VPC3Z0SP(9),'COLO',col_hexcell) 
...
c     Check to see if a PC section is active
        pc2active = 1
        pc3active = 1
        if (pc2arm0.eq.0.and.j.gt.pc23nchmb/2) then
           pc2active = 0
        endif
        if (pc2arm1.eq.0.and.j.le.pc23nchmb/2) then
           pc2active = 0
        endif
        if (pc3arm0.eq.0.and.j.gt.pc23nchmb/2) then
           pc3active = 0
        endif
        if (pc3arm1.eq.0.and.j.le.pc23nchmb/2) then
           pc3active = 0
        endif

C.. ----------- P O S I T I O N    P C 2   D E T E C T O R ------------ ..C

        if (pc2active.eq.1) then
...
C.. Positioning PC2 "shell" volumes 
        ...
        if(j.le.4)then
           CALL GSPOS(VPC2SHELL(J),1,ePC2,XCNTP1,YCNTP1,ZCNTR,
     +                IRTTR(J),'ONLY' )
        else
           CALL GSPOS(VPC2SHELL(J),1,wPC2,XCNTP1,YCNTP1,ZCNTR,
     +                IRTTR(J),'ONLY' )
        endif
...
caar  Temporary area
        ...
        GZCNTR=0.  ! actually a real, not an integer (CFM 12/15/99)
...
C.. 3c) HEXCELL sheet 1.25" thick (left side)
        ...
        XCNTP1 = -((PC2RINST)*TANG-
     >              PC23SBAR-PC23CBAR/2.)/2.-PC23CBAR/2.  
        YCNTP1 = -DHPC2/2.+PC2ELECT+PC2CTSHT+PC2CTHEX/2.
...
C.. 5a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
        ...
        XCNTP1 = -(PC2RINST)*TANG+PC23SBAR/(2.)  
...
c     9f)End bar
           ...
           ZCNTR  =0.
           XCNTP1 =PC2RINST*TANG + endsupdx +.3   !add a three milimeter spacing 
                                                  !for saftey
           YCNTP1 =0.
          
           call gspos('PCM1',1,VPC2SHELL(J),XCNTP1,YCNTP1,
     +          ZCNTR,0,'ONLY')
           ...
           call gspos('PCM1',2,VPC2SHELL(J),XCNTP1,YCNTP1,
     +          ZCNTR,0,'ONLY')

        endif                   ! pc2active check
...
C.. Positioning PC3 "shell" volumes 
        ...
        if(j.le.4)then
           CALL GSPOS(VPC3SHELL(J),1,ePC3,XCNTP1,YCNTP1,ZCNTR,
     +                IRTTR(J),'ONLY' )
        else
           CALL GSPOS(VPC3SHELL(J),1,wPC3,XCNTP1,YCNTP1,ZCNTR,
     +                IRTTR(J),'ONLY' )
        endif
caar  Temporary area
...
C.. 6a) Left side bar (The walls of carbon-fiber parallelepiped tubing)
        ...
        XCNTP1 = -(PC3RINST+2.)*TANG+PC23SBAR/(2.)  
...
C.. 6c) HEXCELL sheet 1.5" thick (left side)
        ...
        XCNTP1 = -((PC3RINST)*TANG-PC23SBAR/COSINE-PC23CBAR/2.)/2.-
     >            PC23CBAR/2.  
...
C.. 6g) Right side bar (The walls of carbon-fiber parallelepiped tubing)
        ...
        XCNTP1 = (PC3RINST)*TANG-PC23SBAR/(2.)  
        YCNTP1 = -DHPC3/2.+2.*PC3GRSHT+PC3GRHEX+PC3GASGP+PC3CTSHT+
     >             PC3CTHEX/2.
...
c     9f)End bar
           ...
           ZCNTR  =0.
           XCNTP1 =PC3RINST*TANG + endsupdx  !add a three milimeter spacing 
                                             !for saftey
           YCNTP1 =0.
          
           call gspos('PCM2',1,VPC3SHELL(J),XCNTP1,YCNTP1,
     +          ZCNTR,0,'ONLY')
           ...
           call gspos('PCM2',2,VPC3SHELL(J),XCNTP1,YCNTP1,
     +          ZCNTR,0,'ONLY')
...
C.. -------------------- END OF DETECTOR GEOMETRY SETUP ------------------ 
 ...
 9999 continue
      write(*,*) 'pc23gem98 - Geometry date stamp: May. 11, 2002' 
      return
  999 continue
      write(6,1000)
 1000  format(/'pc23gem98 - read error in pc23_par segment'/,3x, 
     ...
     +  ' line',/,3x,'****This will cause the program to crash.****',//)
      ...
      stop 'pc23gem98 - stop because of geometry error.'
      end

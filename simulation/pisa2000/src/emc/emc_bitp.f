      SUBROUTINE EMC_BITP(i1, iwall, itype, dele, pos_x,
...
c     minor modifications on Charlie's code (emc_bit_pack.f)
c     and putting limits and stepsizes into a common
c     emcbit.inc so that they can be passed to PISORP via UDETPAR
c     Instead of being PARAMETERS, the limits, etc. are now
c     coming form the COMMON
c
...
cgd***      parameter (r_min_sc = 510.0)
cgd***      parameter (r_max_sc = 560.0)
      real r_min_gl, r_max_gl  ! For Pb Glass
cgd***      parameter (r_min_gl = 565.0)
cgd***      parameter (r_max_gl = 615.0)
      real  r_step         ! Step size in R
cgd***      parameter (r_step = 0.05) ! corresponds to 10 bits
      integer iword_r

      real z_max, z_min    ! Max and min Z values
cgd***      parameter (z_max = 200.0)
cgd***      parameter (z_min = -200.0)
      real z_step               ! Step size in Z coordinate
cgd***      parameter (z_step = 0.10) ! corresponds to 12 bits
      integer iword_z

      real x_max, x_min    ! Max and min X values (one side, as per wall)
      real x_max_sc, x_min_sc      ! Pb Scint
cgd***      parameter (x_max_sc = 560.0) ! Corresponds to 0 degrees PHI
cgd***      parameter (x_min_sc = 280.0) ! Corresponds to 56.25 degrees PHI
      real x_max_gl, x_min_gl      ! Pb Glass
cgd***      parameter (x_max_gl = 610.0) ! Corresponds to 0 degrees PHI
cgd***      parameter (x_min_gl = 490.0) ! Corresponds to 56.25 degrees PHI
      real x_step               ! Step size in X coordinate
cgd***      parameter (x_step = 0.07) ! Corresponds to 12 bits
...
cgd***      parameter (dele_max_sc = 0.0327) ! corresponds to 15 bits at 1 keV
      real dele_max_gl          ! Pb Glass
cgd***      parameter (dele_max_gl = 0.256) ! correspond to 15 bits at 8 keV
cgd***      parameter (dele_min = 0.0)
      real dele_step       ! Step size in DELE
      real dele_step_sc    ! Step size in DELE for Pb Scint
cgd***      parameter (dele_step_sc = 1.0e-6) ! 1 keV
      real dele_step_gl    ! Step size in DELE for Pb Glass
cgd***      parameter (dele_step_gl = 8.0e-6) ! 8 keV
      integer iword_dele

      real tof_max, tof_min      ! Max and min TOF value (ns)
cgd***      parameter (tof_min = 17.0) ! Corresponds to 510 cm position
cgd***      parameter (tof_max = 345.) ! corresponds to 15 bits
      real tof_step       ! Step size in TOF (as in PISA)
cgd***      parameter (tof_step = 0.010) ! 10 ps
      integer iword_tof

      integer iwall_max
cgd***      parameter (iwall_max = 8)
      integer iword_iwall

      integer itype_max
cgd***      parameter (itype_max = 3)
      integer iword_itype

      integer i1_max
cgd***      parameter (i1_max = 8)
      integer iword_i1

      integer ind1_max              ! According to wall
      integer ind1_max_sc           ! Pb Scint
cgd***      parameter (ind1_max_sc = 18)
      integer ind1_max_gl           ! Pb Glass
cgd***      parameter (ind1_max_gl = 192)
      integer iword_ind1

      integer ind2_max              ! According to wall
      integer ind2_max_sc           ! Pb Scint
cgd***      parameter (ind2_max_sc = 144)
      integer ind2_max_gl           ! Pb Glass
cgd***      parameter (ind2_max_gl = 24)
      integer iword_ind2

      integer itrack_max
cgd***      parameter (itrack_max = 32767)  ! corresponds to 15 bits
      integer iword_itrack

      integer spart_max
cgd***      parameter (spart_max = 62)      ! normal particles
      integer iword_spart

      integer ncycle_max
cgd***      parameter (ncycle_max = 31)
...
c
      IF(l_first) THEN
         l_first = .false.
c
c        Initialize limits and stepsizes
c
         r_min_sc       = emc_r_min_sc
         r_max_sc       = emc_r_max_sc
         r_step         = emc_r_step
         r_min_gl       = emc_r_min_gl
         r_max_gl       = emc_r_max_gl
         z_min          = emc_z_min
         z_max          = emc_z_max
         z_step         = emc_z_step
         x_min_sc       = emc_x_min_sc
         x_max_sc       = emc_x_max_sc
         x_step         = emc_x_step
         x_min_gl       = emc_x_min_gl
         x_max_gl       = emc_x_max_gl
         dele_max_sc    = emc_dele_max_sc
         dele_step_sc   = emc_dele_step_sc
         dele_max_gl    = emc_dele_max_gl
         dele_step_gl   = emc_dele_step_gl
         tof_min        = emc_tof_min
         tof_max        = emc_tof_max
         tof_step       = emc_tof_step
         iwall_max      = emc_iwall_max
         itype_max      = emc_itype_max
         i1_max         = emc_i1_max
         ind1_max_sc    = emc_ind1_max_sc
         ind2_max_sc    = emc_ind2_max_sc
         ind1_max_gl    = emc_ind1_max_gl
         ind2_max_gl    = emc_ind2_max_gl
         itrack_max     = emc_itrack_max
         spart_max      = emc_spart_max
         ncycle_max     = emc_ncycle_max
      ENDIF
c
      iword1 = 0 ! initialize
c

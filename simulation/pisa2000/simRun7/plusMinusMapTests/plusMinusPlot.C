void plusMinusPlot(int iOption=1, const Long64_t maxEntries=0,
		   const float minMomentum=0.3, const float maxMomentum=5.0,
		   const float sigmaPC3Cut=3.0) {

  //
  // Macro to study the reconstruction using the +- Run 7 field map
  //
  // Input arguments
  // iOption  plotting choice
  // maxEntries  number of entries of input evalPlusMinus.root NTUPLE which will be processed
  ...
  //
  TFile *f = new TFile("evalPlusMinus.root");
  if (!f) {
    cerr << "\n Unable to evalPlusMinus.root file" << endl;
    exit (1);
  }
  ...
  if(iOption < 3) {
    ...
    westPlusResolution->SetMaximum(195.);
  }
  westPlusResolution->Fit("gaus","","", -0.04, +0.04);
  westPlusResolution->Draw();

  if(iOption==1 || iOption==2) {
    TLatex *t0 = new TLatex(-0.080, 120, "#phi #rightarrow K^{+}K^{-}");
    t0->SetTextAlign(12);
  ...
  if(iOption==3 || iOption==4) {
    TLatex *t0 = new TLatex(-0.080, 120, "#phi #rightarrow e^{+}e^{-}");
    t0->SetTextAlign(12);
    t0->SetTextColor(kBlack);
    t0->SetTextSize(0.07);
    t0->Draw();
  ...
  TLatex *t1 = new TLatex(-0.085, 100, "Variable p_{T}");
  t1->SetTextAlign(12);
  ...
  if(iOption==1 || iOption==3) {
    TLatex *t2 = new TLatex(-0.09, 80, "Fixed Vertex Z=0");
    t2->SetTextAlign(12);
  ...
  if(iOption==2 || iOption==4) {
    TLatex *t2 = new TLatex(-0.095, 80, "-30<Z Vertex<+30 cm");
    t2->SetTextAlign(12);
    t2->SetTextColor(kBlack);
    t2->SetTextSize(0.07);
    t2->Draw();
  ...
  TLatex *t3 = new TLatex(-0.09, 60, "+- Magnetic Field");
  t3->SetTextAlign(12);
  t3->SetTextColor(kBlack);
  t3->SetTextSize(0.07);
  t3->Draw();

  TLatex *t4 = new TLatex(-0.09, 40, "January 10, 2007");
  t4->SetTextAlign(12);
  ...
  if(hbdAbsent) {
    TLatex *t5 = new TLatex(-0.080, 140, "HBD Absent");
    t5->SetTextAlign(12);

...
#
# Usage:  perl -w project93DSTPiPlus++.pl >& project93DSTPiPlus++.log & (change name of script as required)
...
#
$myName = "maguire";
$topName = "/scratch/$myName/0/project93/dstEvents/PiPlus/PosField/";  # top directory for this project's files
...
$eventsPerRun = 10000;  # number of events per run (change as required)
$nCycle = 4; # 4 cycles totaling 100 jobs (100*100000 = 10,000,000 events)

$oscProject = 93;   # project number (change as required for this project)
$idBase = 95251;    # base PISA ID number for this event file  (change as required for this project, one less than first PISA run number)

#
# Set up parts of input and output file names
# Note that the PISA hits input files and the DST output files are located in the /gpfs2 disk 
#
$pisaDirectory = "/gpfs2/RUN6PRDF/Run5/$myName/project93/pisaEvents/"; # (change as required)
$dstDirectory = "/gpfs2/RUN6PRDF/Run5/$myName/project93/dstEvents/"; # (change as required)
$cntNameFirst = "CNT2000_PiPlus++-00000";     # (change as required for the CNT nanoDST)
$dstNameFirst = "DST2000_PiPlus++-00000";     # (change as required for the DST)
$oscNameFirst = "PISA2000_PiPlus++-00000";    # (change as required for the PISA hits file name)
$oscNameLast = "-00$oscProject.rootg";            # (change as required for the PISA hits file name)

#
# write the host name in a text file
#
`echo \$HOST  > $topName/dstHostPiPlus++.log`;

#
# No need to change anything else below except the PBS wall clock and CPU times
#
$wallClockTime = "4:00:00"; # in hours:minutes:seconds, change as required
$cpuTime = "3:59:30";  # in hours:minutes:seconds, change as required and less than wallClockTime
...
for($number=$numberStart; $number<=$numberEnd; $number++) {
  ...
  for($iCycle=$iCycleStart; $iCycle<$nCycle; $iCycle++) {
    ...
    print FILE "pisaToDST95252.pbs:unsetenv ROOTSYS; unsetenv OFFLINE_MAIN; source /home/phenix/phnxVandy_setup.csh pro.74\n";
    print FILE "echo \$LD_LIBRARY_PATH > libPath$iCycleNumber\n";
    ...
    print FILE "cp /tmp/$myName$iCycleNumber/$cntDSTName $dstDirectory\n";
    print FILE "cp /tmp/$myName$iCycleNumber/$dstDSTName $dstDirectory\n";

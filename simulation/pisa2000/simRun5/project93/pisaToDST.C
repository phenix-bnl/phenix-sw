void pisaToDST(Int_t nEvents=10, 
	       ...
	       char *hwgout="simHWG.root",     // HWG is not used in Project 93
               ...
               const Int_t debugFlag=0)
{
  ...
  //
  // Version for Run5, assumes that there is no AFS connection
  //                   all the needed subsystem files should be made locally available 
  //
  ...
  // Only a simulated nanoDST is produced on output

  //
  // Original author: Charles F. Maguire (charles.f.maguire@vanderbilt.edu)
  // Creation date: May 20, 2006 (based on Run4a pisaToDSTNew.C, so many Fun4All versions so little time)
  //
  ...
  //
  rc->set_IntFlag("DCHREQFLAG", dchReqHits);   // default setting is 3 Drift Chamber wire plane hits
  ...
  rc->set_IntFlag("AFSABSENT", 1);  // assume that there is no AFS present

  //
  // simVertexFlag = 0 means that the BBC Z0 value will be used as in HIJING events
  // simVertexFlag = 1 means that the same simZ0Vertex centroid value is used for all events
  // simVertexFlag = 2 (default) means that the Z0 centroid is taken from the PISA event header for each single (particle) event
  // The centroid values are modified by the Width values (in cm and ns) which are Gaussian sigma values
  //
...
//   se->registerSubsystem( muon_mc_filter );
  ...
  */
  ...
  //HWG_IOManager(hwgout, se);     // simulated HWG (omitted for Project 93)

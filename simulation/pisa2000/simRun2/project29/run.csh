...
###############################################################
if( $# != 4 ) then
    echo "usage : $0 eventnum output randomseed1 randomseed2"
    exit
endif
@ eventnum = $1
set outfullfile = $2
...
pushd $DIR_WORK
@ pythia_maxtrack = 150
printlog " Maximum track number per event in pythia is set to " ${pythia_maxtrack}
####################################################################################
printlog " Executing pythia with MB trigger... "
${SRC_DIR}/pythiaLinker.csh >>$ ${flog}
cp ${SRC_DIR}/pythiapar.mb.src ./pythia.par.src
cp ${SRC_DIR}/gen_pythia_par.csh ./
./gen_pythia_par.csh pythia.par.src pythia.par ${randomseed1} ${eventnum} >>& ${flog}
./pythia >>& ${flog}

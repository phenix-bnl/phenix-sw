...
int petopen(const char * etname)
{
  ...
  if ( theState.streamOpened() )
    {
      cout << "Stream is already open" << endl;
      return 1;
    }
  theIterator = new etEventiterator(etname, status);
  if (status)
    {
      delete theIterator;
      theIterator = 0;
      cout << "Could not open the ET pool " << etname << endl;
      return 2;
    }
  theState.setETOpened(etname);
  if (theGui)
    theGui->setStreamLabel(theIterator->getIdTag() );
  return 0;
}
...
int poncsetopen(const char * etname)
{
  ...
  theIterator = new oncsetEventiterator(etname, status);
  if (status)
    {
      delete theIterator;
      theIterator = 0;
...
int
petsetmask(const unsigned int lvl1, const unsigned int lvl2, const int disable)
{
  etEventiterator *EtIterator = dynamic_cast<etEventiterator *> (theIterator);
  if (!EtIterator)
    {
      return -1;
    }
  EtIterator->setSelectWords(-1,lvl1,-1,lvl2,disable);
  if (disable)
    {
      printf("disabling et pool trigger mask\n");
    }
  else
    {
      printf("Setting et pool trigger mask, lvl1 to %08x, lvl2 to %08x\n", lvl1, lvl2);
    }
  return 0;
}

int 
petsetevttype(const unsigned int datatype)
{
  return 0;
}

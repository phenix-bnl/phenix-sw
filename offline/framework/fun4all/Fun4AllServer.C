...
Fun4AllServer::registerSubsystem(SubsysReco *subsystem, const string &topnodename)
{
  ...
  if (iret)
    {
      if (iret == DONOTREGISTERSUBSYSTEM)
        {
          if (verbosity > 0)
            {
      ...
      if ( RetCodes[icnt] )
        {
          ...
          else if (RetCodes[icnt] == ABORTEVENT)
            {
              retcodesmap[ABORTEVENT]++;
              eventbad = 1;
              if (verbosity > 0)
                {
          ...
          else if (RetCodes[icnt] == ABORTRUN)
            {
              retcodesmap[ABORTRUN]++;
...
int Fun4AllServer::BeginRun(const int runno)
{
  ...
  if (! bortime_override)
    {
      ...
      beginruntimestamp = new PHTimeStamp(rc->get_TimeStamp());
    }
  else
    {
  ...
  for (iter = Subsystems.begin(); iter != Subsystems.end(); ++iter)
      ...
      catch (...)
	{
      ...
      else if (iret != EVENT_OK)
        {
          cout << PHWHERE << "Module " << (*iter).first->Name() << " issued non EVENT_OK return code " << iret << " in InitRun()" << endl;
          gSystem->Exit(-2);
          exit(-2);
        }
    }
      ...
      if (require_nevents)
        {
          if (std::find(RetCodes.begin(),
                        RetCodes.end(),
                        static_cast<int>(ABORTEVENT)) == RetCodes.end())
            icnt_good++;
...
Fun4AllServer::setRun(const int runno)
{
  ...
  if (runno != 0)
    {
      tstamp = RunToTime::instance()->getBeginTime(runno );
    }
  if (!tstamp)
    {

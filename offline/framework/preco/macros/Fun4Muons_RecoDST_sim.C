...
)
{
  ...
  if( pdstfile ) cout << "Fun4Muons_RecoDST_sim - pdstfile : " << pdstfile << endl;
  ...
  */
  ...
  if( use_SVX )
  {
    ...
    SubsysReco* svx_reco = new SvxReco();
    se->registerSubsystem( svx_reco );
  ...
  if( use_FVTX )
  {
    ...
    if( FVTX_fastsim )
    {
      ...
      fvtx_reco->run_after_burner(true);         // Adds VTX hits to tracking
      fvtx_reco->set_use_svx_cluster(false);     // Uses smeared PISA hits if false
  ...
  */
  ...
  if( pdstfile && write_pdst )
  {
    ...
    */
    ...
    picoDST->set_run_type(MWGpico::RUN8);
    picoDST->MakePico( MWGpico::DIMUONSOO, pdstfile );
    picoDST->MakePico( MWGpico::SNGMUONS, pdstfile );

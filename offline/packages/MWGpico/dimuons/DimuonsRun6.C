// $Id: DimuonsRun6.C,v 1.19 2017/07/15 04:49:08 phnxbld Exp $
/*!
\file DimuonsRun5.C
...
void MWGpico::BookDimuonsNtupleRun6( TNtuple*& dimuons, TString name, TString title )
{
  ...
  const char* varlist =
    ...
    "Pol_Y:Pol_B:GL1X_ID:Evt_ll1_S1D1S:Evt_ll1_N1D1S:Evt_ll1_SN2D:Mu_TrigBit:Tr0_dAx:Tr1_dAx:zvtx_dimu"; // 59
...
//_________________________________________________________________
void MWGpico::BookDimuonsNtupleRun6BackToBack( TNtuple*& dimuonsb2b, TString name, TString title )
{
  // define variable lists
  /* to make counting of variables easier, please try to write only ten variables/lines */
  const char* varlist =
    ...
    "Pol_B:GL1X_ID:Evt_ll1_S1D1S:Evt_ll1_N1D1S:Evt_ll1_B2BS:Evt_ll1_B2BD";  // 66
...
//_________________________________________________________________
int MWGpico::FillDimuonsRun6(PHMuoTracksOut* &muo, TNtuple* dimuons)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun6" << endl;
  ...
  // pseudoLL1 1D1S
  //Float_t pseudo_ll1_1D1S_south = Tools::LL1_1D1S_Decision( MUTOO::South );
  //Float_t pseudo_ll1_1D1S_north = Tools::LL1_1D1S_Decision( MUTOO::North );

  // BLT decision
  Float_t ll1_1D1S_south = -1;
  Float_t ll1_1D1S_north = -1;
  Float_t ll1_SN2D = -1;
  Float_t ll1_trigbits = -1;
  ...
  if( !(_choice == "simu" || _choice == "simu_file") )
  {
    ...
    if(_trig_lvl1){
      ...
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00040000) ll1_1D1S_north = 1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00080000) ll1_1D1S_south = 1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00200000) ll1_SN2D = 1;
  ...
  if (muo)
  {
    ...
    for (int idimu=0; idimu<ndimu; idimu++)
    {
      ...
      try {
        ...
        // some output
        //cout << "MWGpico::FillDimuonsRun6 - mass direct: " << dimass << " refit: " << vertex.get_mass() << endl;
      ...
      // LL1 1D1S
      varNT[53] = ll1_1D1S_south;
      varNT[54] = ll1_1D1S_north;
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun6 - done." << endl;
  return 0;
}

//_________________________________________________________________
int MWGpico::FillDimuonsRun6BackToBack(PHMuoTracksOut* &muo, TNtuple* dimuonsb2b)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun6BackToBack" << endl;
  ...
  if (muo) {
    ...
    int npart = muo->get_npart();
    if( verbosity >= default_verbosity ) cout << "MWGPico::FillDimuonsRun6BackToBack - totEVT: " << totEVT << " npart: " << npart << endl;
    ...
    for( int idx0 = 0; idx0 < npart; idx0 ++ )
    {
      ...
      for( int idx1 = idx0+1; idx1 < npart; idx1++ )
      {
        ...
        varNT[62] = ll1_1D1S_south; // pseudoLL1 1D1S
        varNT[63] = ll1_1D1S_north;
        varNT[64] = ll1_B2BS;       //B2B deep trigger
        varNT[65] = ll1_B2BD;       //B2B shallow trigger
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun6BackToBack - done." << endl;
  return 0;
}

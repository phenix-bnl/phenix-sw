// $Id: DimuonsRun11pp.C,v 1.2 2017/07/15 04:49:08 phnxbld Exp $
/*!
\file DimuonsRun5.C
...
void MWGpico::BookDimuonsNtupleRun11pp( TNtuple*& dimuons, TString name, TString title )
{
...
//_________________________________________________________________
void MWGpico::BookDimuonsNtupleRun11ppBackToBack( TNtuple*& dimuonsb2b, TString name, TString title )
{
  // define variable lists
  /* to make counting of variables easier, please try to write only ten variables/lines */
  const char* varlist =
...
//_________________________________________________________________
int MWGpico::FillDimuonsRun11pp(PHMuoTracksOut* &muo, TNtuple* dimuons)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun11pp" << endl;
  ...
  if (muo)
  {
    ...
    for (int idimu=0; idimu<ndimu; idimu++)
    {
      ...
      try {
        ...
        // some output
        //cout << "MWGpico::FillDimuonsRun11pp - mass direct: " << dimass << " refit: " << vertex.get_mass() << endl;
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun11pp - done." << endl;
  return 0;
}

//_________________________________________________________________
int MWGpico::FillDimuonsRun11ppBackToBack(PHMuoTracksOut* &muo, TNtuple* dimuonsb2b)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun11ppBackToBack" << endl;
  ...
  if (muo) {
    ...
    int npart = muo->get_npart();
    if( verbosity >= default_verbosity ) cout << "MWGPico::FillDimuonsRun11ppBackToBack - totEVT: " << totEVT << " npart: " << npart << endl;
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun11ppBackToBack - done." << endl;
  return 0;
}

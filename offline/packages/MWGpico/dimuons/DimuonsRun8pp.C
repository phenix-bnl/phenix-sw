// $Id: DimuonsRun8pp.C,v 1.8 2017/07/15 04:49:08 phnxbld Exp $
/*!
\file DimuonsRun5.C
...
void MWGpico::BookDimuonsNtupleRun8pp( TNtuple*& dimuons, TString name, TString title )
{
  ...
  const char* varlist =
    ...
    "Pol_Y:Pol_B:GL1X_ID:Evt_ll1_S2D:Evt_ll1_N2D:Evt_ll1_SN1D:Evt_ll1_SN1H:Mu_TrigBit:Tr0_dAx:Tr1_dAx:zvtx_dimu"; // 61
...
//_________________________________________________________________
void MWGpico::BookDimuonsNtupleRun8ppBackToBack( TNtuple*& dimuonsb2b, TString name, TString title )
{
  // define variable lists
  /* to make counting of variables easier, please try to write only ten variables/lines */
  const char* varlist =
...
//_________________________________________________________________
int MWGpico::FillDimuonsRun8pp(PHMuoTracksOut* &muo, TNtuple* dimuons)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun8pp" << endl;
  ...
  // LL1 decision
  Float_t ll1_2D_south = 0;
  Float_t ll1_2D_north = 0;
  Float_t ll1_SN1D = 0;
  Float_t ll1_SN1H = 0;
  ...
  if( !(_choice == "simu" || _choice == "simu_file") )
  {
    ...
    if(_trig_lvl1){
      ...
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00200000) ll1_2D_north = 1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00400000) ll1_2D_south = 1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00004000) ll1_SN1D = 1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00010000) ll1_SN1H = 1;
  ...
  if (muo)
  {
    ...
    for (int idimu=0; idimu<ndimu; idimu++)
    {
      ...
      try {
        ...
        // some output
        //cout << "MWGpico::FillDimuonsRun8pp - mass direct: " << dimass << " refit: " << vertex.get_mass() << endl;
      ...
      // LL1 1D1S
      varNT[53] = ll1_2D_south;
      varNT[54] = ll1_2D_north;

      varNT[55] = ll1_SN1D;
      varNT[56] = ll1_SN1H;

      varNT[57] = ll1_trigbits;
      varNT[58] = dA0;
      varNT[59] = dA1;
      varNT[60] = z_vertex;
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun8pp - done." << endl;
  return 0;
}

//_________________________________________________________________
int MWGpico::FillDimuonsRun8ppBackToBack(PHMuoTracksOut* &muo, TNtuple* dimuonsb2b)
{

  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun8ppBackToBack" << endl;
  ...
  if (muo) {
    ...
    int npart = muo->get_npart();
    if( verbosity >= default_verbosity ) cout << "MWGPico::FillDimuonsRun8ppBackToBack - totEVT: " << totEVT << " npart: " << npart << endl;
  ...
  if( verbosity >= default_verbosity ) cout << "MWGpico::FillDimuonsRun8ppBackToBack - done." << endl;
  return 0;
}

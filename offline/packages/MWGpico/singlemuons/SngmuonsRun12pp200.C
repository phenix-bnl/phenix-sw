// $Id: SngmuonsRun12pp200.C,v 1.6 2014/12/15 05:46:24 rseidl Exp $

/*!
   \file SngmuonsRun12pp200.C
   \brief single muon ntuple booking and feeling
   \version $Revision: 1.6 $
   \date $Date: 2014/12/15 05:46:24 $
*/
...
void MWGpico::BookSngmuonsNtupleRun12pp200(TNtuple*& sngmuons, TString m_name, TString m_title)
{
  ...
  */
  ...
  if ( _choice == "simu" ){
	  sngvarlist=
		...
		"Evt_realtrig_BBCLL1_narrowvtx:Evt_realtrig_SN_1D:" //45
		"Evt_realtrig_S_SG3_1DH:Evt_realtrig_N_SG3_1DH:xSt1:"//48
		"ySt1:zSt1:xSt2:ySt2:zSt2:" //53
		"xSt3:ySt3:zSt3:ref_vtx_rdca:ref_vtx_r:" //58
		"ref_vtx_z:refit_zvtx:Clock_trig:Evt_realtrig_MB:dAngle:" //63
		"dAngle_xyz:dTheta:dPhi:road_slope:vtx_chi2:vtx_chi2pdf:" //69
		"mc_n_part:mc_px:mc_py:mc_pz:mc_ptot:mc_z:" //75
		"mc_pid:mc_hits:mc_p_pid:mc_p_px:mc_p_py:" //80
		"mc_p_pz:mc_p_ptot:mc_p_z:mc_d_pid:mc_d_px:" //85
		"mc_d_py:mc_d_pz:mc_d_ptot:mc_d_z:mc_d_n:" //90
		"mc_x:mc_y:mc_g_pid:mc_g_px:mc_g_py:" //95
		"mc_g_pz:mc_g_ptot:mc_g_z:mc_trk"; //100
  }else{
	  sngvarlist=
		"Run_Number:Evt_Number:Evt_Nmu:Evt_Z:Evt_bbcZ:" //5
		"Evt_bbcCentrality:Evt_zdcEnerN:charge:px:py:" //10
...
void MWGpico::BookSngmuonsEvtNtupleRun12pp200(TNtuple*& sngvtx, TString v_name, TString v_title )
{
  ...
  vtxvarlist= "Run_Number:Evt_Z:bbcZ:zdcCentrality:bbcCentrality:" //5
	...
	"Evt_realtrig_BBCLL1_narrowvtx:Evt_realtrig_SN_1D:" //17
	"Evt_realtrig_S_SG3_1DH:Evt_realtrig_N_SG3_1DH"; //19
...
//__________________________________________________
int MWGpico::FillSngmuonsNtpRun12pp200(PHMuoTracksOut* &muo, TNtuple* sngmuons,  TNtuple* sngvtx, bool golden)
{
  //=== event selection
  ...
  Float_t realtrigS_SG3_1DH			    =  -999; // real trigger S SG3&(D|H)
  Float_t realtrigN_SG3_1DH			    =  -999; // real trigger N SG3&(D|H)
  ...
  // real triggers: Run12pp200 200GeV GeV runs
  ...
  { 
    ...
    // MGW --- use trigger bit for trigger selection in run12 200GeV runs 
	  
	  //scaled triggers

    // MGW - Special triggers for Run 9 test
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00000002) realtrigBBCLL1=1;
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00000004) realtrigBBCLL1_novtx=1;
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00400000) realtrigSN_1D=1;
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00040000) realtrigS_SG3_1DH=1;
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00800000) realtrigN_SG3_1DH=1;
    ...
    // if ( _trig_lvl1->get_lvl1_trigscaled()&0x00000010) realtrigBBCLL1_narrowvtx=1;
    // if ( _trig_lvl1->get_lvl1_trigscaled()&0x00020000) realtrigSN_1D=1;
    // if ( _trig_lvl1->get_lvl1_trigscaled()&0x00100000) realtrigS_SG3_1DH=1;
    // if ( _trig_lvl1->get_lvl1_trigscaled()&0x00200000) realtrigN_SG3_1DH=1;
  ...
  */
  ...
  Evt_Data[15] = realtrigBBCLL1_narrowvtx;
  Evt_Data[16] = realtrigSN_1D;
  Evt_Data[17] = realtrigS_SG3_1DH;
  Evt_Data[18] = realtrigN_SG3_1DH;
  ...
  if (muo) { 
    ...
    if(golden) {
      ...
      for (int idimu=0; idimu<ndimu; idimu++)
      {
        ...
        if(dimass>=2.6 && dimass<=3.6) {
          jpsi_trackids.insert(idx0);
          jpsi_trackids.insert(idx1);
        }
      }
    }
 
    int npart = muo->get_npart(); //jpsi_trackids.size();
    totMU += npart;
    
    for (int ipart=0; ipart<npart; ipart++) 
    {
      
      if( golden && jpsi_trackids.find(ipart) == jpsi_trackids.end() ) continue;
	...
	*/
	...
	while( TMutTrkMap::const_pointer trk_ptr = trk_iter.next() )
	{
      ...
      varNT[43] = realtrigBBCLL1_narrowvtx;
      varNT[44] = realtrigSN_1D;

      varNT[45] = realtrigS_SG3_1DH;
      varNT[46] = realtrigN_SG3_1DH;
      varNT[47] = xSTI;
      ...
      varNT[54] = ySTIII;
      varNT[55] = zSTIII;
      varNT[56] = ref_vtx_rdca;           	   // muon px at Vertex
      varNT[57] = ref_vtx_r;            	   // muon py at Vertex

      varNT[58] = ref_vtx_z;                       // muon pz at Vertex
      varNT[59] = vertex.get_vtx_z();              // event Z-vtx from refit
      varNT[60] = Clock_trig;
      varNT[61] = realtrig_MB;                     // live real   trigger for MB
      varNT[62] = dANGLE;
      ...
      varNT[65] = dTHETA;    
      varNT[66] = ROAD_SLOPE;
	  varNT[67] = vertex.get_chisquare();
	  varNT[68] = vertex.get_chisquare_pdf();

      varNT[69] = MC_N_PART; //
      varNT[70] = MC_PX; // from MC tracker_map
      varNT[71] = MC_PY; // from MC tracker_map
      varNT[72] = MC_PZ; // from MC tracker_map
      varNT[73] = MC_PTOT; // from MC tracker_map
      varNT[74] = MC_Z; // from MC tracker_map

      varNT[75] = MC_PID; // from MC tracker_map
      varNT[76] = MC_HITS; // hits of the major MC trk
      varNT[77] = MC_P_PID; // from MC tracker_map
      varNT[78] = MC_P_PX; // from MC tracker_map
      varNT[79] = MC_P_PY; // from MC tracker_map

      varNT[80] = MC_P_PZ; // from MC tracker_map
      varNT[81] = MC_P_PTOT; //
      varNT[82] = MC_P_Z; //
      varNT[83] = MC_D_PID; // from MC tracker_map for daughter particle
      varNT[84] = MC_D_PX; // from MC tracker_map

      varNT[85] = MC_D_PY; // from MC tracker_map
      varNT[86] = MC_D_PZ; // from MC tracker_map
      varNT[87] = MC_D_PTOT; //
      varNT[88] = MC_D_Z; // origin_z
      varNT[89] = MC_D_N; // total # of daughter particles

      varNT[90] = MC_X; // from MC tracker_map
      varNT[91] = MC_Y; // from MC tracker_map
      varNT[92] = MC_G_PID; // from MC tracker_map
      varNT[93] = MC_G_PX; // from MC tracker_map
      varNT[94] = MC_G_PY; // from MC tracker_map
	  
      varNT[95] = MC_G_PZ; // from MC tracker_map
      varNT[96] = MC_G_PTOT; //
      varNT[97] = MC_G_Z; //
      varNT[98] = MC_TRK;
      varNT[99] = dphi;

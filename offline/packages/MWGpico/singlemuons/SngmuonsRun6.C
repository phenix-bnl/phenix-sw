// $Id: SngmuonsRun6.C,v 1.12 2014/12/09 06:28:10 rseidl Exp $
...
void MWGpico::BookSngmuonsNtupleRun6(TNtuple*& sngmuons, TString m_name, TString m_title)
{
  ...
  */
  ...
  sngvarlist=
    ...
    "RPbbcrp10:RPbbcrp11:RPbbcrp12:delphi_RP:dphi:dphi_S:dphi_N:v2_obs:v2_obs_S:v2_obs_N:"
    "mc_trk:RPbbcrp01:RPbbcrp02:RPsmdrp00:RPsmdrp01:RPsmdrp02";   // BBC SMD rp
...
void MWGpico::BookSngmuonsEvtNtupleRun6(TNtuple*& sngvtx, TString v_name, TString v_title )
{
  ...
  vtxvarlist= "Run_Number:SpinX_ID:Pol_Y:Pol_B:GL1X_ID:Evt_Z:bbcZ:zdcCentrality:bbcCentrality:BBCQN:"
    ...
    "RPbbcrp10:RPbbcrp11:RPbbcrp12:delphi_RP:Evt_bbcCentralityByPerp:"
...
//__________________________________________________
int MWGpico::FillSngmuonsNtpRun6(PHMuoTracksOut* &muo, TNtuple* sngmuons,  TNtuple* sngvtx)
{
  //=== event selection
  ...
  if (evt) {
    ...
    //
    //    Evt_Z      = evt->getZVertex(); // seems not implemented  YET
    //
  }
  ...
  if (evt) {
    ...
    if (_type=="CuCu") {
      bbcCentralityByPerp = evt->getCentrality();
      //cout <<" getting CuCu centrality from getCentrality"<< endl;
      //cout << BbcZVertex <<"   "<< bbcCentralityByPerp << endl;
  ...
  if(_trig_lvl1){ 
    ...
    if( false )
    {
      ...
      if (TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_S1S&BBCLL1")) realtrigS_1S=1;
      if (TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_N1S&BBCLL1")) realtrigN_1S=1;
      ...
      if (TrigHelp->didLevel1TriggerFire("MUIDLL1_S1S&BBCLL1")) livetrigS_1S=1;
      if (TrigHelp->didLevel1TriggerFire("MUIDLL1_N1S&BBCLL1")) livetrigN_1S=1;
      ...
      //scaled triggers
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00004000) realtrigN_1D=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00008000) realtrigS_1D=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00010000) realtrigN_1S=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00020000) realtrigS_1S=1;
    
      //live triggers
      if (_trig_lvl1->get_lvl1_triglive()&0x00004000) livetrigN_1D=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00008000) livetrigS_1D=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00010000) livetrigN_1S=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00020000) livetrigS_1S=1;
    
    }
   
  //------ run6 pp 62 GeV August 11, 2006 XRWANG
  //Trig_Lvl1 *trig = getClass<Trig_Lvl1> (topNode, "Trig_Lvl1");
  //if (!trig) {
  // cout<<"Trig_Lvl1 does not exist"<<endl;
  //return 0;
  //} 
  
  // Get Trigger info from real data: for 62GeV
  if (_trig_lvl1->get_lvl1_triglive()&0x00000004) realtrig_MB =2;  
  if ( _trig_lvl1->get_lvl1_trigscaled()&0x00000004) realtrig_MB =1;  
  if (_trig_lvl1->get_lvl1_triglive()&0x00000008) realtrig_MB =22;  
  if ( _trig_lvl1->get_lvl1_trigscaled()&0x00000008) realtrig_MB =11;
  ...
  Evt_Data[25] = bbcCentralityByPerp;            // Reaction Plane
  ...
  if (muo) { 
	...
	while( TMutTrkMap::const_pointer trk_ptr = trk_iter.next() )
	{
      ...
      varNT[76] = vertex.get_vtx_z();              // event Z-vtx from refit
      varNT[77] = Clock_trig;
      ...
      varNT[80] = Pol_Y;
      varNT[81] = Pol_B;
      varNT[82] = GL1X_ID;                          // a place holder
      varNT[83] = dANGLE;                            // X1, a place holder
      varNT[84] = dANGLE_xyz;                         // X2, a place holder
      varNT[85] = dPHI;                            // X3, a place holder
      varNT[86] = dTHETA;                            // X4, a place holder
      varNT[87] = ROAD_SLOPE;                            // X5, a place holder
      ...
      varNT[121] = delphi_RP;                      // Reaction Plane
      varNT[122] = dphi;
      varNT[123] = dphi_S;
      varNT[124] = dphi_N;
      varNT[125] = v2_obs;
      ...
      varNT[128] = MC_TRK;
      varNT[129] = RPbbcrp00;                      // Reaction Plane
      varNT[130] = RPbbcrp01;                      // Reaction Plane
      varNT[131] = RPbbcrp02;                      // Reaction Plane
      varNT[132] = RPsmdrp00;                      // Reaction Plane
      varNT[133] = RPsmdrp01;                      // Reaction Plane
      varNT[134] = RPsmdrp02;                      // Reaction Plane

// $Id: SngmuonsRun9.C,v 1.3 2014/12/09 06:28:10 rseidl Exp $
...
void MWGpico::BookSngmuonsNtupleRun9(TNtuple*& sngmuons, TString m_name, TString m_title)
{
  ...
  */
  ...
  sngvarlist=
    ...
    "DS0:refX:refY:mutr_nhits:muid_nhits:Evt_realtrigSN_1D:Evt_realtrigSN_1S:Evt_livetrigSN_1D:Evt_livetrigSN_1S:Evt_pseudotrigS_1D:"
    "Evt_pseudotrigS_1S:Evt_pseudotrigN_1D:Evt_pseudotrigN_1S:xSt1:ySt1:zSt1:xSt2:ySt2:zSt2:xSt3:"
    "ySt3:zSt3:ref_vtx_rdca:ref_vtx_r:ref_vtx_z:refit_zvtx:Clock_trig:Evt_realtrig_MB:SpinX_ID:Pol_Y:"
    "Pol_B:GL1X_ID:dAngle:dAngle_xyz:dTheta:dPhi:road_slope:mc_n_part:mc_px:mc_py:"
    "mc_pz:mc_ptot:mc_z:mc_pid:mc_hits:mc_p_pid:mc_p_px:mc_p_py:mc_p_pz:mc_p_ptot:"
    "mc_p_z:mc_d_pid:mc_d_px:mc_d_py:mc_d_pz:mc_d_ptot:mc_d_z:mc_d_n:mc_x:mc_y:"
    "mc_g_pid:mc_g_px:mc_g_py:mc_g_pz:mc_g_ptot:mc_g_z:mc_trk:vtx_chi2:vtx_chi2pdf";   
...
void MWGpico::BookSngmuonsEvtNtupleRun9(TNtuple*& sngvtx, TString v_name, TString v_title )
{
  ...
  vtxvarlist= "Run_Number:SpinX_ID:Pol_Y:Pol_B:GL1X_ID:Evt_Z:bbcZ:zdcCentrality:bbcCentrality:BBCQN:"
    "BBCQS:BBCNN:BBCNS:ZDCN:ZDCS:Clock_trig:Evt_realtrig_MB:Evt_livetrigSN_1S:Evt_livetrigSN_1D"; // BBC SMD rp
...
//__________________________________________________
int MWGpico::FillSngmuonsNtpRun9(PHMuoTracksOut* &muo, TNtuple* sngmuons,  TNtuple* sngvtx)
{
  //=== event selection
  ...
  Float_t realtrigSN_1D     =  0; // real trigger South
  Float_t realtrigSN_1S     =  0; // real trigger South
  
  Float_t livetrigSN_1D     =  0; // live trigger South
  Float_t livetrigSN_1S     =  0; // live trigger South
  ...
  // real triggers: Run9 200GeV and 500 GeV runs
  
  if(_trig_lvl1){ // Trigger Node exists in data file

    
    // --- use trigger bit for trigger selection to cover run9 200GeV and 500 GeV runs for MUIDLL1_trigger with and w/o BBCLL1
    // MXL

    //scaled triggers
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00400000){ // (MUIDLL1_N1D || MUIDLL1_S1D )&&BBCLL1(novtx)
      realtrigSN_1D=1;
    }
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00040000){  //  MUIDLL1_S1H &&BBCLL1(novtx) 
      realtrigSN_1S=1;
    }    
    if ( _trig_lvl1->get_lvl1_trigscaled()&0x00800000){  //  MUIDLL1_N1H &&BBCLL1(novtx) 
      realtrigSN_1S=1;
    }    

    //live triggers
    if ( _trig_lvl1->get_lvl1_triglive()&0x00400000){ // (MUIDLL1_N1D || MUIDLL1_S1D )&&BBCLL1(novtx)
      realtrigSN_1D=1;
    }
    if ( _trig_lvl1->get_lvl1_triglive()&0x00040000){  //  MUIDLL1_S1H &&BBCLL1(novtx) 
      realtrigSN_1S=1;
    }    
    if ( _trig_lvl1->get_lvl1_triglive()&0x00800000){  //  MUIDLL1_N1H &&BBCLL1(novtx) 
      realtrigSN_1S=1;
    }    
  ...
  Evt_Data[17] = livetrigSN_1S;                    // live real   trigger for MUID_LL1_S_1S
  Evt_Data[18] = livetrigSN_1D;                    // live real   trigger for MUID_LL1_S_1D
  ...
  if (muo) { 
    ...
    for (int ipart=0; ipart<npart; ipart++) 
    {
 	...
 	BbcZVertex = vertex.get_vtx_z();
      ...
      dPHI = (atan2(ySTI,xSTI)-atan2(ySTIII,xSTIII));    // (x,y) -> (y,x) bug fixed  12/03/09  MXL 
      ...
      if (!mut_mctrk_map) cout << " ---->> no mut_mctrk_map "  << endl;
      
      if (mut_trk_map && mut_mctrk_map) {

	//	cout << " >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> " <<endl;
	//	cout << " -------  found mut_trk_map and  mut_mctrk_map !!!!!!! -----------" << endl;
	...
	while( TMutTrkMap::const_pointer trk_ptr = trk_iter.next() )
	{
	  ...
	  //	  cout << " >>>>>>   diff_x == " << diff_x << endl;
          if (diff_x > 0.001) continue; // not the same track

	  //	  cout << " >>>>> Found matching nDST and DST track, diff_x = "  << diff_x  << endl; 
	  //	  cout << "now looking for associated MC track !!!"  << endl;
	  ...
	  mc_trk_iter.reset(); // reset ???
	    ...
	    //  cout << " >>>>>>>>>    thetr are \t " << MC_HITS << " \t MC hits from this associated MC particle" << endl;
	    ...
	    //            MC_P_TRK =-999;    // parent track ID: for debuggin now
	      ...
	      /*	      
	      cout << "in the MCbank,  MC track id trk_IDx == " << trk_IDx;
      ...
      varNT[43] = mutr_nhits;
      varNT[44] = muid_nhits;
      varNT[45] = realtrigSN_1D;                    // real   trigger 1 deep  South
      varNT[46] = realtrigSN_1S;                    // real   trigger 1 sheep  South
      varNT[47] = livetrigSN_1D;                    // live    trigger 1 sheep  South
      varNT[48] = livetrigSN_1S;                    // live   trigger 1 deep  North
      varNT[49] = pseudotrigS_1D;

      varNT[50] = pseudotrigS_1S;
      varNT[51] = pseudotrigN_1D;
      varNT[52] = pseudotrigN_1S;
      varNT[53] = xSTI;
      varNT[54] = ySTI;
      varNT[55] = zSTI;
      varNT[56] = xSTII;
      ...
      varNT[60] = ySTIII;
      varNT[61] = zSTIII;
      varNT[62] = ref_vtx_rdca;           	   // muon px at Vertex
      varNT[63] = ref_vtx_r;            	   // muon py at Vertex
      varNT[64] = ref_vtx_z;                       // muon pz at Vertex
      varNT[65] = vertex.get_vtx_z();              // event Z-vtx from refit
      varNT[66] = Clock_trig;
      varNT[67] = realtrig_MB;                     // live real   trigger for MB
      varNT[68] = SpinX_ID;                         // beam crossing ID
      varNT[69] = Pol_Y;
      ...
      varNT[76] = ROAD_SLOPE;
      varNT[77] = MC_N_PART; //
      varNT[78] = MC_PX; // from MC tracker_map
      varNT[79] = MC_PY; // from MC tracker_map

      varNT[80] = MC_PZ; // from MC tracker_map
      varNT[81] = MC_PTOT; // from MC tracker_map

      varNT[82] = MC_Z; // from MC tracker_map
      varNT[83] = MC_PID; // from MC tracker_map
      varNT[84] = MC_HITS; // hits of the major MC trk
      varNT[85] = MC_P_PID; // from MC tracker_map
      varNT[86] = MC_P_PX; // from MC tracker_map
      varNT[87] = MC_P_PY; // from MC tracker_map
      varNT[88] = MC_P_PZ; // from MC tracker_map
      varNT[89] = MC_P_PTOT; //

      varNT[90] = MC_P_Z; //
      varNT[91] = MC_D_PID; // from MC tracker_map for daughter particle
      varNT[92] = MC_D_PX; // from MC tracker_map
      varNT[93] = MC_D_PY; // from MC tracker_map
      varNT[94] = MC_D_PZ; // from MC tracker_map
      varNT[95] = MC_D_PTOT; //
      varNT[96] = MC_D_Z; // origin_z
      varNT[97] = MC_D_N; // total # of daughter particles
      varNT[98] = MC_X; // from MC tracker_map
      varNT[99] = MC_Y; // from MC tracker_map

      varNT[100] = MC_G_PID; // from MC tracker_map
      varNT[101] = MC_G_PX; // from MC tracker_map
      varNT[102] = MC_G_PY; // from MC tracker_map
      varNT[103] = MC_G_PZ; // from MC tracker_map
      varNT[104] = MC_G_PTOT; //
      varNT[105] = MC_G_Z; //
      varNT[106] = MC_TRK;
      varNT[107] = vertex.get_chisquare();
      varNT[108] = vertex.get_chisquare_pdf();

// $Id: SngmuonsRun8.C,v 1.8 2014/12/09 06:28:10 rseidl Exp $
...
void MWGpico::BookSngmuonsNtupleRun8(TNtuple*& sngmuons, TString m_name, TString m_title)
{
  ...
  */
  ...
  sngvarlist=
    "Run_Number:Evt_Number:Evt_Nmu:Evt_Z:Evt_bbcZ:Evt_bbcCentrality:Evt_zdcEnerN:charge:px:py:"
    "pz:pxSt1:pySt1:pzSt1:pxSt3:pySt3:pzSt3:pT:p:pSt1:"
    "ELoss:chi2:idhits:idquad:trhits:DS3:DS3ctp:idchi2:gap0x:gap0y:"
    "gap0z:dxdz:dydz:trstat:ghost:lastGap:eta:phi:DG0:DDG0:"
    "DS0:refX:refY:mutr_nhits:muid_nhits:Evt_realtrigS_1D:Evt_realtrigS_1S:Evt_realtrigN_1D:Evt_realtrigN_1S:Evt_livetrigS_1D:"
    "Evt_livetrigS_1S:Evt_livetrigN_1D:Evt_livetrigN_1S:Evt_pseudotrigS_1D:Evt_pseudotrigS_1S:Evt_pseudotrigN_1D:Evt_pseudotrigN_1S:Evt_recoS_1D:Evt_recoS_1S:Evt_recoN_1D:"
    "Evt_recoN_1S:xSt1:ySt1:zSt1:xSt2:ySt2:zSt2:xSt3:ySt3:zSt3:"
    "ref_vtx_rdca:ref_vtx_r:ref_vtx_z:refit_zvtx:Clock_trig:Evt_realtrig_MB:SpinX_ID:Pol_Y:Pol_B:GL1X_ID:"
    "dAngle:dAngle_xyz:dTheta:dPhi:road_slope:mc_n_part:mc_px:mc_py:mc_pz:mc_ptot:"
    "mc_z:mc_pid:mc_hits:mc_p_pid:mc_p_px:mc_p_py:mc_p_pz:mc_p_ptot:mc_p_z:mc_d_pid:"
    "mc_d_px:mc_d_py:mc_d_pz:mc_d_ptot:mc_d_z:mc_d_n:mc_x:mc_y:mc_g_pid:mc_g_px:"
    "mc_g_py:mc_g_pz:mc_g_ptot:mc_g_z:mc_trk:RPbbcrp10:RPbbcrp11:RPbbcrp12:delphi_RP:dphi:"
    "dphi_S:dphi_N:v2_obs:v2_obs_S:v2_obs_N:RPbbcrp00:RPbbcrp01:RPbbcrp02:RPsmdrp00:RPsmdrp01:"
    "RPsmdrp02";   // BBC SMD rp
...
void MWGpico::BookSngmuonsEvtNtupleRun8(TNtuple*& sngvtx, TString v_name, TString v_title )
{
...
//__________________________________________________
int MWGpico::FillSngmuonsNtpRun8(PHMuoTracksOut* &muo, TNtuple* sngmuons,  TNtuple* sngvtx)
{
  //=== event selection
  ...
  { 
    ...
    if( false )
    {
      ...
      //real triggers
      if( TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_S1D&BBCLL1") ) realtrigS_1D=1;
      if( TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_N1D&BBCLL1") ) realtrigN_1D=1;
      if( TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_S1H&BBCLL1") || TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_S1S&BBCLL1") ) realtrigS_1S=1;
      if( TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_N1H&BBCLL1") || TrigHelp->didLevel1TriggerGetScaled("MUIDLL1_N1S&BBCLL1") ) realtrigN_1S=1;
    
      // live triggers
      if( TrigHelp->didLevel1TriggerFire("MUIDLL1_S1D&BBCLL1") ) livetrigS_1D=1;
      if( TrigHelp->didLevel1TriggerFire("MUIDLL1_N1D&BBCLL1") ) livetrigN_1D=1;
      if( TrigHelp->didLevel1TriggerFire("MUIDLL1_S1H&BBCLL1") || TrigHelp->didLevel1TriggerFire("MUIDLL1_S1S&BBCLL1") ) livetrigS_1S=1;
      if( TrigHelp->didLevel1TriggerFire("MUIDLL1_N1H&BBCLL1") || TrigHelp->didLevel1TriggerFire("MUIDLL1_N1S&BBCLL1") ) livetrigN_1S=1;
      ...
      //scaled triggers
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00010000) realtrigN_1D=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00020000) realtrigS_1D=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00040000) realtrigN_1S=1;
      if ( _trig_lvl1->get_lvl1_trigscaled()&0x00080000) realtrigS_1S=1;
      
      //live triggers
      if (_trig_lvl1->get_lvl1_triglive()&0x00010000) livetrigN_1D=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00020000) livetrigS_1D=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00040000) livetrigN_1S=1;
      if (_trig_lvl1->get_lvl1_triglive()&0x00080000) livetrigS_1S=1;
  ...
  Evt_Data[7] = zdcCentrality;
  Evt_Data[8] = bbcCentrality;
  Evt_Data[9] = BBCQN;
  ...
  Evt_Data[21] = RPbbcrp10;                      // Reaction Plane
  Evt_Data[22] = RPbbcrp11;                      // Reaction Plane
  Evt_Data[23] = RPbbcrp12;                      // Reaction Plane
  Evt_Data[24] = delphi_RP;                      // Reaction Plane
  Evt_Data[25] = -9999;            // 
  Evt_Data[26] = RPbbcrp00;                      // Reaction Plane
  Evt_Data[27] = RPbbcrp01;                      // Reaction Plane
  Evt_Data[28] = RPbbcrp02;                      // Reaction Plane
  Evt_Data[29] = RPsmdrp00;                      // Reaction Plane
  Evt_Data[30] = RPsmdrp01;                      // Reaction Plane
  Evt_Data[31] = RPsmdrp02;                      // Reaction Plane
  ...
  if (muo) { 
	...
	while( TMutTrkMap::const_pointer trk_ptr = trk_iter.next() )
	{
      ...
      varNT[25] = dS30;                            // DS3 of 1st muon (à la Olivier)
      varNT[26] = dS3ctp0;                         // DS3 of 1st muon (w const theta & phi)
      varNT[27] = muIDchis0;                       // Muid Chi2
      varNT[28] = gap0x;            // Muid Gap0 x
      varNT[29] = gap0y;            // Muid Gap0 y

      varNT[30] = gap0z;            // Muid Gap0 z
      varNT[31] = dirX;             // Muid Road dxdz
      varNT[32] = dirY;             // Muid Road  dydz
      varNT[33] = muo->get_TMutTrk_status(ipart);       // Mutr track status
      varNT[34] = muo->get_ghostflag(ipart);
      varNT[35] = lastGap;
      varNT[36] = pseudo_rapidity;		   // pseudo rapidity
      varNT[37] = atan(py/px);       // Reaction plane phi
      varNT[38] = DG0;				   // DG0
      varNT[39] = DDG0;				   // DDG0 - opening angle

      varNT[40] = DS0;				   // DS0 - muID road doca @z=0
      varNT[41] = refX;
      varNT[42] = refY;
      varNT[43] = mutr_nhits;
      varNT[44] = muid_nhits;
      varNT[45] = realtrigS_1D;                    // real   trigger 1 deep  South
      varNT[46] = realtrigS_1S;                    // real   trigger 1 sheep  South
      varNT[47] = realtrigN_1D;                    // real   trigger 1 deep  North
      varNT[48] = realtrigN_1S;                    // real   trigger 1 sheep North
      varNT[49] = livetrigS_1D;                    // live   trigger 1 deep  South

      varNT[50] = livetrigS_1S;                    // live    trigger 1 sheep  South
      varNT[51] = livetrigN_1D;                    // live   trigger 1 deep  North
      varNT[52] = livetrigN_1S;                    // live   trigger 1 sheep North
      varNT[53] = pseudotrigS_1D;
      varNT[54] = pseudotrigS_1S;
      varNT[55] = pseudotrigN_1D;
      varNT[56] = pseudotrigN_1S;
      varNT[57] = recoS_1D;
      varNT[58] = recoS_1S;
      varNT[59] = recoN_1D;
      ...
      varNT[70] = ref_vtx_rdca;           	   // muon px at Vertex
      varNT[71] = ref_vtx_r;            	   // muon py at Vertex
      varNT[72] = ref_vtx_z;                       // muon pz at Vertex
      varNT[73] = vertex.get_vtx_z();              // event Z-vtx from refit
      varNT[74] = Clock_trig;
      varNT[75] = realtrig_MB;                     // live real   trigger for MB
      varNT[76] = SpinX_ID;                         // beam crossing ID
      varNT[77] = Pol_Y;
      varNT[78] = Pol_B;
      varNT[79] = GL1X_ID;
      ...
      varNT[84] = ROAD_SLOPE;
      varNT[85] = MC_N_PART; //
      varNT[86] = MC_PX; // from MC tracker_map
      varNT[87] = MC_PY; // from MC tracker_map
      varNT[88] = MC_PZ; // from MC tracker_map
      varNT[89] = MC_PTOT; // from MC tracker_map

      varNT[90] = MC_Z; // from MC tracker_map
      varNT[91] = MC_PID; // from MC tracker_map
      varNT[92] = MC_HITS; // hits of the major MC trk
      varNT[93] = MC_P_PID; // from MC tracker_map
      varNT[94] = MC_P_PX; // from MC tracker_map
      varNT[95] = MC_P_PY; // from MC tracker_map
      varNT[96] = MC_P_PZ; // from MC tracker_map
      varNT[97] = MC_P_PTOT; //
      varNT[98] = MC_P_Z; //
      varNT[99] = MC_D_PID; // from MC tracker_map for daughter particle

      varNT[100] = MC_D_PX; // from MC tracker_map
      varNT[101] = MC_D_PY; // from MC tracker_map
      varNT[102] = MC_D_PZ; // from MC tracker_map
      varNT[103] = MC_D_PTOT; //
      varNT[104] = MC_D_Z; // origin_z
      varNT[105] = MC_D_N; // total # of daughter particles
      varNT[106] = MC_X; // from MC tracker_map
      varNT[107] = MC_Y; // from MC tracker_map
      varNT[108] = MC_G_PID; // from MC tracker_map
      varNT[109] = MC_G_PX; // from MC tracker_map

      varNT[110] = MC_G_PY; // from MC tracker_map
      varNT[111] = MC_G_PZ; // from MC tracker_map
      varNT[112] = MC_G_PTOT; //
      varNT[113] = MC_G_Z; //
      varNT[114] = MC_TRK;

      varNT[115] = RPbbcrp10;                      // Reaction Plane
      varNT[116] = RPbbcrp11;                      // Reaction Plane
      varNT[117] = RPbbcrp12;                      // Reaction Plane
      varNT[118] = delphi_RP;                      // Reaction Plane
      varNT[119] = dphi;
      ...
      varNT[123] = v2_obs_S;
      varNT[124] = v2_obs_N;
      varNT[125] = RPbbcrp00;                      // Reaction Plane
      varNT[126] = RPbbcrp01;                      // Reaction Plane
      varNT[127] = RPbbcrp02;                      // Reaction Plane
      varNT[128] = RPsmdrp00;                      // Reaction Plane
      varNT[129] = RPsmdrp01;                      // Reaction Plane
      varNT[130] = RPsmdrp02;                      // Reaction Plane

// $Id: NewSngmuonsRun11.C,v 1.11 2017/07/15 04:49:08 phnxbld Exp $
...
void MWGpico::BookNewSngmuonsNtupleRun11(TTree*& newsngmuons, TString m_name, TString m_title)
{
...
//   cout << " booked new eventdata" << endl; 
   ...
   newsngmuons->Branch("RecoTracks",0,"Evt_Nmu[_RecoTracks]/I:charge[_RecoTracks]/F:px[_RecoTracks]/F:py[_RecoTracks]/F:"
		       ...
		       "DS0[_RecoTracks]/F:refX[_RecoTracks]/F:refY[_RecoTracks]/F:mutr_nhits[_RecoTracks]/F:muid_nhits[_RecoTracks]/F:Evt_pseudotrigNS_1D[_RecoTracks]/I:Evt_pseudotrigS_SG1D[_RecoTracks]/I:Evt_pseudotrigNS_1H[_RecoTracks]/I:Evt_pseudotrigN_SG1D[_RecoTracks]/I:Evt_recoNS_1D[_RecoTracks]/I:Evt_recoS_SG1D[_RecoTracks]/I:Evt_recoNS_1H[_RecoTracks]/I:"
...
//      cout << " finished booking NewSngmuonsRun11" << endl;
   return;
}

//__________________________________________________________________
void MWGpico::BookNewSngmuonsEvtNtupleRun11(TNtuple*& newsngvtx, TString v_name, TString v_title )
{
...
//__________________________________________________
int MWGpico::FillNewSngmuonsNtpRun11(PHMuoTracksOut* &muo, TTree* newsngmuons,  TNtuple* newsngvtx)
{

   //  cout <<  "in FillNewSngmuonsNtpRun11" << endl;
//=== event information
...
// event/run number
   ...
   fRunNumber11 = new Int_t[1];
   fRunNumber11[0] = ( run_header ) ?  run_header->get_RunNumber():0;


   EventNumber11 = new Int_t[1];
   EventNumber11[0] = (event_header ) ? event_header->get_EvtSequence():0;
  
// try load MC if failed from PHGlobal
   {
      bool error( false );
      if( !fRunNumber11[0] ) fRunNumber11[0] = Tools::runNumberMC( header, error );
      if( !EventNumber11[0] ) EventNumber11[0]  = Tools::eventNumberMC( header, error );
   }
...
// cout << "before triggers ini NEwSngmuonsRun11" << endl;  
   if(_trig_lvl1){ // Trigger Node exists in data file


//for run 11 almost all of the run with the following triggers:
...
//
// RCS use bits for run11 for the moment, only deep, halted, and SG1Ds
//scaled triggers
...
//=== Fill branches
      ((TLeaf *) fBranch->GetListOfLeaves()->At(0))->SetAddress(fRunNumber11);                       // run number
//cout << " after runnr" << endl;
      ((TLeaf *) fBranch->GetListOfLeaves()->At(1))->SetAddress(EventNumber11);                     // event number
...
//_____________ end of sngvtx _____________
      ...
      Int_t *pseudotrigNS_1D=NULL;
      Int_t *pseudotrigS_SG1D=NULL;
      Int_t *pseudotrigNS_1H=NULL;
...
//cout << "inb RPC hitmap, sizwe " << hit_map->size() << endl;         	
	    ...
	    fNRpcHits11=0;
	    while(hit_iter.next()) { fNRpcHits11++; }
	
//   cout << "inb RPC hitmap, range " << fNRpcHits11 << endl;         
	    if(fNRpcHits11>0){    
	  
	       rpc_t  = new Float_t[fNRpcHits11];
	  
	       rpc_c = new Float_t[fNRpcHits11];
	  
	       arm        = new Int_t[fNRpcHits11];
	       station    = new Int_t[fNRpcHits11];
	       octant     = new Int_t[fNRpcHits11];
	       halfoctant = new Int_t[fNRpcHits11];
	       radsegment = new Int_t[fNRpcHits11];
	       strip      = new Int_t[fNRpcHits11];
	       int rp3oct[fNRpcHits11];
	       int rp3ho[fNRpcHits11];
	  
	       for(int i=0; i<fNRpcHits11; i++) {
...
//	    if ( fNRpcHits11<10){
		  rpc_t[fNRpcHits11] = (float)hit_ptr->get()->get_t();
	    
//      cout << fNRpcHits11 << " " << hit_ptr->get()->get_arm() << " station " << hit_ptr->get()->get_station()  << "  octant  " <<hit_ptr->get()->get_octant()  << "  half " <<hit_ptr->get()->get_half_octant()  << "  seg " <<hit_ptr->get()->get_rseg()  << "  st " <<hit_ptr->get()->get_strip()  << "  time " << (int)hit_ptr->get()->get_t() << endl ;
		  rpc_c[fNRpcHits11] = hit_ptr->get()->get_arm()*100000+ hit_ptr->get()->get_station()*100000+hit_ptr->get()->get_octant()*10000+hit_ptr->get()->get_half_octant()*1000+hit_ptr->get()->get_rseg()*100+hit_ptr->get()->get_strip() ;
      
	    
		  arm[fNRpcHits11] = hit_ptr->get()->get_arm();      
		  station[fNRpcHits11] = hit_ptr->get()->get_station();         
		  octant[fNRpcHits11] = hit_ptr->get()->get_octant();          
		  halfoctant[fNRpcHits11] = hit_ptr->get()->get_half_octant();      
		  radsegment[fNRpcHits11] = hit_ptr->get()->get_rseg();      
		  strip[fNRpcHits11] = hit_ptr->get()->get_strip();           
	    
		  if(station[fNRpcHits11]==2){
...
//      TRpcCoord_v1 *rpccord =  new TRpcCoord_v1(hit_ptr->get()->get_key(),(UShort_t) arm[fNRpcHits11], (UShort_t)station[fNRpcHits11], (UShort_t)octant[fNRpcHits11], (UShort_t)halfoctant[fNRpcHits11], (UShort_t)radsegment[fNRpcHits11], (UShort_t)strip[fNRpcHits11] );
...
//cout << " Station 3 hist" << st3hits << endl;
	       ...
	       ((TLeaf *) fBranch2->GetListOfLeaves()->At(0))->SetAddress(&fNRpcHits11);
...
//cout << " got RPC branches" << endl;
      ...
      if (muo) { 
//cout << " in muo \n";
//  fNMU11 = (muo->get_npart()>0)?1:0;
	 fNMU11 = muo->get_npart();
	 if (fNMU11>0) {
		  
		  
	    RPC2_x= new Float_t[fNMU11];
	    RPC2_y= new Float_t[fNMU11];
	    RPC3_x= new Float_t[fNMU11];
	    RPC3_y= new Float_t[fNMU11];
	    RPC2r_x=new Float_t[fNMU11];
	    RPC2r_y=new Float_t[fNMU11];
	    RPC3r_x=new Float_t[fNMU11];
	    RPC3r_y=new Float_t[fNMU11];
	    ...
	    res100=new Float_t[fNMU11];
	    res101=new Float_t[fNMU11];
	    res110=new Float_t[fNMU11];
	    res111=new Float_t[fNMU11];
	    res120=new Float_t[fNMU11];
	    res121=new Float_t[fNMU11];
	    res200=new Float_t[fNMU11];
	    res201=new Float_t[fNMU11];
	    res210=new Float_t[fNMU11];
	    res211=new Float_t[fNMU11];
	    res220=new Float_t[fNMU11];
	    res221=new Float_t[fNMU11];
	    res300=new Float_t[fNMU11];
	    res301=new Float_t[fNMU11];
	    res310=new Float_t[fNMU11];
	    res311=new Float_t[fNMU11];
	
	
	    station1hits=new Int_t[fNMU11];
	    station2hits=new Int_t[fNMU11];
	    station3hits=new Int_t[fNMU11];
	

	
	    allstation1hits=new Int_t[fNMU11];
	    allstation2hits=new Int_t[fNMU11];
	    allstation3hits=new Int_t[fNMU11];

	    allhits=new Int_t[fNMU11];
	
	    invmass1=new Float_t[fNMU11];
	    invmass2=new Float_t[fNMU11];
	    invmass3=new Float_t[fNMU11];
	
	
	    icharge1=new Int_t[fNMU11];
	    icharge2=new Int_t[fNMU11];
	    icharge3=new Int_t[fNMU11];
	
	    thrust1=new Float_t[fNMU11];
	    thrust2=new Float_t[fNMU11];
	    thrust3=new Float_t[fNMU11];
	

// muon variables
	
	
	    dS30 = new Float_t[fNMU11];     
	    dS3ctp0 = new Float_t[fNMU11];  
	    muIDchis0 = new Float_t[fNMU11];
	    DG0 = new Float_t[fNMU11];	     
	    DG4 = new Float_t[fNMU11];	     
	    DCA_r = new Float_t[fNMU11];	     
	    DCA_z = new Float_t[fNMU11];	     
	    DDG0 = new Float_t[fNMU11];          
	    DS0  = new Float_t[fNMU11];     

	    dS30[0]=999; dS3ctp0[0]=999; muIDchis0[0]=999; DG0[0]=999; DDG0[0]=999; DS0[0]=999;
	
	    pseudo_rapidity = new Float_t[fNMU11];
	    muIDquad0= new Int_t[fNMU11];	 
	    muTRhits0 = new Int_t[fNMU11];		 
	    lastGap= new Int_t[fNMU11]; 
	    ...
	    charge = new Float_t[fNMU11];	
	    muchisquare = new Float_t[fNMU11];
	
	    refit_z = new Float_t[fNMU11];
	    status = new Int_t[fNMU11];
	    newphi = new Float_t[fNMU11];
	    ghostflag = new Float_t[fNMU11];

	
	    gap0x = new Float_t[fNMU11]; 
	    gap0y = new Float_t[fNMU11]; 
	    gap0z = new Float_t[fNMU11];			
	    dirX = new Float_t[fNMU11];
	    dirY = new Float_t[fNMU11];
	    refX  = new Float_t[fNMU11];
	    refY = new Float_t[fNMU11];
	    muTRhits  = new Float_t[fNMU11];
	    muIDhits = new Int_t[fNMU11];


	    mutr_nhits = new Float_t[fNMU11];
	    muid_nhits = new Float_t[fNMU11];
	
	    px =         new Float_t[fNMU11];
	    py =         new Float_t[fNMU11];
	    pz =         new Float_t[fNMU11];
	
	    xSTI  =      new Float_t[fNMU11];
	    ySTI  =      new Float_t[fNMU11];
	    zSTI  =      new Float_t[fNMU11];
	    pxSTI =      new Float_t[fNMU11];
	    pySTI =      new Float_t[fNMU11];
	    pzSTI =      new Float_t[fNMU11];
	
	    xSTII  =     new Float_t[fNMU11];
	    ySTII  =     new Float_t[fNMU11];
	    zSTII  =     new Float_t[fNMU11];
	 
	    xSTIII  =    new Float_t[fNMU11];
	    ySTIII  =    new Float_t[fNMU11];
	    zSTIII  =    new Float_t[fNMU11];
	    pxSTIII =    new Float_t[fNMU11];
	    pySTIII =    new Float_t[fNMU11];
	    pzSTIII =    new Float_t[fNMU11];
	 
	    pT =         new Float_t[fNMU11];
	    p =          new Float_t[fNMU11];
	    pSTI =       new Float_t[fNMU11];
	    ELoss =      new Float_t[fNMU11];
	
	
	    rpc3z = new  Float_t [fNMU11];
	    rpc2z = new	 Float_t[fNMU11]; 
	
	    rpc3st3  = new	 Float_t[fNMU11]; 
	    rpc2st3  = new	 Float_t[fNMU11]; 
	    rpc2g5   = new	 Float_t[fNMU11]; 
	    rpc3g5  = new	 Float_t[fNMU11]; 

	    Float_t costheta;
	    Float_t costheta_xyz;
	    Float_t xyz_St1;
	
	    dANGLE = new Float_t[fNMU11];
	    dANGLE_xyz= new Float_t[fNMU11];
	    dPHI= new Float_t[fNMU11];
	    dTHETA= new Float_t[fNMU11];
	    ROAD_SLOPE= new Float_t[fNMU11];
	
	
	    ref_vtx_rdca = new Float_t[fNMU11];
	    ref_vtx_r = new Float_t[fNMU11];
	    ref_vtx_z = new Float_t[fNMU11];
	
	
	    pseudotrigNS_1D   = new Int_t[fNMU11];
	    pseudotrigS_SG1D   = new Int_t[fNMU11];
	    pseudotrigNS_1H   = new Int_t[fNMU11];
	    pseudotrigN_SG1D   = new Int_t[fNMU11];
	    recoNS_1D = new Int_t[fNMU11];
	    recoS_SG1D = new Int_t[fNMU11];
	    recoNS_1H = new Int_t[fNMU11];
	    recoN_SG1D = new Int_t[fNMU11];	

	    MC_N_PART = new Float_t[fNMU11];   // total # of MC particles associated with this reco'd track
	    MC_PX= new     Float_t[fNMU11];        // the major MC particle's px
	    MC_PY= new     Float_t[fNMU11];
	    MC_PZ= new     Float_t[fNMU11];
	    MC_PTOT = new  Float_t[fNMU11];      // the major MC particle's lastGap
	    MC_X= new      Float_t[fNMU11];         // the major MC particle's X_orign
	    MC_Y= new      Float_t[fNMU11];         // the major MC particle's Y_orign
	    MC_Z= new      Float_t[fNMU11];         // the major MC particle's Z_orign
	    MC_PID = new   Float_t[fNMU11];
	    MC_HITS = new  Float_t[fNMU11];     // the major MC particle's total contribution to muTr hits
	
	    MC_P_PID = new Float_t[fNMU11];    // MC particle's parent information
	    MC_P_PX = new  Float_t[fNMU11];     //
	    MC_P_PY = new  Float_t[fNMU11];
	    MC_P_PZ = new  Float_t[fNMU11];
	    MC_P_PTOT = new Float_t[fNMU11] ;
	    MC_P_Z = new   Float_t[fNMU11];
	
	    MC_D_PID = new Float_t[fNMU11];    // MC particle's daughter information
	    MC_D_PX = new  Float_t[fNMU11];     //
	    MC_D_PY = new  Float_t[fNMU11];
	    MC_D_PZ = new  Float_t[fNMU11];
	    MC_D_PTOT = new Float_t[fNMU11];
	    MC_D_Z = new   Float_t[fNMU11] ;
	    MC_D_N = new   Float_t[fNMU11] ;
	
	    MC_G_PID = new Float_t[fNMU11] ;    // MC particle's grandparent (ancestor, original, very primary) informationi
	    MC_G_PX = new  Float_t[fNMU11] ;     //
	    MC_G_PY = new  Float_t[fNMU11] ;
	    MC_G_PZ = new  Float_t[fNMU11] ;
	    MC_G_PTOT = new Float_t[fNMU11]  ;
	    MC_G_Z = new   Float_t[fNMU11] ;
	
	    MC_TRK   = new  Int_t[fNMU11]   ;
	    MC_P_TRK = new  Int_t[fNMU11]   ;    // parent track ID: for debuggin now
	    MC_D_TRK = new  Int_t[fNMU11]   ;    // daughter track ID: for debuggin now
	    MC_G_TRK = new  Int_t[fNMU11];    // grandparent track ID: for debuggin now

	    RpcDCA  = new Float_t[fNMU11];
	    RpcpT   = new Float_t[fNMU11];
	    Rpctime = new Float_t[fNMU11];
	    //	    printf("before defining DCA zmult\n");
	    DCA_zmult0 = new Float_t[fNMU11];	     	
	    DCA_zmult1 = new Float_t[fNMU11];	     
	    DCA_zmult2 = new Float_t[fNMU11];	     	
	    DCA_zmult3 = new Float_t[fNMU11];	     	
	    DCA_zmult4 = new Float_t[fNMU11];	     	
	    DCA_zmult5 = new Float_t[fNMU11];	     
	    DCA_zmult6 = new Float_t[fNMU11];	     	
	    DCA_zmult7 = new Float_t[fNMU11];	     	
	    DCA_zmult8 = new Float_t[fNMU11];	     	
	    zmult0 = new Float_t[fNMU11];	     	
	    zmult1 = new Float_t[fNMU11];	     
	    zmult2 = new Float_t[fNMU11];	     	
	    zmult3 = new Float_t[fNMU11];	     	
	    zmult4 = new Float_t[fNMU11];	     	
	    zmult5 = new Float_t[fNMU11];	     
	    zmult6 = new Float_t[fNMU11];	     	
	    zmult7 = new Float_t[fNMU11];	     	
	    zmult8 = new Float_t[fNMU11];	     	
	
	
	    npart = new int[fNMU11];

	    for(int i=0; i<fNMU11; i++){ 
	       RPC2_x [i]=-9999;
	       RPC2_y [i]=-9999;
	       RPC3_x [i]=-9999;
	       RPC3_y [i]=-9999;
	       ...
	       ref_vtx_r  [i]=-9999;
	       ref_vtx_z  [i]=-9999;
	       pseudotrigNS_1D    [i]=-9999;
	       pseudotrigS_SG1D    [i]=-9999;
	       pseudotrigNS_1H    [i]=-9999;
...
//	float v2_obs_S = cos(2*dphi_S);
	       ...
	       pseudotrigNS_1D[ipart]   = -1;
	       pseudotrigS_SG1D[ipart]   = -1;
	       pseudotrigNS_1H[ipart]   = -1;
		  ...
		  pseudotrigN_SG1D[ipart] =  Tools::LL1_1S_Decision( MUTOO::North );
		  pseudotrigNS_1H[ipart] =  Tools::LL1_1D_Decision( MUTOO::North );
	  
		  pseudotrigS_SG1D[ipart] =  Tools::LL1_1S_Decision( MUTOO::South );
		  pseudotrigNS_1D[ipart] =  Tools::LL1_1D_Decision( MUTOO::South );
	       }
...
//cout << " after tracks loop" << endl;
	    ...
	    ((TLeaf *) fBranch->GetListOfLeaves()->At(0))->SetAddress(&fNMU11);
...
//   cout << "pseudorap is " << pseudo_rapidity[0] << endl;
	    ...
	    ((TLeaf *) fBranch->GetListOfLeaves()->At(39))->SetAddress(pseudotrigNS_1D);
	    ((TLeaf *) fBranch->GetListOfLeaves()->At(40))->SetAddress(pseudotrigS_SG1D);
	    ((TLeaf *) fBranch->GetListOfLeaves()->At(41))->SetAddress(pseudotrigNS_1H);
...
//      if ( fNMU11 >0  || fNRpcHits11>0) { //used for debugging, but gives lots of trackless rpc events
      if ( fNMU11 >0 ) {  //used for only good muon tracks
//  if ( fNMU11 >0  && fNRpcHits11>0) {
	 ...
	 delete [] ref_vtx_r;
	 delete [] ref_vtx_z;
	 delete [] pseudotrigNS_1D;
	 delete [] pseudotrigS_SG1D;
	 delete [] pseudotrigNS_1H;

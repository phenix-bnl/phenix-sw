...
#ifndef __SVXPIXELHOTDEADMAP_HH_
#define __SVXPIXELHOTDEADMAP_HH_
...
public:
  ...
  /** Constructor */
  SvxPixelHotDeadMap(const int verbosityLevel = 0);
   ...
   */
  bool readChipMapFromFile(int &runStart, int &runEnd, std::string filename);
  bool readPixelMapFromFile(int &runStart, int &runEnd, std::string filename);
  bool readPixelMapFromFile(int &runStart, int &runEnd, const int module, const int ROC, std::string filename);
  bool writeChipMapToFile(const int runStart, const int runEnd, std::string filename);
  bool writePixelMapToFile(const int runStart, const int runEnd, std::string filename);
  bool writePixelMapToFile(const int runStart, const int runEnd, const int module, const int ROC, std::string filename);
   ...
   */
  bool readFromDatabase(const int run, bool read_reference=false);
  bool readChipMapFromDatabase(const int run);
  bool readPixelMapFromDatabase(const int run);
  bool readPixelMapFromDatabase(const int run, const int module, const int ROC);
  bool readReferencePixelMapFromDatabase(const int run);
  bool readReferencePixelMapFromDatabase(const int run, const int module, const int ROC);
  ...
  bool writeReferencePixelMapToDatabase(const int runStart, const int runEnd, const int module, const int ROC);
  bool writePixelMapToDatabase(const int runStart, const int runEnd, const int module, const int ROC);
  ...
  void printBadPixels(int module, int roc) const;
  
  void setBlankStatus(char blank);
  
private:
  void initializeMaps();
  void initializeChipMap();
  void initializePixelMap(int module, int ROC);
  bool rangeCheck(int module, int ROC) const;
  bool rangeCheck(int module, int ROC, int column,int row) const;
  bool rangeCheck(int module, int ROC, int tile) const;
  ...
  char map[NMODULE][NROC][NCOLUMN][NROW];
  ...
  int verbosity;
  int countBadChips();
  int countBadPixels(int module, int ROC);

  bool m_useChipMap;
  bool m_usePixelMap;
  
  int reference_map_bankID_offset;
  void setReferenceOffset(){reference_map_bankID_offset = (1<<16);}

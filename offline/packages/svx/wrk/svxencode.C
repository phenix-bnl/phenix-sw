...
void svxencode(
   ...
   char *fileout="/phenix/subsys/vtx/lebedev/newsim/simdst/simDST_tmp_00.root"
...
//  (dynamic_cast<SvxReco*>(svxrec))->set_ReadParFromFile(0);
//  svxrec->Verbosity(0);
//  se->registerSubsystem(svxrec);

  SubsysReco *svxencode = new SvxEncode();
  svxencode->Verbosity(1);
  se->registerSubsystem(svxencode);

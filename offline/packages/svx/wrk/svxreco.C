...
(
 int nEvents = 20000, 
 char *filein =  "/phenix/subsys/vtx/julyaf/pisafiles/PISAEvent_phpythia19_20evts.root",
 char *fileout = "/phenix/subsys/vtx/julyaf/simdst/simDSTsvxonly_phpythia19_20evts.root"
 )
{
  cout << "Loading libraries...\n";
  //gSystem->Load("/phenix/subsys/vtx/akimoto/svx/svxsrc/install/lib/libsvx.so");  /// put your svx library
  gSystem->Load("libsvx.so");  /// put your svx library
...
//  (dynamic_cast<SvxSimulator*>(svxsim))->set_StripixelNoise(15.0);
...
//  svxsim->Verbosity(1);
  ...
  (dynamic_cast<SvxReco*>(svxrec))->set_ThisIsSimulation();
//  svxrec->Verbosity(1);
...
//  svxstandalone->Verbosity(1);
  ...
  simDST->AddNode("SvxGhitRawhitList");
  simDST->AddNode("SvxRawhitClusterList");
  simDST->AddNode("SvxGhitClusterList");

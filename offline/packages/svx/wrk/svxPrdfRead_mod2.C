...
//  (dynamic_cast<SvxSimulator*>(svxsim))->set_ReadParFromFile(0);
...
*/
  ...
  (dynamic_cast<SvxDecode*>svxdecode)->setAdcCutoff(-24);
  se->registerSubsystem(svxdecode);

  SubsysReco *svxrec = new SvxReco();
//  (dynamic_cast<SvxReco*>(svxrec))->set_ReadParFromFile(1);
  (dynamic_cast<SvxReco*>(svxrec))->set_StripixelAdcThreshold(90);
  (dynamic_cast<SvxReco*>(svxrec))->set_StripixelAdcSumThreshold(90);
  svxrec->Verbosity(1);

// ====================
// FILE: SvxCentralTrackv6.C
// ====================

#include "SvxCentralTrackv6.h"

ClassImp(SvxCentralTrackv6)

using namespace std;

SvxCentralTrackv6::SvxCentralTrackv6()
{  
  Reset();
}

void SvxCentralTrackv6::Reset()
{
  m_dchindex = -9999;

  m_ninfo = 0;
  for(int i=0; i<MAXCLUSTERS; i++){
    m_vinfo[i] = SvxClusterInfov4();
  }
  m_hitpattern = 0;
...
void  SvxCentralTrackv6::addClusterInfo(SvxClusterInfo* cinfo){
  SvxClusterInfov4* cinfov4 = dynamic_cast<SvxClusterInfov4 *>(cinfo);
  if(cinfov4!=NULL) {
    if(m_ninfo<MAXCLUSTERS){
      int sublayer = cinfov4->get_sublayer();
...
//      cout<<"DEBUG : "<<sublayer<<" "<<(int)cinfov3->getLayer()<<" "<<(int)cinfov3->getLadder()<<endl;
      ...
      m_vinfo[m_ninfo] = *cinfov4;
      m_ninfo++;
    }
    else {
      cout<<" SvxCentralTrackv6::addClusterInfo : N associated tracks exceeds "<<MAXCLUSTERS<<endl;
    }
  } 
  else {
    cout<<"SvxCentralTrackv6::addClusterInfo failed to add"<<endl;
  }
}

SvxClusterInfo* SvxCentralTrackv6::getClusterInfo(int hit){
  if(isValidHit(hit)) {
    return &(m_vinfo[hit]);
  }
  return NULL;
...
short SvxCentralTrackv6::getNhits(){
  return (short)m_ninfo;
}

short SvxCentralTrackv6::getNhits(int layer){
  short int nhit = 0;
  if(0<=layer&&layer<2) 
  {
...
float SvxCentralTrackv6::getClosestApproach(int coor){
...
float SvxCentralTrackv6::get3MomentumAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {
...
float SvxCentralTrackv6::get3MomErrorAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {
...
void SvxCentralTrackv6::addSecondHit(int layer, SvxResidualInfo* cinfo){
  if(!isValidResidualLayer(layer)){
    cerr<<"SvxCentralTrackRecov6::addSecondHit InvaridLayer : "<<layer<<endl;
    return;
  }
  ...
  if(cinfov1!=NULL) {
    ...
    cout<<"SvxCentralTrackv6::addResidualInfo failed to add"<<endl;
  }
}

SvxResidualInfo* SvxCentralTrackv6::getSecondHit(int layer, int hit){

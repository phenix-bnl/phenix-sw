// ====================
// FILE: SvxCentralTrackv4.C
// ====================

#include "SvxCentralTrackv4.h"

ClassImp(SvxCentralTrackv4)

using namespace std;

SvxCentralTrackv4::SvxCentralTrackv4()
{  
  Reset();
}

void SvxCentralTrackv4::Reset()
{
  m_dchindex = -9999;
...
void  SvxCentralTrackv4::addClusterInfo(SvxClusterInfo* cinfo){
  SvxClusterInfov2* cinfov2 = dynamic_cast<SvxClusterInfov2 *>(cinfo);
  if(cinfov2!=NULL) {
    if(m_ninfo<MAXCLUSTERS){
      int sublayer = cinfov2->get_sublayer();
      if(0<=sublayer&&sublayer<8) m_hitpattern |= (0x1<<sublayer);
      else                     {cout<<"unknown sublayer : "<<sublayer<<endl; return;}
//      cout<<"DEBUG : "<<sublayer<<" "<<(int)cinfov2->getLayer()<<" "<<(int)cinfov2->getLadder()<<endl;

      m_vinfo[m_ninfo] = *cinfov2;
      m_ninfo++;
    }
    else {
      cout<<" SvxCentralTrackv4::addClusterInfo : N associated tracks exceeds "<<MAXCLUSTERS<<endl;
    }
  } 
  else {
    cout<<"SvxCentralTrackv4::addClusterInfo failed to add"<<endl;
  }
}

SvxClusterInfo* SvxCentralTrackv4::getClusterInfo(int hit){
  if(isValidHit(hit)) {
    return &(m_vinfo[hit]);
  }
  return NULL;
...
short SvxCentralTrackv4::getNhits(){
  return (short)m_ninfo;
}

short SvxCentralTrackv4::getNhits(int layer){
  short int nhit = 0;
  if(0<=layer&&layer<2) 
  {
...
float SvxCentralTrackv4::getClosestApproach(int coor){
...
float SvxCentralTrackv4::get3MomentumAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {
...
float SvxCentralTrackv4::get3MomErrorAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {
...
void SvxCentralTrackv4::addSecondHit(int layer, SvxResidualInfo* cinfo){
  if(!isValidResidualLayer(layer)){
    cerr<<"SvxCentralTrackRecov4::addSecondHit InvaridLayer : "<<layer<<endl;
    return;
  }
  ...
  if(cinfov1!=NULL) {
    ...
    cout<<"SvxCentralTrackv4::addResidualInfo failed to add"<<endl;
  }
}

SvxResidualInfo* SvxCentralTrackv4::getSecondHit(int layer, int hit){

// ========================
// FILE: SvxClusterListv2.C
// ========================
// ****************************************************************************
// Implementation of the container SvxClusterListv2 for the SvxClusterv2
// ---
// Created by Sasha Lebedev <lebedev@iastate.edu> in July 2010
//
// ****************************************************************************

#include <SvxClusterListv2.h>
...
ClassImp(SvxClusterListv2)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxClusterListv2::SvxClusterListv2(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxClusterv2", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxClusterListv2 object created" << std::endl;
}

SvxClusterListv2::~SvxClusterListv2()
{
  //std::cout << "SvxClusterListv2 object destroyed" << std::endl;
}

// The "standard PHObject response" functions...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxClusterListv2::Reset()
{
  m_hit_id_unused = 0;
  m_hit_list->Clear();
...
int SvxClusterListv2::isValid() const
{
  return ( ( get_nClusters() > 0 ) ? 1 : 0 ) ;
}

void SvxClusterListv2::identify(std::ostream &os) const
{
  os << "SvxClusterListv2 object: nClusters = "
     << get_nClusters() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxCluster* SvxClusterListv2::addCluster(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nClusters() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxCluster* cluster = new(P[index]) SvxClusterv2(this);
...
void SvxClusterListv2::removeCluster (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxClusterListv2::Compress()
{
  m_hit_list->Compress();
  return get_nClusters();
}

int SvxClusterListv2::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxClusterListv2::restoreListPointers ()
{
  SvxClusterv2* cluster = NULL;
  for (int i = 0; i < get_nClusters(); i++ ) {
    if ( ( cluster = (SvxClusterv2*) m_hit_list->UncheckedAt(i) ) )
      cluster->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxClusterListv2::sort_hitID () {
...
void SvxClusterListv2::sort_sensorID () {
...
void SvxClusterListv2::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
  m_hit_list->UnSort();
}

void SvxClusterListv2::SortSTL()
{
...
int SvxClusterListv2::indexOfCluster (
   const int hitid, int ifrom, int iupto) const
{
  ...
  if (ifrom < 0 || iupto >= get_nClusters() || ifrom > iupto) {
     std::cerr << "SvxClusterListv2::indexOfCluster(): Invalid search range, from "
...
//int SvxClusterListv2::indexOfCluster (const SvxSensor* sensor,
//				      const int         ifrom,
...
int SvxClusterListv2::indexOfCluster (const SvxCluster* hit  ,
				      const int         ifrom,
				      const int         iupto) const {
  int indto = ( iupto < 0 ) ? get_nClusters() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxClusterListv2::indexOfCluster(const SvxSensor* sensor, 
                                      int& idx_lb, int& idx_ub, 
                                      int ifrom, int iupto) const
{
   if (! sensorIDsorted) {
      std::cerr << "SvxClusterListv2::indexOfCluster(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   SvxClusterv2* hit_cmp = new SvxClusterv2();
...
// Methods
// """""""
void SvxClusterListv2::print() const
{
  std::cout << "SvxClusterListv2: nClusters = " << get_nClusters()

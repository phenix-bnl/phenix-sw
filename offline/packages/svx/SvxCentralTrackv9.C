// ====================
// FILE: SvxCentralTrackv9.C
// ====================

#include "SvxCentralTrackv9.h"

ClassImp(SvxCentralTrackv9)

using namespace std;

SvxCentralTrackv9::SvxCentralTrackv9()
{  
  Reset();
}

void SvxCentralTrackv9::Reset()
{
  m_dchindex = -9999;
...
void  SvxCentralTrackv9::addClusterInfo(SvxClusterInfo* cinfo) {
...
//      cout<<"DEBUG : "<<sublayer<<" "<<(int)cinfov3->getLayer()<<" "<<(int)cinfov3->getLadder()<<endl;
      ...
      cout<<" SvxCentralTrackv9::addClusterInfo : N associated tracks exceeds "<<MAXCLUSTERS<<endl;
    }
  } 
  else {
    cout<<"SvxCentralTrackv9::addClusterInfo failed to add"<<endl;
  }
}

SvxClusterInfo* SvxCentralTrackv9::getClusterInfo(int hit) {
  if(isValidHit(hit)) {
    return &(m_vinfo[hit]);
  }
  return NULL;
...
void SvxCentralTrackv9::setLivePercentage(int layer, float livepercent)
{
  if (layer < 0 || layer > 3) {
...
float SvxCentralTrackv9::getLivePercentage(int layer)
{
  if (layer < 0 || layer > 3) {
    cerr << PHWHERE << "Invalid Layer: " << layer << endl;
    return -1;
...
short SvxCentralTrackv9::getNhits() {
  return (short)m_ninfo;
}

short SvxCentralTrackv9::getNhits(int sublayer) {
  short int nhit = 0;
...
float SvxCentralTrackv9::getClosestApproach(int coor) {
...
float SvxCentralTrackv9::get3MomentumAtPrimaryVertex(int coor) {
  if(isValidCoordinate(coor)) {
...
//--float SvxCentralTrackv9::get3MomErrorAtPrimaryVertex(int coor) {
//--  if(isValidCoordinate(coor)) {

// ====================
// FILE: SvxCentralTrackv2.C
// ====================

#include "SvxCentralTrackv2.h"

ClassImp(SvxCentralTrackv2)

using namespace std;

SvxCentralTrackv2::SvxCentralTrackv2()
{  
  Reset();
}

void SvxCentralTrackv2::Reset()
{
  m_dchindex = -9999;
...
void  SvxCentralTrackv2::addClusterInfo(SvxClusterInfo* cinfo){
  ...
  if(cinfov1!=NULL) {
    if(m_ninfo<MAXCLUSTERS){
      ...
      cout<<" SvxCentralTrackv2::addClusterInfo : N associated tracks exceeds "<<MAXCLUSTERS<<endl;
    }
  } 
  else {
    cout<<"SvxCentralTrackv2::addClusterInfo failed to add"<<endl;
  }
}

SvxClusterInfo* SvxCentralTrackv2::getClusterInfo(int hit){
  if(isValidHit(hit)) {
    return &(m_vinfo[hit]);
  }
  return NULL;
...
short SvxCentralTrackv2::getNhits(){
  return (short)m_ninfo;
}


float SvxCentralTrackv2::getClosestApproach(int coor){
...
float SvxCentralTrackv2::get3MomentumAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {
...
float SvxCentralTrackv2::get3MomErrorAtPrimaryVertex(int coor){
  if(isValidCoordinate(coor)) {

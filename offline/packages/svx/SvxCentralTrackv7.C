// ====================
// FILE: SvxCentralTrackv7.C
// ====================

#include "SvxCentralTrackv7.h"

ClassImp(SvxCentralTrackv7)

using namespace std;

SvxCentralTrackv7::SvxCentralTrackv7()
{  
  Reset();
}

void SvxCentralTrackv7::Reset()
{
  m_dchindex = -9999;
...
void  SvxCentralTrackv7::addClusterInfo(SvxClusterInfo* cinfo) {
...
//      cout<<"DEBUG : "<<sublayer<<" "<<(int)cinfov3->getLayer()<<" "<<(int)cinfov3->getLadder()<<endl;
      ...
      cout<<" SvxCentralTrackv7::addClusterInfo : N associated tracks exceeds "<<MAXCLUSTERS<<endl;
    }
  } 
  else {
    cout<<"SvxCentralTrackv7::addClusterInfo failed to add"<<endl;
  }
}

SvxClusterInfo* SvxCentralTrackv7::getClusterInfo(int hit) {
  if(isValidHit(hit)) {
    return &(m_vinfo[hit]);
  }
  return NULL;
...
void SvxCentralTrackv7::setLivePercentage(int layer, float livepercent)
{
  if (layer < 0 || layer > 3) {
...
float SvxCentralTrackv7::getLivePercentage(int layer)
{
  if (layer < 0 || layer > 3) {
    cerr << PHWHERE << "Invalid Layer: " << layer << endl;
    return -1;
...
short SvxCentralTrackv7::getNhits() {
  return (short)m_ninfo;
}

short SvxCentralTrackv7::getNhits(int sublayer) {
  short int nhit = 0;
...
float SvxCentralTrackv7::getClosestApproach(int coor) {
...
float SvxCentralTrackv7::get3MomentumAtPrimaryVertex(int coor) {
  if(isValidCoordinate(coor)) {
...
float SvxCentralTrackv7::get3MomErrorAtPrimaryVertex(int coor) {
  if(isValidCoordinate(coor)) {
...
void SvxCentralTrackv7::addSecondHit(int layer, SvxResidualInfo* cinfo) {
  if(!isValidResidualLayer(layer)) {
    cerr<<"SvxCentralTrackRecov7::addSecondHit InvaridLayer : "<<layer<<endl;
    return;
  }
  ...
  if(cinfov1!=NULL) {
    ...
    cout<<"SvxCentralTrackv7::addResidualInfo failed to add"<<endl;
  }
}

SvxResidualInfo* SvxCentralTrackv7::getSecondHit(int layer, int hit) {
 return (isValidResidualLayer(layer)) ? 
          (&m_vresidual[layer][hit]) : NULL;
}

void SvxCentralTrackv7::print()
{

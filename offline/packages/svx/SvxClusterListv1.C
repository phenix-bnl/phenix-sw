// ========================
// FILE: SvxClusterListv1.C
// ========================
// ****************************************************************************
// Implementation of the container SvxClusterListv1 for the SvxClusterv1
...
ClassImp(SvxClusterListv1)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxClusterListv1::SvxClusterListv1(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxClusterv1", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxClusterListv1 object created" << std::endl;
}

SvxClusterListv1::~SvxClusterListv1()
{
  delete m_hit_list;
}
...
int SvxClusterListv1::isValid() const
{
  return ( ( get_nClusters() > 0 ) ? 1 : 0 ) ;
}

void SvxClusterListv1::identify(std::ostream &os) const
{
  os << "Identify yourself: SvxClusterListv1 object: nClusters = "
     << get_nClusters() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxCluster* SvxClusterListv1::addCluster(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nClusters() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxCluster* cluster = new(P[index]) SvxClusterv1(this);
...
void SvxClusterListv1::removeCluster (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxClusterListv1::Compress()
{
  m_hit_list->Compress();
  return get_nClusters();
}

int SvxClusterListv1::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxClusterListv1::restoreListPointers ()
{
  SvxClusterv1* cluster = NULL;
  for (int i = 0; i < get_nClusters(); i++ ) {
    if ( ( cluster = (SvxClusterv1*) m_hit_list->UncheckedAt(i) ) )
      cluster->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxClusterListv1::sort_hitID () {
...
void SvxClusterListv1::sort_sensorID () {
...
void SvxClusterListv1::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
  m_hit_list->UnSort();
}

void SvxClusterListv1::SortSTL()
{
...
int SvxClusterListv1::indexOfCluster (
   const int hitid, int ifrom, int iupto) const
{
  ...
  if (ifrom < 0 || iupto >= get_nClusters() || ifrom > iupto) {
     std::cerr << "SvxClusterListv1::indexOfCluster(): Invalid search range, from "
...
//int SvxClusterListv1::indexOfCluster (const SvxSensor* sensor,
//				      const int         ifrom,
...
int SvxClusterListv1::indexOfCluster (const SvxCluster* hit  ,
				      const int         ifrom,
				      const int         iupto) const {
  int indto = ( iupto < 0 ) ? get_nClusters() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxClusterListv1::indexOfCluster(const SvxSensor* sensor, 
                                      int& idx_lb, int& idx_ub, 
                                      int ifrom, int iupto) const
{
   if (! sensorIDsorted) {
      std::cerr << "SvxClusterListv1::indexOfCluster(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   SvxClusterv1* hit_cmp = new SvxClusterv1();
...
// Methods
// """""""
void SvxClusterListv1::print() const
{
  std::cout << "SvxClusterListv1: nClusters = " << get_nClusters()

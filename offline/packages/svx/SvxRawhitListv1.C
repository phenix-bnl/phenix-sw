// =======================
// FILE: SvxRawhitListv1.C
// =======================
// ****************************************************************************
// Implementation of the container SvxRawhitListv1 for the SvxRawhitv1
// Template used: SvxGhitListv1.C by Jeffery Mitchell as of 11/20/2003
// ---
// Created  by V. L. Rykov on 09-Mar-2003
//
// Modified by V. L. Rykov on 13-May-2004:
...
ClassImp(SvxRawhitListv1)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxRawhitListv1::SvxRawhitListv1(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxRawhitv1", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxRawhitListv1 object created" << std::endl;
}

SvxRawhitListv1::~SvxRawhitListv1()
{
  delete m_hit_list;
}
...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxRawhitListv1::Reset()
{
  m_hit_id_unused = 0;
  m_hit_list->Clear();
  if ( m_hit_list->GetSize() > SVXNRAWHIT ) 
    {
       m_hit_list->Expand(SVXNRAWHIT);
    }
  unSort();
  listPointersSet = false;
}

int SvxRawhitListv1::isValid() const
{
  return ( ( get_nRawhits() > 0 ) ? 1 : 0 ) ;
}

void SvxRawhitListv1::identify(std::ostream &os) const
{
  os << "Identify yourself: SvxRawhitListv1 object: nRawhits = "
     << get_nRawhits() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxRawhit* SvxRawhitListv1::addRawhit(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nRawhits() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxRawhit* rawhit = new(P[index]) SvxRawhitv1(this);
...
void SvxRawhitListv1::removeRawhit (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxRawhitListv1::Compress()
{
  m_hit_list->Compress();
  return (int) get_nRawhits();
}

int SvxRawhitListv1::set_TClonesArraySize (const unsigned int nhit)
{
  if ( nhit > (unsigned int)get_nRawhits() )
    {
      m_hit_list->Expand(nhit);
    }
...
// """"""""""""""""""""""""
void SvxRawhitListv1::restoreListPointers ()
{
  SvxRawhitv1* rawhit = NULL;
  for (int i = 0; i < get_nRawhits(); i++ ) {
    if ( ( rawhit = (SvxRawhitv1*) m_hit_list->UncheckedAt(i) ) )
      rawhit->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxRawhitListv1::sort_hitID () {
...
void SvxRawhitListv1::sort_sensorID () {
...
void SvxRawhitListv1::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
//  m_hit_list->UnSort();
}

void SvxRawhitListv1::SortSTL()
{
...
int SvxRawhitListv1::indexOfRawhit (
   const int hitid, int ifrom, int iupto) const
{
  ...
  if (ifrom < 0 || iupto >= get_nRawhits() || ifrom > iupto) {
     std::cerr << "SvxHitListv1::FindHitIndex(): Invalid search range, from "
...
//int SvxRawhitListv1::indexOfRawhit (const SvxSensor* sensor,
//				    const int         ifrom,
...
//                                       && channel       (npar=3) 
int SvxRawhitListv1::indexOfRawhit (      SvxRawhit& rawhit,
				    const int        npar  ,
				    const int        ifrom ,
				    const int        iupto ) const {
  ...
  SvxRawhitv1* hit = (SvxRawhitv1*) &rawhit;
  hit->set_sortingSwitch(1);
  ...
  if ( indto-ifrom > 8 && sensorIDsorted ) {
	...
	if ( ( phit = get_Rawhit(i) ) ) {
	  if ( hit->Compare(phit) ) {
	    ...
	    if ( npar > 2 )
               std::cout << "SvxRawhitListv1::indexOfRawhit--WARNING: "
...
int SvxRawhitListv1::indexOfRawhit (const SvxRawhit* hit  ,
				    const int        ifrom,
				    const int        iupto) const {
  int indto = ( iupto < 0 ) ? get_nRawhits() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxRawhitListv1::indexOfRawhit(const SvxSensor* sensor, 
                                    int& idx_lb, int& idx_ub, 
                                    int ifrom, int iupto) const
{
   if (! sensorIDsorted) {
      std::cerr << "SvxRawhitListv1::indexOfRawhit(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   if (iupto < 0) iupto = get_nRawhits() - 1;
   if (ifrom < 0 || iupto >= get_nRawhits() || ifrom > iupto) {
      std::cerr << "SvxRawhitListv1::indexOfRawhit(): Invalid search range, from "
           << ifrom << " upto " << iupto << ", where the hit list has " 
           << get_nRawhits() << " entries." << std::endl;
      exit(1);
   }

   SvxRawhitv1* hit_cmp = new SvxRawhitv1();
...
// Methods
// """""""
void SvxRawhitListv1::print() const
{
   std::cout << "SvxRawhitListv1: nRawhits = " << get_nRawhits()

// =======================
// FILE: SvxRawhitListv4.C
// =======================
// ****************************************************************************
// Implementation of the container SvxRawhitListv4 for the SvxRawhitv4
// ****************************************************************************
...
ClassImp(SvxRawhitListv4)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxRawhitListv4::SvxRawhitListv4(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxRawhitv4", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxRawhitListv4 object created" << std::endl;
}

SvxRawhitListv4::~SvxRawhitListv4()
{
  delete m_hit_list;
  //std::cout << "SvxRawhitListv4 object destroyed" << std::endl;
}

// The "standard PHObject response" functions...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxRawhitListv4::Reset()
{
...
int SvxRawhitListv4::isValid() const
{
  return ( ( get_nRawhits() > 0 ) ? 1 : 0 ) ;
}

void SvxRawhitListv4::identify(std::ostream &os) const
{
  os << "Identify yourself: SvxRawhitListv4 object: nRawhits = "
     << get_nRawhits() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxRawhit* SvxRawhitListv4::addRawhit(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nRawhits() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxRawhit* rawhit = new(P[index]) SvxRawhitv4(this);
...
void SvxRawhitListv4::removeRawhit (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxRawhitListv4::Compress()
{
  m_hit_list->Compress();
  return (int) get_nRawhits();
}

int SvxRawhitListv4::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxRawhitListv4::restoreListPointers ()
{
  SvxRawhitv4* rawhit = NULL;
  for (int i = 0; i < get_nRawhits(); i++ ) {
    if ( ( rawhit = (SvxRawhitv4*) m_hit_list->UncheckedAt(i) ) )
      rawhit->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxRawhitListv4::sort_hitID () {
...
void SvxRawhitListv4::sort_sensorID () {
...
void SvxRawhitListv4::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
//  m_hit_list->UnSort();
}

void SvxRawhitListv4::SortSTL()
{
...
int SvxRawhitListv4::indexOfRawhit (
...
//int SvxRawhitListv4::indexOfRawhit (const SvxSensor* sensor,
//				    const int         ifrom,
...
//                                       && channel       (npar=3) 
int SvxRawhitListv4::indexOfRawhit (      SvxRawhit& rawhit,
				    const int        npar  ,
				    const int        ifrom ,
				    const int        iupto ) const {
  ...
  SvxRawhitv4* hit = (SvxRawhitv4*) &rawhit;
  hit->set_sortingSwitch(1);
  ...
  if ( indto-ifrom > 8 && sensorIDsorted ) {
	...
	if ( ( phit = get_Rawhit(i) ) ) {
	  if ( hit->Compare(phit) ) {
	    ...
	    if ( npar > 2 )
               std::cout << "SvxRawhitListv4::indexOfRawhit--WARNING: "
...
int SvxRawhitListv4::indexOfRawhit (const SvxRawhit* hit  ,
				    const int        ifrom,
				    const int        iupto) const {
  int indto = ( iupto < 0 ) ? get_nRawhits() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxRawhitListv4::indexOfRawhit(const SvxSensor* sensor, 
                                    int& idx_lb, int& idx_ub, 
                                    int ifrom, int iupto) const
{

   if (! sensorIDsorted) {
      std::cerr << "SvxRawhitListv4::indexOfRawhit(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   if (iupto < 0) iupto = get_nRawhits() - 1;
   if (ifrom < 0 || iupto >= get_nRawhits() || ifrom > iupto) {
      std::cerr << "SvxRawhitListv4::indexOfRawhit(): Invalid search range, from "
           << ifrom << " upto " << iupto << ", where the hit list has " 
           << get_nRawhits() << " entries." << std::endl;
      exit(1);
   }

   SvxRawhitv4* hit_cmp = new SvxRawhitv4();
...
// Methods
// """""""
void SvxRawhitListv4::print() const
{
   std::cout << "SvxRawhitListv4: nRawhits = " << get_nRawhits()

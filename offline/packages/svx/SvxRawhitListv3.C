// =======================
// FILE: SvxRawhitListv3.C
// =======================
// ****************************************************************************
// Implementation of the container SvxRawhitListv3 for the SvxRawhitv3
// ****************************************************************************
...
ClassImp(SvxRawhitListv3)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxRawhitListv3::SvxRawhitListv3(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxRawhitv3", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxRawhitListv3 object created" << std::endl;
}

SvxRawhitListv3::~SvxRawhitListv3()
{
  delete m_hit_list;
  //std::cout << "SvxRawhitListv3 object destroyed" << std::endl;
}

// The "standard PHObject response" functions...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxRawhitListv3::Reset()
{
...
int SvxRawhitListv3::isValid() const
{
  return ( ( get_nRawhits() > 0 ) ? 1 : 0 ) ;
}

void SvxRawhitListv3::identify(std::ostream &os) const
{
  os << "Identify yourself: SvxRawhitListv3 object: nRawhits = "
     << get_nRawhits() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxRawhit* SvxRawhitListv3::addRawhit(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nRawhits() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxRawhit* rawhit = new(P[index]) SvxRawhitv3(this);
...
void SvxRawhitListv3::removeRawhit (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxRawhitListv3::Compress()
{
  m_hit_list->Compress();
  return (int) get_nRawhits();
}

int SvxRawhitListv3::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxRawhitListv3::restoreListPointers ()
{
  SvxRawhitv3* rawhit = NULL;
  for (int i = 0; i < get_nRawhits(); i++ ) {
    if ( ( rawhit = (SvxRawhitv3*) m_hit_list->UncheckedAt(i) ) )
      rawhit->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxRawhitListv3::sort_hitID () {
...
void SvxRawhitListv3::sort_sensorID () {
...
void SvxRawhitListv3::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
//  m_hit_list->UnSort();
}

void SvxRawhitListv3::SortSTL()
{
...
int SvxRawhitListv3::indexOfRawhit (
...
//int SvxRawhitListv3::indexOfRawhit (const SvxSensor* sensor,
//				    const int         ifrom,
...
//                                       && channel       (npar=3) 
int SvxRawhitListv3::indexOfRawhit (      SvxRawhit& rawhit,
				    const int        npar  ,
				    const int        ifrom ,
				    const int        iupto ) const {
  ...
  SvxRawhitv3* hit = (SvxRawhitv3*) &rawhit;
  hit->set_sortingSwitch(1);
  ...
  if ( indto-ifrom > 8 && sensorIDsorted ) {
	...
	if ( ( phit = get_Rawhit(i) ) ) {
	  if ( hit->Compare(phit) ) {
	    ...
	    if ( npar > 2 )
               std::cout << "SvxRawhitListv3::indexOfRawhit--WARNING: "
...
int SvxRawhitListv3::indexOfRawhit (const SvxRawhit* hit  ,
				    const int        ifrom,
				    const int        iupto) const {
  int indto = ( iupto < 0 ) ? get_nRawhits() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxRawhitListv3::indexOfRawhit(const SvxSensor* sensor, 
                                    int& idx_lb, int& idx_ub, 
                                    int ifrom, int iupto) const
{

   if (! sensorIDsorted) {
      std::cerr << "SvxRawhitListv3::indexOfRawhit(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   if (iupto < 0) iupto = get_nRawhits() - 1;
   if (ifrom < 0 || iupto >= get_nRawhits() || ifrom > iupto) {
      std::cerr << "SvxRawhitListv3::indexOfRawhit(): Invalid search range, from "
           << ifrom << " upto " << iupto << ", where the hit list has " 
           << get_nRawhits() << " entries." << std::endl;
      exit(1);
   }

   SvxRawhitv3* hit_cmp = new SvxRawhitv3();
...
// Methods
// """""""
void SvxRawhitListv3::print() const
{
   std::cout << "SvxRawhitListv3: nRawhits = " << get_nRawhits()

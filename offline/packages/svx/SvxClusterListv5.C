// ========================
// FILE: SvxClusterListv5.C
// ========================
// ****************************************************************************
// Implementation of the container SvxClusterListv5 for the SvxClusterv5
// ---
...
ClassImp(SvxClusterListv5)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxClusterListv5::SvxClusterListv5(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxClusterv5", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxClusterListv5 object created" << std::endl;
}

SvxClusterListv5::~SvxClusterListv5()
{
  delete m_hit_list;
  //std::cout << "SvxClusterListv5 object destroyed" << std::endl;
}

// The "standard PHObject response" functions...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxClusterListv5::Reset()
{
  m_hit_id_unused = 0;
  m_hit_list->Clear();
...
int SvxClusterListv5::isValid() const
{
  return ( ( get_nClusters() > 0 ) ? 1 : 0 ) ;
}

void SvxClusterListv5::identify(std::ostream &os) const
{
  os << "SvxClusterListv5 object: nClusters = "
     << get_nClusters() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxCluster* SvxClusterListv5::addCluster(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nClusters() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxCluster* cluster = new(P[index]) SvxClusterv5(this);
...
void SvxClusterListv5::removeCluster (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxClusterListv5::Compress()
{
  m_hit_list->Compress();
  return get_nClusters();
}

int SvxClusterListv5::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxClusterListv5::restoreListPointers ()
{
  SvxClusterv5* cluster = NULL;
  for (int i = 0; i < get_nClusters(); i++ ) {
    if ( ( cluster = (SvxClusterv5*) m_hit_list->UncheckedAt(i) ) )
      cluster->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxClusterListv5::sort_hitID () {
...
void SvxClusterListv5::sort_sensorID () {
...
void SvxClusterListv5::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
  m_hit_list->UnSort();
}

void SvxClusterListv5::SortSTL()
{
...
int SvxClusterListv5::indexOfCluster (
   const int hitid, int ifrom, int iupto) const
{
  ...
  if (ifrom < 0 || iupto >= get_nClusters() || ifrom > iupto) {
     std::cerr << "SvxClusterListv5::indexOfCluster(): Invalid search range, from "
...
//int SvxClusterListv5::indexOfCluster (const SvxSensor* sensor,
//				      const int         ifrom,
...
int SvxClusterListv5::indexOfCluster (const SvxCluster* hit  ,
				      const int         ifrom,
				      const int         iupto) const {
  int indto = ( iupto < 0 ) ? get_nClusters() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxClusterListv5::indexOfCluster(const SvxSensor* sensor, 
                                      int& idx_lb, int& idx_ub, 
                                      int ifrom, int iupto) const
{
   if (! sensorIDsorted) {
      std::cerr << "SvxClusterListv5::indexOfCluster(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   SvxClusterv5* hit_cmp = new SvxClusterv5();
...
// Methods
// """""""
void SvxClusterListv5::print() const
{
  std::cout << "SvxClusterListv5: nClusters = " << get_nClusters()

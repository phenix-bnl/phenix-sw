// ========================
// FILE: SvxClusterListv4.C
// ========================
// ****************************************************************************
// Implementation of the container SvxClusterListv4 for the SvxClusterv4
// ---
...
ClassImp(SvxClusterListv4)

// Constructor(s) & destructor
// """""""""""""""""""""""""""
SvxClusterListv4::SvxClusterListv4(const unsigned int length)
{
  m_hit_list   = new TClonesArray("SvxClusterv4", length);
  m_hit_id_unused = 0;
  unSort();
  listPointersSet = false;
  //std::cout << "SvxClusterListv4 object created" << std::endl;
}

SvxClusterListv4::~SvxClusterListv4()
{
  delete m_hit_list;
  //std::cout << "SvxClusterListv4 object destroyed" << std::endl;
}

// The "standard PHObject response" functions...
// """""""""""""""""""""""""""""""""""""""""""""
void SvxClusterListv4::Reset()
{
  m_hit_id_unused = 0;
  m_hit_list->Clear();
...
int SvxClusterListv4::isValid() const
{
  return ( ( get_nClusters() > 0 ) ? 1 : 0 ) ;
}

void SvxClusterListv4::identify(std::ostream &os) const
{
  os << "SvxClusterListv4 object: nClusters = "
     << get_nClusters() << std::endl;
}

// Add/remove/set/get methods...
// """""""""""""""""""""""""""""
SvxCluster* SvxClusterListv4::addCluster(const int ihit)
{
  int         index = ( ihit < 0 ) ? get_nClusters() : ihit ;
  TClonesArray   &P = *m_hit_list;
  SvxCluster* cluster = new(P[index]) SvxClusterv4(this);
...
void SvxClusterListv4::removeCluster (const unsigned int ihit)
{
...
// """""""""""""""""""""""""""""""""""""""""""
int SvxClusterListv4::Compress()
{
  m_hit_list->Compress();
  return get_nClusters();
}

int SvxClusterListv4::set_TClonesArraySize (const unsigned int nhit)
{
...
// """"""""""""""""""""""""
void SvxClusterListv4::restoreListPointers ()
{
  SvxClusterv4* cluster = NULL;
  for (int i = 0; i < get_nClusters(); i++ ) {
    if ( ( cluster = (SvxClusterv4*) m_hit_list->UncheckedAt(i) ) )
      cluster->set_ListPointer(this);
  }
  listPointersSet = true;
}

void SvxClusterListv4::sort_hitID () {
...
void SvxClusterListv4::sort_sensorID () {
...
void SvxClusterListv4::unSort() {
  hitIDsorted    = false;
  sensorIDsorted = false;
  m_hit_list->UnSort();
}

void SvxClusterListv4::SortSTL()
{
...
int SvxClusterListv4::indexOfCluster (
   const int hitid, int ifrom, int iupto) const
{
  ...
  if (ifrom < 0 || iupto >= get_nClusters() || ifrom > iupto) {
     std::cerr << "SvxClusterListv4::indexOfCluster(): Invalid search range, from "
...
//int SvxClusterListv4::indexOfCluster (const SvxSensor* sensor,
//				      const int         ifrom,
...
int SvxClusterListv4::indexOfCluster (const SvxCluster* hit  ,
				      const int         ifrom,
				      const int         iupto) const {
  int indto = ( iupto < 0 ) ? get_nClusters() : iupto ;
  for ( int i = ifrom; i < indto; i++ ) {
...
bool SvxClusterListv4::indexOfCluster(const SvxSensor* sensor, 
                                      int& idx_lb, int& idx_ub, 
                                      int ifrom, int iupto) const
{
   if (! sensorIDsorted) {
      std::cerr << "SvxClusterListv4::indexOfCluster(): The hit list hasn't been sorted by sensorID." << std::endl;
      exit(1);
   }
   idx_lb = -1;
   idx_ub = -2;
   ...
   SvxClusterv4* hit_cmp = new SvxClusterv4();
...
// Methods
// """""""
void SvxClusterListv4::print() const
{
  std::cout << "SvxClusterListv4: nClusters = " << get_nClusters()

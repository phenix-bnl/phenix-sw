...
// Calculates phi angle in Gottfried-Jackson frame
// in the range -pi to pi
// z-axis defined as momentum of one of the
...
Float_t Tools::phiGJ(Float_t MASS, Float_t P0[3], Float_t P1[3], Float_t beamP[3])
{  
  ...
  //get axes
  TVector3 zaxis = vbeam1.Unit();
  TVector3 yaxis = vbeam1.Cross(vbeam2);
  yaxis = yaxis.Unit();

...
void Tools::dca(PHMuoTracksOut* &muo, Int_t idx1, Int_t idx2, Float_t &r_dca, Float_t &x_dca, Float_t &y_dca, Float_t &z_dca)
{
  /* add by tram 27/10 */
  /* x1[2],y1[2],z1[2]; position à la station 1 des 2 traces */
  ...
  x1[1] = muo->get_xpos(1,idx2);
  y1[1] = muo->get_ypos(1,idx2);
  z1[1] = muo->get_zpos(1,idx2);

  p[0][0] = muo->get_px(1,idx1);
  p[0][1] = muo->get_py(1,idx1);
  p[0][2] = muo->get_pz(1,idx1);
  p[1][0] = muo->get_px(1,idx2);
  p[1][1] = muo->get_py(1,idx2);
  p[1][2] = muo->get_pz(1,idx2);
  ...
  for(Int_t k =0;k<2;k++)
    {
      P1[k]=pow(p[k][0],2)+pow(p[k][1],2)+pow(p[k][2],2);
      P1[k]=sqrt(P1[k]);
      xp[k]=p[k][0]/p[k][2];
      yp[k]=p[k][1]/p[k][2];
	...
	y_dca =(Y1+Y2)/2;
	x_dca =(X1+X2)/2; 
	r_dca = pow((X2-X1),2)+pow((Y2-Y1),2)+pow((Z2-Z1),2);
	r_dca = sqrt(r_dca);
	return;
}

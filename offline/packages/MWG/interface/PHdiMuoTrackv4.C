...
ClassImp(PHdiMuoTrackv4)

  PHdiMuoTrackv4::PHdiMuoTrackv4()
    :mass(0),charge(0),px(0),py(0),pz(0), vtx_ndf( 1 ), vtx_chisquare(0)
{
  for (int i=0; i<2; i++) trkIndex[i]=-1;
  for (int i=0; i<9; i++) for (int j=0; j<9; j++) vtx_cov[i][j]=0;
}

void PHdiMuoTrackv4::identify(std::ostream & out) const
{
...
int PHdiMuoTrackv4::get_trkIndex(short arrayid) const
{
  return (0<=arrayid && arrayid<2) ? trkIndex[arrayid] : -1;
}
float PHdiMuoTrackv4::get_vtx_cov(short arrayid1, short arrayid2) const
{
...
void PHdiMuoTrackv4::set_trkIndex(short arrayid, int newVal)
{
...
void PHdiMuoTrackv4::set_vtx_cov(short arrayid1, short arrayid2, float newVal)
{

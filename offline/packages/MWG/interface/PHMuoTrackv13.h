#ifndef __PHMuoTrackv13_H
#define __PHMuoTrackv13_H
...
*/
class PHMuoTrackv13 : public TObject
{
 public:
  ...
  */
  ...
  { 
    ...
    {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_delta_q id out of range.\n";
      return 0;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_delta_q_error id out of range.\n";
      return 0;
    }    
  ...
  float get_fvtx_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_fvtx_x coordinate out of range \n";
      return 0;
      }
  ...
  float get_fvtx_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_fvtx_p coordinate out of range \n";
      return 0;
      }
    return fvtx_p[coord];
  }
  ...
  float get_fvtxmutr_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_fvtxmutr_x coordinate out of range \n";
      return 0;
      }
  ...
  float get_fvtxmutr_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::get_fvtxmutr_p coordinate out of range \n";
      return 0;
      }
  ...
  size_t get_fvtx_cluster_size(size_t plane) const
  {
    if (plane>7)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv13::get_fvtx_cluster_size plane should be in [0,8] \n";
	return 0;
      }
  ...
  float get_fvtx_cluster_charge(size_t station) const
  {
    if (station>3)
      {
	std::cout<<PHWHERE<<"PHMuoTrackv13::get_fvtx_cluster_charge station should be in [0,3] \n";
	return -999.;
      }
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv13::set_delta_q id out of range.\n";
      return;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv13::set_delta_q_error id out of range.\n";
      return;
    }    
  ...
  void set_fvtx_vtx(size_t coord, float x)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::set_fvtx_x coordinate out of range \n";
      return;
      }
    fvtx_vtx[coord] = x;
  }
  ...
  void set_fvtx_p(size_t coord, float p)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv13::set_fvtx_p coordinate out of range \n";
      return;
      }
    fvtx_p[coord] = p;
  }
  ...
  void set_fvtx_cluster_size(size_t plane, size_t cluster_size)
  {
    if (plane>7)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv13::set_fvtx_cluster_size plane should be in [0,8] \n";
	return;
      }
    if (cluster_size>15)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv13::set_fvtx_cluster_size cluster size shoudl be <16 \n";
	return;
      }
  ...
  void set_fvtx_cluster_charge(size_t station, float charge)
  {
    if (station>3)
      {
	std::cout<<PHWHERE<<"PHMuoTrackv13::set_fvtx_cluster_charge station should be in [0,3] \n";
	return;
      }

// $Id: MWGInclusiveNanoCutsv2.h,v 1.1 2009/07/04 18:32:20 hpereira Exp $
 ...
 *  Status         : 
 */
#ifndef MWGInclusiveNanoCutsv2_H
#define MWGInclusiveNanoCutsv2_H
...
\paragraph{History}
 ...
 \end{enumerate}
*/
class MWGInclusiveNanoCutsv2 : public PHInclusiveNanoCuts {

public:

  MWGInclusiveNanoCutsv2();
  virtual ~MWGInclusiveNanoCutsv2() { }

  // These are necessary for all PHObjects:
  int  isValid() const {return(1);}
  void identify(std::ostream &os=std::cout) const {os << "identify yourself: MWGInclusiveNanoCutsv2 Object" << std::endl;}
  ...
  */
  ...
  void set_dofilter         ( const bool val )  { dofilter=val; }
  void set_min_dimass_filter( const float val ) { min_dimass_filter=val; }
  void set_max_dimass_filter( const float val ) { max_dimass_filter=val; }
  ...
  bool  get_dofilter ()           const { return dofilter; }
  float get_min_dimass_filter ()  const { return min_dimass_filter; }
  float get_max_dimass_filter ()  const { return max_dimass_filter; }
  ...
  bool ghostsel;
	...
	//! single mu min transverse momentum 
  float ptlowcut;

	//! single mu max transverse momentum 
  float pthighcut;
	
	//! vertex position cut
  float vertexcut;
	
	//! single mu min number of hits/track
  int   minhitcut;
	
	//! min number of tracks/event
  int   mintrackcut;
	
	//! max number of tracks/event
  int   maxtrackcut;
	
	//! set to true to make dimuons
  bool  dodimu;
	
	//! minumum dimuon mass
  float dimasscut;
  
	//! filtering flag (obsolete, unused)
  bool  dofilter;
	
	//! min mass for filter (obsolete, unused)
  float min_dimass_filter;

	//! max mass for filter (obsolete, unused)
  float max_dimass_filter;
  
  ClassDef(MWGInclusiveNanoCutsv2,1)

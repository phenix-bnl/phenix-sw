#ifndef __PHMuoTrackv16_H
#define __PHMuoTrackv16_H
...
*/
class PHMuoTrackv16 : public TObject
{
 public:
  ...
  */
  ...
  { 
    ...
    {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_delta_q id out of range.\n";
      return 0;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_delta_q_error id out of range.\n";
      return 0;
    }    
  ...
  float get_fvtx_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtx_x coordinate out of range \n";
      return 0;
      }
  ...
  float get_fvtx_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtx_p coordinate out of range \n";
      return 0;
      }
    return fvtx_p[coord];
  }
  ...
  float get_fvtxmutr_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtxmutr_x coordinate out of range \n";
      return 0;
      }
  ...
  float get_fvtxmutr_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtxmutr_p coordinate out of range \n";
      return 0;
      }
  ...
  size_t get_fvtx_cluster_size(size_t plane) const
  {
    if (plane>7)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtx_cluster_size plane should be in [0,8] \n";
	return 0;
      }
  ...
  float get_fvtx_cluster_charge(size_t station) const
  {
    if (station>3)
      {
	std::cout<<PHWHERE<<"PHMuoTrackv16::get_fvtx_cluster_charge station should be in [0,3] \n";
	return -999.;
      }
  ...
  //! returns VTX+FVTX hit pattern
  //! NA NA NA NA VTX4 VTX3 VTX2 VTX1 FVTX4_1 FVTX_4_0 FVTX3_1 FVTX3_0 FVTX2_1 FVTX2_0 FVTX1_1 FVTX1_0
  unsigned short get_fvtx_hits() const
  {
    return fvtx_hits;
  }

  /*
  //! return 3-vector swapped FVTX vertex position
  float get_sfvtx_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtx_x coordinate out of range \n";
    ...
    return sfvtx_vtx[coord];
  }

  //! return 3-vector swapped FVTX momentum
  float get_sfvtx_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtx_p coordinate out of range \n";
    ...
    return sfvtx_p[coord];
  }

  //! return 3-vector swapped FVTX+MuTr refit vertex position
  float get_sfvtxmutr_vtx(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtxmutr_x coordinate out of range \n";
    ...
    return sfvtxmutr_vtx[coord];
  }

  //! return 3-vector swapped FVTX+MuTr refit momentum
  float get_sfvtxmutr_p(size_t coord) const
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::get_fvtxmutr_p coordinate out of range \n";
    ...
    return sfvtxmutr_p[coord];
  }

  //! return chi2/NDF for swapped FVTX+MuTr refit momentum
  float get_sfvtxmutr_chi2() const
  {
    return sfvtxmutr_chi2;;
  }

  //! return phi residual btw. MuTr and swapped FVTX track in sigmas considering fvtx track window
  float get_sfvtx_dphi() const
  {
    return sfvtx_dphi;
  }

  //! return theta residual btw. MuTr and swapped FVTX track
  float get_sfvtx_dtheta() const
  {
    return sfvtx_dtheta;
  }
  
  //! return radius residual btw. MuTr and swapped FVTX track projection at Z=+/-40cm
  float get_sfvtx_dr() const
  {
    return sfvtx_dr;
  }

  //! return chi^2 for MuTr+ swapped FVTX track
  float get_sfvtx_chi2() const
  {
    return sfvtx_chi2;
  }

  //! return covariant matrix of swapped FVTX track
  float get_sfvtx_cov(size_t i, size_t j) const
  {
    return sfvtx_cov[i][j];
  }

  //! return covariant matrix of swapped FVTX+MuTr refit track
  float get_sfvtxmutr_cov(size_t i, size_t j) const
  {
    return sfvtxmutr_cov[i][j];
  }
  */
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_delta_q id out of range.\n";
      return;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_delta_q_error id out of range.\n";
      return;
    }    
  ...
  void set_fvtx_vtx(size_t coord, float x)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_x coordinate out of range \n";
      return;
      }
    fvtx_vtx[coord] = x;
  }
  ...
  void set_fvtx_p(size_t coord, float p)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_p coordinate out of range \n";
      return;
      }
    fvtx_p[coord] = p;
  }
  ...
  void set_fvtx_cluster_size(size_t plane, size_t cluster_size)
  {
    if (plane>7)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_cluster_size plane should be in [0,8] \n";
	return;
      }
    if (cluster_size>15)
      {
	std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_cluster_size cluster size shoudl be <16 \n";
	return;
      }
  ...
  void set_fvtx_cluster_charge(size_t station, float charge)
  {
    if (station>3)
      {
	std::cout<<PHWHERE<<"PHMuoTrackv16::set_fvtx_cluster_charge station should be in [0,3] \n";
	return;
      }
  ...
  {
    fvtx_strip[station] = (strip&0x7ff)
      | ((column&0x1) << 12)
      | ((sector&0x3f) << (1+12))
      | ((station&0x03) << (6+1+12))
      | ((cage&0x01) << (2+6+1+12))
  ...
  //! set the VTX+FVTX hit pattern
  //! NA NA NA NA VTX4 VTX3 VTX2 VTX1 FVTX4_1 FVTX_4_0 FVTX3_1 FVTX3_0 FVTX2_1 FVTX2_0 FVTX1_1 FVTX1_0
  void set_fvtx_hits( unsigned short a)
  {
    fvtx_hits = a;
  }

  /*
    //! set 3-vector swapped FVTX vertex position
  void set_sfvtx_vtx(size_t coord, float x)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_x coordinate out of range \n";
  ...
  void set_sfvtx_p(size_t coord, float p)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_p coordinate out of range \n";
  ...
  //! set 3-vector swapped FVTX+MuTr refit vertex position
  void set_sfvtxmutr_vtx(size_t coord, float x)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_x coordinate out of range \n";
    ...
    sfvtxmutr_vtx[coord] = x;
  }

  //! set 3-vector swapped FVTX+MuTr refit momentum
  void set_sfvtxmutr_p(size_t coord, float p)
  {
    if (coord >= 3)
      {
      std::cout<<PHWHERE<<" PHMuoTrackv16::set_fvtx_p coordinate out of range \n";
  ...
  //! set chi2/NDF for swapped FVTX+MuTr refit momentum
  void set_sfvtxmutr_chi2(float a)
  {
    sfvtxmutr_chi2 = a;
  }

  //! set phi residual btw. MuTr and swapped FVTX track
  void set_sfvtx_dphi(float dphi)
  {
    sfvtx_dphi = dphi;
  }

  //! set theta residual btw. MuTr and swapped FVTX track
  void set_sfvtx_dtheta(float dtheta)
  {
    sfvtx_dtheta = dtheta;
  }
  
  //! set radius residual btw. MuTr and swapped FVTX track projected at Z=+/-40cm
  void set_sfvtx_dr(float dr)
  {
    sfvtx_dr = dr;
  }

  //! set chi^2 for MuTr+ swapped FVTX track
  void set_sfvtx_chi2(float chi2)
  {
    sfvtx_chi2 = chi2;
  }

  //! set covariant matrix of swapped FVTX track
  void set_sfvtx_cov(size_t i, size_t j, float cov)
  {
    sfvtx_cov[i][j] = cov;
  }

  //! set covariant matrix of swapped FVTX+MuTr refit track
  void set_sfvtxmutr_cov(size_t i, size_t j, float cov)
  {
    sfvtxmutr_cov[i][j] = cov;
  }
  */
  //@}
  ...
  */
  ...
  //! hit pattern 
  //! NA NA NA NA VTX4 VTX3 VTX2 VTX1 FVTX4_1 FVTX_4_0 FVTX3_1 FVTX3_0 FVTX2_1 FVTX2_0 FVTX1_1 FVTX1_0
  unsigned short fvtx_hits;

  /*
    //! 3-vector swapped FVTX momentum assuming total momentum from MuTr
  float sfvtx_p[3];

  //! 3-vector swapped FVTX vertex position
  float sfvtx_vtx[3];

  //! covariant matrix of swapped FVTX track
  float sfvtx_cov[_covdim][_covdim];

  //! 3-vector swapped FVTX+MuTr refit momentum assuming total momentum from MuTr
  float sfvtxmutr_p[3];

  //! 3-vector swapped FVTX+MuTr refit vertex position
  float sfvtxmutr_vtx[3];

  //! chi2/NDF for refit vertex position of swapped FVTX track
  float sfvtxmutr_chi2;

  //! covariant matrix of swapped FVTX+MuTr refit track
  float sfvtxmutr_cov[_covdim][_covdim];

  //! delta_phi residual btw. MuTr and swapped FVTX track
  float sfvtx_dphi;

  //! delta_theta residual btw. MuTr and swapped FVTX track
  float sfvtx_dtheta;

  //! delta_r residual  btw. MuTr and swapped FVTX track at z=+/-40cm 
  float sfvtx_dr;

  //! track chi^2 for MuTr+ swapped FVTx track
  float sfvtx_chi2;
  */

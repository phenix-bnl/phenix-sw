#ifndef __PHdiMuoTracksv11_H
#define __PHdiMuoTracksv11_H
...
*/
class PHdiMuoTracksv11 : public PHMuoTracksOut
{
  ...
  {
      os << "identify yourself:  PHdiMuoTracksv11 object" << std::endl;
      return;
  }
  ...
  void AddPHDimuon(const unsigned int idimu){
      new ((*diMuoTracks)[idimu]) PHdiMuoTrackv5();
  }
  ...
  //! dimuon unique id
  ULong_t get_dimuon_uid( const unsigned int idimu ) const
  {
  ...
  //! dimuon uid
  void set_dimuon_uid( const unsigned int idimu, ULong_t value )
  {
    PHdiMuoTrackv5* dimu = (PHdiMuoTrackv5*) diMuoTracks->UncheckedAt(idimu);
    if(dimu) dimu->set_uid(value); 
  }
  ...
  { new ((*MuoTracks)[itrk]) PHMuoTrackv11(); }

#ifndef __PHMuoTrackv8_H
#define __PHMuoTrackv8_H
#include "PHObject.h"
#include "phool.h"
...
*/
class PHMuoTrackv8 : public TObject
{
 public:
  ...
  enum
  {
    ...
    */
    ...
    /* 
      the ifdef used here is historical. At some point at "backward compatibility broken" version of
      PHMuoTracksv8 was committed, that would prevent from being able to read Run4, 5, 6 nanoDSTs. 
      The backward compatibility was restored (and the class switched to v9), but that prevented the 
      reading of versions generated with the broken class. This flag allows one to compile a local 
      "compatibility broken" version of MWG, and thus read back such DSTs
    */
    #ifdef BROKEN
  ...
  */
  ...
  { 
    ...
    {
      std::cout<<PHWHERE<<" PHMuoTrackv8::get_delta_q id out of range.\n";
      return 0;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv8::get_delta_q_error id out of range.\n";
      return 0;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv8::set_delta_q id out of range.\n";
      return;
    }    
  ...
  { 
    if( coord_id >= _max_gap_coords ) 
    {
      std::cout<<PHWHERE<<" PHMuoTrackv8::set_delta_q_error id out of range.\n";
      return;
    }    
  ...
  float muID_proj_hit_dist[5][2][_max_mui_hits]; // 5 gaps; 2 orientations (h/v); _max_mui_hits closest hits
  short muID_proj_hit_size[5][2][_max_mui_hits];

#ifndef __PHMuoTracksv12_H
#define __PHMuoTracksv12_H
#include <iostream>
#include "PHObject.h"
...
*/
class PHMuoTracksv12 : public PHMuoTracksOut
{
  ...
  //! constructor
  PHMuoTracksv12(TClonesArray* MuT);
  
  //! destructor
  virtual ~PHMuoTracksv12();
  ...
  {
      os << "identify yourself:  PHMuoTracksv12 object" << std::endl;
      return;
  }
  ...
  {
      new ((*MuoTracks)[itrk]) PHMuoTrackv12();
  }
  ...
  virtual PHMuoTracksv12* clone() const;
  ...
  unsigned short get_fvtx_strip(const unsigned int itrk, const size_t station) const;
  ...
  long long get_nfvtx_trks_word(const unsigned int itrk) const;

  //! return number of FVTX tracks close to this track in $\Delta \eta$ and $\Delta \phi$
  size_t get_nfvtx_tracks(const unsigned int itrk, const float deta, const float dphi) const;
  ...
  void set_fvtx_strip(const unsigned int itrk, const size_t station, const unsigned short index);
  ...
  //! set bit pattern with number of FVTX tracks withing a region dEta=N*0.01, const dphi=N*pi/48, 0<N<=21
  //! each dEta, dphi region corresponds to a 3-bit set
  void set_nfvtx_trks_word(const unsigned int itrk, const long long word);

  //!set number of FVTX tracks withing a region dEta=N*0.01, const dphi=N*pi/48, 0<N<=21
  void set_nftvx_tracks(const unsigned int itrk, const float deta, const float dphi, const size_t ntracks);

  //!set number of FVTX tracks withing a region dEta=N*0.01, dphi=N*pi/48, 0<N<=21
  void fill_nfvtx_tracks(const unsigned int itrk, const float deta, const float dphi);

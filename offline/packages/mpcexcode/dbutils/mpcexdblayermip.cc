...
static const char* classname = "PdbMpcExLayerMIPBank";
static const char* calibname = "mpcexlayermipcorr";
...
bool openConnection(const char* method){
  ...
  if(bankManager == NULL){
    std::cout<<PHWHERE<<" mpcexdblayermip::"<<method<<" - cannot obtain PdbBankManager"<<std::endl;
    return false;
  }

  app = PdbApplication::instance();
  if(app == NULL){
    std::cout<<PHWHERE<<" mpcexdblayermip::"<<method<<" - cannot obtain PdbApplication"<<std::endl;
    return false;
  }
 ...
 * <arm> <layer> <mpv> <sigma>
 */
void commit_layermip_data(const std::string& filename, int begin_run_number, int end_run_number, const std::string& username, const std::string& description){

  if(openConnection("commit_layermip_data")){
    if(app->startUpdate()){
      ...
      static const unsigned int nlayers = 16;
      bank->setLength(nlayers);

      unsigned short arm, layer;
      float mpv, sigma;
      unsigned int index = 0;
      while(file>>arm>>layer>>mpv>>sigma){
	//grab the layer entry in the table and fill it with the input from the text file
	PdbMpcExLayerMIP *data = (PdbMpcExLayerMIP*) &(bank->getEntry(index));
	data->set_data(arm,layer,mpv,sigma);
	...
	app->commit(bank);
      ...
      std::cout<<PHWHERE<<" mpcexdblayermip::commit_layermip_data - cannot update application"<<std::endl;
    }
  }

}

void dump_layermip_data(int run_number){
  if(openConnection("dump_layermip_data")){
	...
	for(int i=0; i<nentries; i++){
	  PdbMpcExLayerMIP data = (PdbMpcExLayerMIP&)bank->getEntry(i);
	  data.print();
	}
	...
	std::cout<<PHWHERE<<" mpcexdblayermip::dump_layermip_data -- no bank is valid for run "<<run_number<<std::endl;
      }
...
void printUsage(){
  ...
  std::cout<<"\tmpcexdclayermip [commit|test|dump|help]"<<std::endl;
...
int main(int argc, char** argv){
  ...
  if(what=="commit" || what=="test"){
    ...
    test = (what == "test");
    commit_layermip_data(filename,begin_runnumber,end_runnumber,username,description);
  }

  //dump the data
  if(what=="dump"){
    int run_number = atoi(argv[2]);
    dump_layermip_data(run_number);
  }

...
static const char* classname = "PdbMpcExSmearMCMinipadsBank";
static const char* calibname = "mpcexsmearminipad";
...
bool openConnection(const char* method){
  ...
  if(bankManager == NULL){
    std::cout<<PHWHERE<<" mpcexdbsmearminipad::"<<method<<" - cannot obtain PdbBankManager"<<std::endl;
    return false;
  }

  app = PdbApplication::instance();
  if(app == NULL){
    std::cout<<PHWHERE<<" mpcexdbsmearminipad::"<<method<<" - cannot obtain PdbApplication"<<std::endl;
    return false;
  }
 ...
 * <key> <scale> <smear>
 */
void commit_minipadsmear_data(const std::string& filename, int nminipads, int begin_run_number, int end_run_number, const std::string& username, const std::string& description){

  if(openConnection("commit_minipadsmear_data")){
    if(app->startUpdate()){
      ...
      float scale, smear;
      int index = 0;
      while(file>>key>>scale>>smear){
	std::cout<<key<<" "<<scale<<" "<<smear<<std::endl;

	//grab the minipad entry in the table and fill it with the input from the text file
	PdbMpcExSmearMCMinipads *data = (PdbMpcExSmearMCMinipads*) &(bank->getEntry(index));
	data->set_data(key,scale,smear);
	...
	app->commit(bank);
      ...
      std::cout<<PHWHERE<<" mpcexdbsmearminipad::commit_minipadsmear_data - cannot update application"<<std::endl;
    }
  }

}

void dump_minipadsmear_data(int run_number){
  if(openConnection("dump_minipadsmear_data")){
	...
	for(int i=0; i<nentries; i++){
	  PdbMpcExSmearMCMinipads data = (PdbMpcExSmearMCMinipads&)bank->getEntry(i);
	  data.print();
	}
	...
	std::cout<<PHWHERE<<" mpcexdbsmearminipad::dump_minipadsmear_data -- no bank is valid for run "<<run_number<<std::endl;
      }
...
void printUsage(){
  ...
  std::cout<<"\tmpcexdbsmearminipad [commit|test|dump|help]"<<std::endl;
...
int main(int argc, char** argv){
  ...
  if(what=="commit" || what=="test"){
    ...
    test = (what == "test");
    commit_minipadsmear_data(filename,nminipads,begin_runnumber,end_runnumber,username,description);
  }

  //dump the data
  if(what=="dump"){
    int run_number = atoi(argv[2]);
    dump_minipadsmear_data(run_number);
  }

...
MWGRpcMuoTrackReco::MWGRpcMuoTrackReco() : 
  //  _timer( PHTimeServer::get()->insert_new("MWGRpcMuoTrackReco") )
  _timer( new PHTimer("MWGRpcMuoTrackReco") )
{
        RPCOO::TRACE("initializing module MWGRpcMuoTrackReco",RPCOO::ALOT);
...
MWGRpcMuoTrackReco::~MWGRpcMuoTrackReco()
{
  if(_muocuts) { delete _muocuts; }
  if(_timer) { delete _timer; }
}

//_____________________________________________
PHBoolean MWGRpcMuoTrackReco::event(PHCompositeNode* top_node)
{
...
//_____________________________________________
void MWGRpcMuoTrackReco::attach_track()
{
  if(!_muoo) {
    std::cout << PHWHERE << " Error - cannot find PHMuoTracksOut" << std::endl; 
    return; }

  //  std::cout << "There are " << _muoo->get_npart() << " tracks to match" << std::endl;
  ...
  for(uint itrk=0 ; itrk<_muoo->get_npart() ; itrk++) {
    ...
    //    std::cout << "Projected RPC3 = " << fProjHitXVtx3 << " " << fProjHitYVtx3 << std::endl;
    //std::cout << "Projected RPC1 = " << fProjHitXVtx1 << " " << fProjHitYVtx1 << std::endl;
    ...
    //std::cout << "Projected RPC3 = " << fProjHitXSt13 << " " << fProjHitYSt13 << std::endl;
    //std::cout << "Projected RPC1 = " << fProjHitXSt11 << " " << fProjHitYSt11 << std::endl;
    ...
    //std::cout << "Projected RPC3 = " << fProjHitXSt33 << " " << fProjHitYSt33 << std::endl;
    //std::cout << "Projected RPC1 = " << fProjHitXSt31 << " " << fProjHitYSt31 << std::endl;
	...
	fProjHitYMUID1 = pyz_muid*(fRPC1z-z_muid) + y_muid;
      ...
      {
	//std::cout << PHWHERE << " Could not find a \"best\" road" << std::endl; 
      }
    ...
    //std::cout << "Projected RPC3 = " << fProjHitXMUID3 << " " << fProjHitYMUID3 << std::endl;
    //std::cout << "Projected RPC1 = " << fProjHitXMUID1 << " " << fProjHitYMUID1 << std::endl;
...
// 
    ...
    //std::cout << "RPC1vtx " << sqrt(pow(px_vtx,2)+pow(py_vtx,2)) << " " << fLowDCAVtx[0] << " " << fLowTimeVtx[0] << std::endl;
    //std::cout << "RPC3vtx " << fLowDCAVtx[1] << " " << fLowTimeVtx[1] << std::endl;
...
void MWGRpcMuoTrackReco::set_interface_ptrs(PHCompositeNode* top_node)
{   
  // IOC pointers

...
SimpleHistogrammer::SimpleHistogrammer()
{
  ...
  momcut=0.5;
  changematch =0;
  /*  m->set_xxx_z_match(0,0,0,0.51,0.6);   //PC2_Z
  m->set_xxx_z_match(1,0,0,0.81,0.83);  //PC3E_Z
  m->set_xxx_z_match(2,0,0,0.872,0.81); //PC3W_Z
  m->set_xxx_z_match(3,0,0,0.91,0.95);  //TOF_Z
  m->set_xxx_z_match(5,0,0,1,2.5);      //PBGL_Z
  m->set_xxx_z_match(6,0,0,1.4,2.5);    //PBSCE_Z
  m->set_xxx_z_match(7,0,0,1.8,2.4);    //PBSCW_Z

  m->set_xxx_phi_match(0,0,0,0,0,0.00152,0.00151); //PC2_P
  m->set_xxx_phi_match(1,0,0,0,0,0.002,0.0018);    //PC3E_P
  m->set_xxx_phi_match(2,0,0,0,0,0.0021,0.0018);   //PC3W_P
  m->set_xxx_phi_match(3,0,0,0,0,0.00219,0.00369); //TOF_P
  m->set_xxx_phi_match(5,0.00417,0.00405,-0.00388,-0.00422,0.0024,0.0036); //PBGL_P
  m->set_xxx_phi_match(6,0.00181,0.00218,-0.00212,0.00132,0.0026,0.0046); //PBSCE_P
  m->set_xxx_phi_match(7,0.00227,-0.00044,-0.00199,-0.00157,0.0027,0.0047); //PBSCW_P
  */
  flagInit = 0;
...
int SimpleHistogrammer::initializeFile(char* name=0)
{
  if (!flagInit) {
    if(name){
      filename = name;
...
void SimpleHistogrammer::Fill(){
  ...
  for(int icgl=0;icgl<ntrk;icgl++){
    ...
    /*    if(icrk>-1) {
      Central_n0[icgl]    = d_crkring->get_n0(icrk);
      Central_npe0[icgl]  = d_crkring->get_npe0 (icrk);
      Central_n1[icgl]    = d_crkring->get_n1 (icrk);
      Central_npe1[icgl]  = d_crkring->get_npe1 (icrk);
      Central_tcrk[icgl]  = d_crkring->get_tcrk (icrk);
      Central_disp[icgl]  = d_crkring->get_disp (icrk);
    }else{
    */
  ...
  float sibpx[10],sibpy[10],sibpz[10],sibphi[10],sibthe[10],sibthe0[10],sibphi0[10],sibalp[10],sibpt[10];
  int sibid[10];
  //cout << "-------------------------" << MCNTRACK << endl;
  int found = 0;
  if(d_primary){
    for(i=0;i<d_primary->RowCount();i++){
  ...
  for (j = 0; j < ntrk; j++) { // central track
    found = 0;
	...
	if((Central_phi[j]<1.57&&dowest)||(Central_phi[j]>1.57&&doeast)){  
	  if(fabs(mom)>momcut){
	    ...
	    //
	    good =1;
	    /*if(fabs(Central_momentum[j] - McRecoTrk_MOMENTUM[k])>0.000001){
	      cout<<"bad "<<Central_momentum[j]<<" "<<McRecoTrk_MOMENTUM[k]<<" "<<McRecoTrk_RECOID[k]<<endl;
	      bad++;
	      good = 0;
	      }*/
	    array[m++] = good;

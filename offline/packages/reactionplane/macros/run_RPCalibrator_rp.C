...
void run_RPCalibrator_rp(int nevt=0, char* infile, char* output = "RPCalib.root"
)
{
  ...
  rc->set_CharFlag("RPCALIB_CALIBFILENAME", "/phenix/hhj/hachiya/13.09/source/reactionplane/macros/plot/rpcalib_flat_347602.dat");

  rc->set_IntFlag("RP_SKIP_RECENTERING", 0);
  rc->set_IntFlag("RP_SKIP_FLATTENING", 0);
  ...
  int recalflag  = 2; // 0:re-centering 1:flattening 2:use calibration parameter 
  int masterflag = 1; // 1;
  int vnflag     = 0;

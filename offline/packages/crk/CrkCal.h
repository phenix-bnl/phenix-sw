...
public:
  ...
  void get_description(char *str){strcpy(str,d_description.c_str());} 
  void set_description(char *str){d_description = str;} 
  void set_starttime(PHTimeStamp time){d_start_time = time;}
  void set_endtime(PHTimeStamp time){d_end_time = time;}
...
//
// Because of a limitation of g++, one can not separate compilation of a
// template class. All member function should be defined in the header file.
//

template <class T>
bool CrkCal<T>::skip_comment(std::ifstream& ifile) {
  char c = ' ';
...
template <class T>
void CrkCal<T>::read_time(std::istream& is, PHTimeStamp &time) {
  char c;
...
void CrkCal<T>::store_DB(void) {
  ...
  if(app->startUpdate()) {
    ...
    for(unsigned int i=0;i<d_calib.size();i++) {

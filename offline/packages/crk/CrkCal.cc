...
template<> CrkCal<PdbRichADC>::CrkCal(const char *parname) {
  d_status = 1;
  d_classname = "PdbRichADCBank";
  d_calibname = "calib.rich.adc";

  if( string(parname) == "adcphe") {
    d_name      = "ADCPHE";
    d_bankID.setInternalValue(1);
  } else if( string(parname) == "adcped") {
    d_name      = "ADCPED";
...
template<> CrkCal<PdbRichPar>::CrkCal(const char *parname) {
  d_status = 1;

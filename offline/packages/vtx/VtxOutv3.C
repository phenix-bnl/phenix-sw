...
void VtxOutv3::identify(ostream& out) const
{
  PHPoint vtx, vtxerr;

  out << "identify yourself: I am a VtxOutv3 object, list of vertices:" << endl;
...
void VtxOutv3::Reset()  // reset data members (called by phool node reset)
{
  short i;
  VtxList = 0;
...
PHPoint VtxOutv3::get_Vertex() const
{
  int ivtx;
  ivtx = ChooseVtx();
  PHPoint vertex;
...
PHPoint VtxOutv3::get_VertexError() const
{
  int ivtx;
  PHPoint vertexerror;
  ivtx = ChooseVtx();
...
float VtxOutv3::get_ZVertex() const
{
  switch (ChooseVtx())
    {
    case BBCVTX:
...
float VtxOutv3::get_ZVertexError() const
{
  switch (ChooseVtx())
    {
    case BBCVTX:
...
int VtxOutv3::set_BbcVtx(const float vtx, const float vtxerr)
{
  BbcVtx = vtx;
  BbcVtxErr = vtxerr;
  VtxList |= VTX::BBCBIT;
...
int VtxOutv3::set_NtcVtx(const float vtx, const float vtxerr)
{
  NtcVtx = vtx;
  NtcVtxErr = vtxerr;
  VtxList |= VTX::NTCBIT;
...
int VtxOutv3::set_ZdcVtx(const float vtx, const float vtxerr)
{
  ZdcVtx = vtx;
  ZdcVtxErr = vtxerr;
  VtxList |= VTX::ZDCBIT;
...
bool VtxOutv3::isBbcVtx() const
{
  return VtxList&VTX::BBCBIT;
}

bool VtxOutv3::isZdcVtx() const
{
  return VtxList&VTX::ZDCBIT;
}

bool VtxOutv3::isNtcVtx() const
{
...
int VtxOutv3::use_Vertex(const char *subsystem)
{
...
int VtxOutv3::use_Vertex(const PHPoint *vtx, const PHPoint *vtxerr)
{
...
int VtxOutv3::set_DefaultVtx(const PHPoint *vtx, const PHPoint *vtxerr)
{
...
const char *VtxOutv3::which_Vtx() const
{
  switch (ChooseVtx())
    {
      case MVDVTX:

    ...
    int run = 421989)
{
    ...
    std::cout<<"This is /offline/packages/svxdstqa/VTXPEvntMisalign/macros/look_eventoffset.C"<<std::endl;
  ...
  if(print_images || save_histos)
    gSystem->Exec(Form("mkdir -p %s/%d",imageoutdir,run));
  ...
  for(int iladder = 0; iladder < 30; ++iladder)
  {
    for(int ichip = 0; ichip < 16; ++ichip)
    {
      ...
      {
        evntmis->save_histos(
            Form("%s/%d/%d_B%dL%dS%dC%d.root",imageoutdir,run,run,lay,lad,sen,chi),false);
        // evntmis->save_histos(
      ...
      {
        evntmis->print_image(
            Form("%s/%d/%d_B%dL%dS%dC%d.png",imageoutdir,run,run,lay,lad,sen,chi),false);
        //evntmis->print_image(

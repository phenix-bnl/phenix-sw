...
ClassImp(emcRawDataProcessorv3)
...
emcRawDataProcessorv3::emcRawDataProcessorv3(emcCalibrationDataHelper* ch)
  : emcRawDataProcessor()
{
  fCH = ch;
}
...
//_____________________________________________________________________________
emcRawDataProcessorv3::~emcRawDataProcessorv3()
{
}

//_____________________________________________________________________________
bool
emcRawDataProcessorv3::chooseLowGainPbSc(emcTowerContent* tower, float& scale)
{
  int lgpp = tower->LGPP();

  static emcDataError dataerror;

  static int badhighgain = 
    dataerror.HG_PRE_OUT() +
    dataerror.HG_POST_OUT();

  bool rv = ( ( lgpp > 192 ) ||
	      ( tower->DataError() & badhighgain ) );
  // if both high gain samples are bad,
...
//_____________________________________________________________________________
bool 
emcRawDataProcessorv3::chooseLowGainPbGl(emcTowerContent* t, float& scale)
{
  bool badHighGain = ( t->HGPP() < 0 && t->LGPP() > 50 );
  bool goodLowGain = ( t->LGPP() >170 );

  bool rv = ( t->DataError() & 0x003f || 
	      badHighGain || 
	      t->HGPost() < 1024 || 
	      goodLowGain );
  ...
  if ( rv ) 
    {
      ...
      assert(calfem!=0);
      scale = calfem->getValueFast(t->Channel(),3);
     
      if ( scale < 10. || scale > 21.) 
	{
	  scale = 15.33;
	}
    }
...
//_____________________________________________________________________________
int
emcRawDataProcessorv3::isValid() const
{
  return 1;
}
...
//_____________________________________________________________________________
void
emcRawDataProcessorv3::identify(std::ostream& out) const
{
  out << "emcRawDataProcessorv3::identify" << std::endl;
}

//_____________________________________________________________________________
void
emcRawDataProcessorv3::Reset()
{
}

//_____________________________________________________________________________
bool
emcRawDataProcessorv3::toADCandTDC(emcTowerContainer* pbsc, 
				   emcTowerContainer* pbgl,
				   const emcBadModules& bad)
{
  if (pbsc)
    {
      FPTR ptr = &emcRawDataProcessorv3::chooseLowGainPbSc;
  ...
  if (pbgl)
    {
      FPTR ptr = &emcRawDataProcessorv3::chooseLowGainPbGl;
...
//_____________________________________________________________________________
void
emcRawDataProcessorv3::toADCandTDC(emcTowerContent* tower, 
				   FPTR function_ptr,
				   const emcBadModules& bad)
{

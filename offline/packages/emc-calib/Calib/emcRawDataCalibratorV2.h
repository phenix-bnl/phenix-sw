#ifndef __EMCRAWDATACALIBRATORV2_H__
#define __EMCRAWDATACALIBRATORV2_H__
...
/** (OLD) Implementation of emcCalibrator for Run2/3.
...
class emcRawDataCalibratorV2 : public emcCalibrator
{
 public:

  /// ctor
  emcRawDataCalibratorV2();
  emcRawDataCalibratorV2(emcCalibrationDataHelper * fch);
  /// dtor
  ...
  */
  ...
  virtual bool hasHelper() {return fCH!=NULL;}
  virtual void storeHelper(emcCalibrationDataHelper * fch){fCH = fch;}
  ...
  // private:
  //  Implementation from emcDCProcessorv3
  bool calibrateEnergyPbGl(float & adc, int index, int twrId, int ifem, time_t ti);
  bool calibrateEnergyPbSc(float & adc, int index, int twrId, int ifem, time_t ti);
  bool calibrateTimePbGl(float adc, float & tdc, int index, int ifem, time_t ti);
  bool calibrateTimePbSc(float adc, float & tdc, int index, int ifem, time_t ti);

...
namespace
{
  bool namer(const emcLCTofFEM& hlr,
	     int to_or_from,
	     std::string& filename)
  {
    int SN, SM144N;
  ...
  bool writer(const emcLCTofFEM& tofFEM, int)
  {
    ...
    for ( size_t i = 0; i < tofFEM.size(); ++i )
      {
	out << i << " " << tofFEM.getValue(i,0) << " "
	    << tofFEM.getValue(i,1) << std::endl;
      }
    ...
    if ( dm->GetVerboseLevel() )
      {
	std::cout << "emcOMLCTofFEM written to " 
		  << filename << std::endl;
      }
  ...
  emcOMAsciiT<emcLCTofFEM> 
  gemcOMLCTofFEM("emcOMLCTofFEM",
		 "Read/Write emcLCTofFEM objects from/to files",
		 reader,
		 writer);
}

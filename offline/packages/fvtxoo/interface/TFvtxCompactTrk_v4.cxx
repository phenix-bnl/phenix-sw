...
TFvtxCompactTrk_v4::TFvtxCompactTrk_v4() :
  arm(false),
  index(0),
...
//_______________________________________
TFvtxCompactTrk_v4::TFvtxCompactTrk_v4(const Key& key,
...
//_______________________________________
TFvtxCompactTrk_v4::TFvtxCompactTrk_v4(const TFvtxCompactTrk* base_ptr) :
  ...
  svxhit_pattern(base_ptr->get_svxhit_pattern())
{
  for (size_t i=0; i<ncovel; i++)
...
//_______________________________________
TFvtxCompactTrk_v4::TFvtxCompactTrk_v4(const TFvtxTrk* base_ptr) :
  TFvtxCompactTrk()
{
  arm = base_ptr->get_arm();
  index = base_ptr->get_index();
...
//_______________________________________
void TFvtxCompactTrk_v4::print(std::ostream& os) const
{
...
unsigned short TFvtxCompactTrk_v4::FloatToShort( const float rval ) const
{
...
float TFvtxCompactTrk_v4::ShortToFloat( const unsigned short ival ) const
{
 ...
 PHPoint TFvtxCompactTrk_v4::get_fvtx_vtx() const
 {
...
float TFvtxCompactTrk_v4::get_fvtx_phi() const
{
  return ShortToFloat(fvtx_phi);
}

float TFvtxCompactTrk_v4::get_fvtx_theta() const
{
  return ShortToFloat(fvtx_theta);
}

float TFvtxCompactTrk_v4::get_fvtx_eta() const
{
  return -log(tan(get_fvtx_theta()/2));
}

float TFvtxCompactTrk_v4::get_chi2_ndf() const
{
  return ShortToFloat(chi2_ndf);
}

float TFvtxCompactTrk_v4::get_cov(short i) const
{
...
short TFvtxCompactTrk_v4::get_cov_index(short i, short j) const
{
...
float TFvtxCompactTrk_v4::get_cov(short i, short j) const
{
  short k = get_cov_index(i,j);
  return get_cov(k);
}

void TFvtxCompactTrk_v4::set_track_vtx( PHPoint pnt )
{
...
void TFvtxCompactTrk_v4::set_fvtx_phi( float phi )
{
  fvtx_phi = FloatToShort(phi);
}

void TFvtxCompactTrk_v4::set_fvtx_theta( float theta )
{
  fvtx_theta = FloatToShort(theta);
}

void TFvtxCompactTrk_v4::set_fvtx_eta( float eta )
{
  fvtx_theta = FloatToShort(2*atan(exp(-eta)));
}

void TFvtxCompactTrk_v4::set_chi2_ndf( float _chi2 )
{
  chi2_ndf = FloatToShort(_chi2);
}

void TFvtxCompactTrk_v4::set_cov(short i, short j, float a)
{
  short k = get_cov_index(i,j);

...
TFvtxCompactTrk_v2::TFvtxCompactTrk_v2() :
  arm(false),
  index(0),
...
//_______________________________________
TFvtxCompactTrk_v2::TFvtxCompactTrk_v2(const Key& key,
...
//_______________________________________
TFvtxCompactTrk_v2::TFvtxCompactTrk_v2(const TFvtxCompactTrk* base_ptr) :
...
//_______________________________________
TFvtxCompactTrk_v2::TFvtxCompactTrk_v2(const TFvtxTrk* base_ptr) :
  TFvtxCompactTrk()
{
  arm = base_ptr->get_arm();
  index = base_ptr->get_index();
...
//_______________________________________
void TFvtxCompactTrk_v2::print(std::ostream& os) const
{
...
unsigned short TFvtxCompactTrk_v2::FloatToShort( const float rval ) const
{
...
float TFvtxCompactTrk_v2::ShortToFloat( const unsigned short ival ) const
{
 ...
 PHPoint TFvtxCompactTrk_v2::get_fvtx_vtx() const
 {
...
float TFvtxCompactTrk_v2::get_fvtx_phi() const
{
  return ShortToFloat(fvtx_phi);
}

float TFvtxCompactTrk_v2::get_fvtx_theta() const
{
  return ShortToFloat(fvtx_theta);
}

float TFvtxCompactTrk_v2::get_fvtx_eta() const
{
  return -log(tan(get_fvtx_theta()/2));
}

float TFvtxCompactTrk_v2::get_chi2_ndf() const
{
  return ShortToFloat(chi2_ndf);
}

void TFvtxCompactTrk_v2::set_track_vtx( PHPoint pnt )
{
...
void TFvtxCompactTrk_v2::set_fvtx_phi( float phi )
{
  fvtx_phi = FloatToShort(phi);
}

void TFvtxCompactTrk_v2::set_fvtx_theta( float theta )
{
  fvtx_theta = FloatToShort(theta);
}

void TFvtxCompactTrk_v2::set_fvtx_eta( float eta )
{
  fvtx_theta = FloatToShort(2*atan(exp(-eta)));
}

void TFvtxCompactTrk_v2::set_chi2_ndf( float _chi2 )
{

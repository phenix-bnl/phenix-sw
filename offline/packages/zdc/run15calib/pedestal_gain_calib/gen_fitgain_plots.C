...
void gen_fitgain_plots(const int runnumber= 423451){

  char inname[255];
  sprintf(inname,"adchisto/adc%d.root",runnumber);
  TFile* in = new TFile(inname);
  TCanvas* c=new TCanvas("c","c",1150,1800);
  c->Divide(5,6);
  ...
  for( int ch=0; ch<32; ch++ )
      ...
      if( p0> p2 )
	{
      ...
      h->GetXaxis()->SetRangeUser(0,4095);
    
      h->Draw();
    }
  c->cd(0);
  char cname[128];
  sprintf(cname,"resultsfitpng/gain_latex.eps");
  c->Print(cname);
}

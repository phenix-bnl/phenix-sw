...
int FitSlew1(char *rootfile){
  ...
  TF1 *f2 = new TF1("f2","[0]+ [1]/sqrt(x-[2])+exp([3]+[4]*(x-[2]))+pol3(5)+pol2(9)");
  TF1 *f1 = new TF1("f1","[0] + [1]/([2]-x) + [3]*x + [4]*x^2 + [5]*x^3+pol3(6)");
  f1 -> SetLineColor(4);
  f2 -> SetLineColor(4);
  ...
  for(int j=0; j<8; j++) GetBin(1, j, h1[j] );  
  for(int j=0; j<8; j++) g[1][j] -> SetMarkerStyle(20);

  pict0->Print("Slew1_fit.pdf[","pdf");

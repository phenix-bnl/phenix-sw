#ifndef __TRIGRUNLVL1V2_H
#define __TRIGRUNLVL1V2_H
...
class TrigRunLvl1v2: public TrigRunLvl1
{
 public:
  TrigRunLvl1v2();
  TrigRunLvl1v2(const TrigRunLvl1v2&);

  virtual ~TrigRunLvl1v2() {}

  TrigRunLvl1v2* clone() const { return new TrigRunLvl1v2(*this); }
  ...
  void Reset() {return;} // the content should not be resetted event by event
   ...
   const char * get_lvl1_trigger_description() const {return trigger_description.Data();}
   ...
   const char * get_lvl1_bbcll1_description() const {return bbcll1_description.Data();}
   ...
   const char * get_lvl1_partition_name() const {return partition_name.Data();}
   ...
   const char * get_lvl1_trig_name(const unsigned int i) const {return lvl1_trig_name[i].Data();}
   ...
   const char * get_lvl1_rbit_name(const unsigned int i) const {return lvl1_rbit_name[i].Data();}
  ...
  private:
   void copyTo(TrigRunLvl1v2&) const;
 ...
 protected:
   ...
   TString       trigger_description;          // the phTrigger version (eg. BigPartition)
   unsigned int  trigger_version;              // the phTrigger version number (eg. 80)   

   TString       bbcll1_description;           // the phTrigger BBCLL1 (eg. BONA_FIDA_QGP)
   unsigned int  bbcll1_version;               // version number (eg. 5)

   TString       partition_name;               // name of partition run for this PRDF

   TString       lvl1_trig_name[32];           // list of the 32 level1 trigger names
                                               //          (eg. "BBCLL1>=2")
   TString       lvl1_rbit_name[130];          // list of the 130 reduced bit names
   ...
   ClassDef(TrigRunLvl1v2,1)

};

#endif /*  __TRIGRUNLVL1V2_H */

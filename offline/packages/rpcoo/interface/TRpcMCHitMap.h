// $Id: TRpcMCHitMap.h,v 1.2 2008/08/28 00:50:21 kempel Exp $

#ifndef _TRpcMCHitMap_h_
#define _TRpcMCHitMap_h_
...
/*! \ingroup container */
//! Container for TRpcMCHit objects

/*! 
  TRpcMCHitMap is the container for TRpcMCHit interface objects.  As is
  ...
  TRpcMCHitMap::get methods provide access to objects in the containers.
  The return value of all TRpcMCHitMap::get methods is a iterator or
  const_iterator.  Iterators are also described in the <a
  href=index.html> FAQ </a>.

*/
class TRpcMCHitMap : public PHMap< PHKey::key_type, TRpcMCHit, TRpcMCHit_v1 >  
{
...
public:
  ...
  /*! Construct with key */
  TRpcMCHitMap(PHKey::map_key_type map_key): 
  	PHMap<PHKey::key_type,TRpcMCHit, TRpcMCHit_v1>(map_key),
  	_count(0)
	{}  

  /*! Virtual destructor */
  virtual ~TRpcMCHitMap() 
	{}

  //! @name Insertors
  //@{    
  /*! 
    Insert an new TRpcMCHit into map and return an iterator to the newly created
    object.
  */
  ...
  /*! Get an iterator to all hits in given radial segment*/
  iterator get(UShort_t arm, UShort_t station, UShort_t octant, UShort_t halfoctant, UShort_t rseg );
  
  /*! Get a const iterator to all hits in given radial segment */	       
  ...
  void clear() { 
		_count=0; 
		PHMap<PHKey::key_type, TRpcMCHit, TRpcMCHit_v1>::clear(); 
	}
  //@}

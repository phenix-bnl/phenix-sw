#ifndef __DEMCCLUSTERLOCALWRAPPER_H__
#define __DEMCCLUSTERLOCALWRAPPER_H__

#include <stddef.h>
#include "PHTable.hh"
#include "dEmcClusterLocal.h"
class dEmcClusterLocalWrapper: public PHTable
{
public:
  dEmcClusterLocalWrapper(const char* name = "dEmcClusterLocal", const size_t& max_rows = 1);
//  dEmcClusterLocalWrapper(const dEmcClusterLocalWrapper& source);
//  dEmcClusterLocalWrapper& operator=(const dEmcClusterLocalWrapper& source);

  ~dEmcClusterLocalWrapper();

  void* RawTableData();
  DEMCCLUSTERLOCAL_ST* TableData();

  DEMCCLUSTERLOCAL_ST& operator[](const size_t& row);
  const DEMCCLUSTERLOCAL_ST& operator[](const size_t& row) const;
...
private:
  DEMCCLUSTERLOCAL_ST* fTableData;

  ClassDef(dEmcClusterLocalWrapper,1)
};
#endif /*__DEMCCLUSTERLOCALWRAPPER_H__*/

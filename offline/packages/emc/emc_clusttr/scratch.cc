 { 
   ...
   //-----------------------------------
   /*
   \cp clust_scaninit.root clust_scaninit_cpy.root
   \cp clust_scanrun.root clust_scanrun_cpy.root
   \cp clust_scansmtwr.root clust_scansmtwr_cpy.root
   \cp clust_scanplot.root clust_scanplot_cpy.root
   */
   //-----------------------------------
   ...
   Evt* evt =  new Evt();
   evt->Init_mdst(nt_evt,nt_trk,nt_emc);
   ...
   while( evt->Next() ) if( evt->GetN() % 100 == 0 ) cout<<" evnum = "<<evt->GetN()<<" / "<<evt->GetEntries()<<endl;

   //------------------------------------------------------------------------------------------
w   gSystem->Load("/phenix/u/htorii/lib/libmicrodst.so");
   gSystem->Load("/phenix/u/htorii/lib/libdstrun.so");
   gSystem->Load("/phenix/u/htorii/local/photon/clusttr/.libs/libclusttr.so");

   TFile*  f = new TFile("/phenix/data17/phnxreco/v05/dsts/DST_v05_Stream01-0000012468-0000.proot");
   TTree* T = (TTree*)(gROOT->FindObject("T"));
   Evt* evt =  new Evt();
   evt->Init_DST(T);
   TH1F* h_nemc = new TH1F("h_nemc","nemc",100,0,400);

   {   
    while( evt->Next() ){
      if( evt->GetN()%100==0 ) cout<<" evnum = "<<evt->GetN()<<" / "<<evt->GetEntries()<<endl;
      h_nemc->Fill(evt->_glb.emcnhit);
     }
   }

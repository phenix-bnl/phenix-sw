...
int SumupTH1(TH1* hout, TH1* hin, TF1* fx, char *opt = ""){
  ...
  for( binx = ax_first; binx <= ax_last; binx++){
    ...
    if( cont != 0. ){
      ...
      slx = NSLICETH1;
      while( slx-- ){
	xin = ( upx - lowx ) * (slx + 0.5)/NSLICETH1 + lowx;
	xout = fx->Eval(xin);
	hout->Fill(xout,cont);
      }
    } // End of if(cont!=0.)
...
int SumupTH2(TH2* hout, TH2* hin, TF1* fx, TF1* fy, char *opt = ""){
  ...
  for( binx = ax_first; binx <= ax_last; binx++){
    for( biny = ay_first; biny <= ay_last; biny++){
      ...
      if( cont != 0. ){
	cont = cont / NSLICETH2 / NSLICETH2 ;
	slx = NSLICETH2;
	while( slx-- ){
	  sly = NSLICETH2;
	  while( sly-- ){
	    xin = ( upx - lowx ) * (slx + 0.5)/NSLICETH2 + lowx;
	    yin = ( upy - lowy ) * (sly + 0.5)/NSLICETH2 + lowy;
...
int SumupTH3(TH3* hout, TH3* hin, TF1* fx, TF1* fy, TF1* fz, char *opt = ""){
  ...
  for( binx = ax_first; binx <= ax_last; binx++){
	...
	if( cont != 0. ){
	  cont = cont / NSLICETH3 / NSLICETH3 / NSLICETH3 ;
	  //
	  slx = NSLICETH3;
	  while( slx-- ){
	    sly = NSLICETH3;
	    while( sly-- ){
	      slz = NSLICETH3;
	      while( slz-- ){
		xin = ( upx - lowx ) * (slx + 0.5)/NSLICETH3 + lowx;
		yin = ( upy - lowy ) * (sly + 0.5)/NSLICETH3 + lowy;
		zin = ( upz - lowz ) * (slz + 0.5)/NSLICETH3 + lowz;

...
TF1* fit_mpeak(TH1F* hist,float gmin,float gmax,
	       float emin,float emax,char* addname,
	       int opt,char* fitopt){

  cout<<" =================== fit_mpeak ==================== "<<endl;
  if( hist == 0 ) {
    cerr<<" Error in usage:: fit_mpeak.cc(TH1F*) "<<endl;
  ...
  TH1F* histe = fit_bg->GetHistogram();
  histe->DrawCopy("same");
  fitg->SetFillColor(4);
  ...
  TH1F* h_bg = (TH1F*)(hist->Clone());
  h_bg->Eval(fit_bg,"R");
  TH1F* hg = (TH1F*)(hist->Clone());
  hg->Add(h_bg,-1);
  ...
  cout<<" TH1F :"<<mipname<<"    is created"<<endl;

#ifndef __DEMCCLUSTERLOCALEXTWRAPPER_H__
#define __DEMCCLUSTERLOCALEXTWRAPPER_H__

#include <stddef.h>
#include "PHTable.hh"
#include "dEmcClusterLocalExt.h"
class dEmcClusterLocalExtWrapper: public PHTable
{
public:
  dEmcClusterLocalExtWrapper(const char* name = "dEmcClusterLocalExt", const size_t& max_rows = 1);
//  dEmcClusterLocalExtWrapper(const dEmcClusterLocalExtWrapper& source);
//  dEmcClusterLocalExtWrapper& operator=(const dEmcClusterLocalExtWrapper& source);

  ~dEmcClusterLocalExtWrapper();

  void* RawTableData();
  DEMCCLUSTERLOCALEXT_ST* TableData();

  DEMCCLUSTERLOCALEXT_ST& operator[](const size_t& row);
  const DEMCCLUSTERLOCALEXT_ST& operator[](const size_t& row) const;
  ...
  // MV 2001/12/04 deadmap & warnmap added
  ...
  void set_e9(size_t n, float v) {
    fTableData[n].e9 = v;
  }
  float get_e9(size_t n) const {
    return fTableData[n].e9;
  }
  void set_re9(size_t n, float v) {
    fTableData[n].re9 = v;
  }
  float get_re9(size_t n) const {
    return fTableData[n].re9;
  }
  
  void set_yz_cg(size_t d0, size_t n, float v) {
    fTableData[n].yz_cg[d0] = v;
  }
  float get_yz_cg(size_t d0, size_t n) const {
    return fTableData[n].yz_cg[d0];
  }


private:
  DEMCCLUSTERLOCALEXT_ST* fTableData;

  // MV 2001/12/12 Incremented class version 1 -> 2
  ClassDef(dEmcClusterLocalExtWrapper,2)
};
#endif /*__DEMCCLUSTERLOCALEXTWRAPPER_H__*/

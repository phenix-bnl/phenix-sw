...
// Returns the energy of the ich-tower (0 if ich not found in the fHitList)
{
  if ( _contains_towers)
    {
      vector<emcTowerContent*>::iterator ph;
      if( fTowerList.empty() ) return 0;
      ph = fTowerList.begin();
      while( ph != fTowerList.end() ) {
	if( (*ph)->TowerID() == ich ) return (*ph)->Energy();
	ph++;
      }
      return 0;
    }
  ...
  if ( _contains_towers)
    {
      ...
      while( ph != fTowerList.end() ) 
	{
	  EmcIndexer::iPXiSiST((*ph)->TowerID(),iS,iST);
	  EmcIndexer::iSTxyST(iS,iST,ixl,iyl);
	  if( ixl == ix && iyl == iy ) return (*ph)->Energy();
	  ph++;
	}
      return 0;  
    } 
...
int EmcCluster::GetNDead()
{
  if (  _contains_towers)
    {
      emcTowerContent* hmax = GetMaxTowerContentPtr();
      int dead = hmax->ErrorNeighbours();
      int ndead = 0;
...
// Returns the energy in 2x2 towers around the cluster Center of Gravity
{
    ...
    if ( _contains_towers)
      {
	vector<emcTowerContent*>::iterator p;
	int iS,iST;
	es=0;
	if( fTowerList.empty() ) return 0;
	p = fTowerList.begin();
	while( p != fTowerList.end() ) 
	  {
...
// Returns the energy in 3x3 towers around the cluster Center of Gravity
{
  ...
  if ( _contains_towers)
      ...
      while( ph != fTowerList.end() ) 
	{
	  EmcIndexer::iPXiSiST((*ph)->TowerID(),iS,iST);
	  EmcIndexer::iSTxyST(iS,iST,ixl,iyl);
	  if( abs (ixl - ix0) <=1  && abs (iyl - iy0) <=1   ) e+= (*ph)->Energy();
	  ph++;
	}
      return e;  
    }
...
emcTowerContent*  EmcCluster::GetMaxTowerContentPtr()
{
  // here we cache the once-obtained result
  if ( my_hmax) return my_hmax;

  if ( ! _contains_towers) return 0;

  vector<emcTowerContent*>::iterator ph;
  float emax=0;
  emcTowerContent *ht = 0;
...
/// Returns n EmcModules (sorted) with the maximum energy
vector<emcTowerContent*>&  EmcCluster::GetTowerVector()
{
...
//  Returns cluster 1-st (px,py) and 2-d momenta (pxx,pxy,pyy)
{
	...
	*pxx = 0; *pxy = 0; *pyy = 0;
	nhit=fTowerList.size();
	if( nhit <= 0 ) return;
	
	fOwner->Momenta( nhit, &fTowerList[0], &e, &x, &y, &xx, &yy, &xy );
...
int EmcCluster::GetPeaks( EmcPeakarea* PkList, EmcModule* ppeaks )
{
  ...
  for( ic=0; ic<nhit; ic++ ) {
	...
	in--;
      ...
      if( npk >= fgMaxNofPeaks ) { delete [] hlist; return -1; }
...
// this is an internal function which is getting called in case we 
// have a towercontent-based object

void EmcPeakarea::GetCharLean( float* pe, float* pec, 
...

{
  ...
  *pe = 0;
  ...
  fOwner->Gamma(nh, &fTowerList[0], &chi, &chi0, &e1, &x1, &y1, &e2, &x2, &y2, ndf);
  fNdf=ndf;
  *pchi=chi0;
...
void EmcPeakarea::GetChar_compactPWG( const float pxcgmin, const float pycgmin, const float pecore,
...
// GetCorrPos, GetGlobalPos, GetErrors and "EmcPeakarea" methods GetCGmin, GetChi2
{
  ...
  *pe = 0;
  ...
  int ndf; // Gamma parameter list changed MV 28.01.00
  //    fOwner->Gamma(nh, phit, &chi, &chi0, &e1, &x1, &y1, &e2, &x2, &y2, ndf);
  fOwner->Gamma(nh, &fHitList[0], &chi, &chi0, &e1, &x1, &y1, &e2, &x2, &y2, ndf);
  fNdf=ndf;
  *pchi=chi0;
...
{
    float chi, chi0 /*, chicorr -- flagged by scan-build as dead assignment */;
    float e1, x1, y1, e2, x2, y2;
    int nh;
    EmcModule *phit, *vv;
    vector<EmcModule>::iterator ph;
...
int EmcPeakarea::GetGammas( EmcEmshower* ShList)
{
  ...
  float chi, chi0, e1, x1, y1, e2, x2, y2 /*, chicorr -- flagged by scan-build as dead assignment */;
  int nh, ig;
  EmcModule *phit, *vv;

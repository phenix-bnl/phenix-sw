...
//
// Implementation of class : mEmcTOFCorr6Module
//
...
//          Modified for Run03V01 production by Peter Tarjan July/2/2003
//          Modified for Run04 200 GeV Au-Au production by Peter Tarjan May/28/2004
//-------------------------------------------------------------------------
...
mEmcTOFCorr6Module::mEmcTOFCorr6Module()
{ 
...
mEmcTOFCorr6Module* mEmcTOFCorr6Module::instance()
{ 
  static mEmcTOFCorr6Module* instance_ = new mEmcTOFCorr6Module;
  return instance_;
};
...
void mEmcTOFCorr6Module::readDataFromDB(const int run_number)
{
  ...
  if (!bankManager)
    {
      std::cerr << PHWHERE << " Can not get DMBS=" 
		<< emcDBMS::asString()
		<< std::endl;
      return;
    }
...
// fetch sector corrections
      ...
      if (emcBank)
	{
	  ...
	  cout << "<I> mEmcTOFCorr6Module::readDataFromObjy(): Sector corrections read in." << endl;
	  delete emcBank;
	  isvalid = 1;
	}      

// fetch tower corrections      
      ...
      if(emcBank)
	{
	  ...
	  cout << "<I> mEmcTOFCorr6Module::readDataFromDB(): Tower corrections read in." << endl;
	  delete emcBank;
	  isvalid = 1;
	}

// fetch global sector corrections
      calibname = "calib.emc.VD_T0Sector";
      emcBank = bankManager->fetchBank("PdbEmcT0SectorBank", bankID, calibname, run_number);
      if (emcBank)
	{
	  TheSectorVd = *( (PdbEmcT0Sector *)( & emcBank->getEntry(0) ) );
	  cout << "<I> mEmcTOFCorr6Module::readDataFromDB(): VD corrections read in." << endl;
	  delete emcBank;
	  isvalid = 1;
	}      
...
//
float mEmcTOFCorr6Module::get_correction( int iarm, int isect, int iz, int iy )
{
 
  // the sectorT0 correction are not yet determined at this point
//    cout 
//      << iarm << ".." << isect << ".." << iy << ".." << iz << "......" 
...
{
  cout<<" mEmcTOFCorr6Module::eventFirst() This is obsolete method.. "<<endl;
  return false;
};
//-------------------------------------------------------------------------
PHBoolean mEmcTOFCorr6Module::event(PHCompositeNode *topNode) 
{  
  //  int irun;
  float tof,tofcorr;
  ...
  if ( !towers->isValid() ) return false;
  ...
  for ( size_t i = 0; i < towers->size(); ++i ) 
  	  ...
  	  iz >= 0 && iz < 96 && iy >= 0 && iy < 48 )
	{
  	...
  	{
	  cout << " mEmcTOFCorr6Module:: arm(" << iarm << "),sect(" << isect

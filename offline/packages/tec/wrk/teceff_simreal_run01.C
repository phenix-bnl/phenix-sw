...
void teceff_simreal_run01(Int_t firstEvent=2, Int_t lastEvent=1200, const char *prdfIFile2="/phenix/data14/eventdata_2001/EVENTDATAxxx_P01-0000027197-0005.PRDFF", char *prdfIFile1="/phenix/workarea/lebedev/trkeff_central_run01/pionplus_1200evts_4sectors_gain3000_6gev.prdf") {
...
//TFile *top = new TFile("trkeff27197-0005.root","recreate");
float ntpart[38];
...
// Initialize the tables
  ...
  tecNode->addNode(TecOutNodeTest);
...
// Create TAO, TGO and TCO  
  ...
  TecCalibrationObject* TecCalibration2 = new TecCalibrationObject();
    ...
    PHIODataNode<TObject>* TecDetCalNode = 
      new PHIODataNode<TObject>(TecCalibration,"TecCalibration");
    parNode->addNode(TecDetCalNode);

// Set search time for the test simulated single particle
...
// Get the data from Objy database for the simulated particle
   TecAddress->Fetch();
   TecGeometry->Fetch();
...
cout << "  Copying TecOut..." << endl;
  ...
  for(int i=0; i<tecout->getNTracks(); i++) {
    ...
    nok++;
     ...
     cout << "First (simulated) TecOut : " << tecout ->getNTracks() << " " 
                               << tecout->getNHits() << endl;
     cout << "Second (simulated) TecOut : " << tecouttest ->getNTracks() << " " 
...
//    cout << "Event from second (data) file fetched." << endl;
    if(firstevent==0) {
      runNumber = thisEvent2->getRunNumber();
        TecAddress2->setRunNumber(runNumber);
          PHTimeStamp timeStamp = TecAddress2->getTimeStamp();
        TecCalibration2->setTimeStamp(timeStamp);
        TecAddress2->Fetch();
...
cout << "Bbc Vertex, Trigger : " << dBbcOut->get_VertexPoint(0) << 
        "      " << trigScaled[0] << " " << (trigScaled[0] & 0x40) << endl;
...
// Analyze this event only if it is good
...
//    dTecFemData->Show();
     ...
     cout << "      First (real) TecOut : " << tecout ->getNTracks() << " "
                               << tecout->getNHits() << endl;
     cout << "      Second (simulated) TecOut(TEST) : " << tecouttest ->getNTracks() << " "
                                << tecouttest->getNHits() << endl;

// $Id: TMutClus_v3.cxx,v 1.6 2011/12/29 20:19:29 slash Exp $

/*!
   \file    TMutClus_v3.cxx
...
ClassImp(TMutClus_v3)
using namespace std;

//________________________________________________________
TMutClus_v3::TMutClus_v3() : 
  _arm(0),
  _station(0),
  _octant(0),
  _half_octant(0),
...
//________________________________________________________
TMutClus_v3::TMutClus_v3(const TMutClus* base_ptr) :       
  TMutClus(*base_ptr), 
  ...
  _status(base_ptr->get_status())
{
  ...
  _centroid_list.push_back( new centroid_value_type( *base_ptr->get_centroid( index ) ) );

}

//________________________________________________________
TMutClus_v3::TMutClus_v3(const TMutClus& base_ref) :       
  TMutClus(base_ref), 
  ...
  _status(base_ref.get_status())
{
  ...
  _centroid_list.push_back( new centroid_value_type( *base_ref.get_centroid( index ) ) );

}

//________________________________________________________
void TMutClus_v3::print(ostream& os) const 
{
  ...
  if(get_refit_less()) os << "REFIT_LESS ";
  if(get_refit_more()) os << "REFIT_MORE ";
  os << endl;
  ...
  os << hit_ptr->get()->get_strip() << ":" << hit_ptr->get()->get_q() << ":" << hit_ptr->get()->get_error_q() << " ";
  os << endl;

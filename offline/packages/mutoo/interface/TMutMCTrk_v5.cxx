// $Id: TMutMCTrk_v5.cxx,v 1.1 2015/04/01 16:04:22 snowball Exp $
...
ClassImp(TMutMCTrk_v5);

TMutMCTrk_v5::TMutMCTrk_v5() :   
  _arm(0),
  _index(0),
  _file_key(0),
  _track_id(0),
  ...
  _pz_ds_gap(0),
  _parent_track_id(0),
  _grandparent_track_id(0){;}

TMutMCTrk_v5::TMutMCTrk_v5(const Key& key, 
			   UShort_t arm, 
			   UShort_t index) :
  TMutMCTrk(key), 
  _arm(arm), 
...
TMutMCTrk_v5::TMutMCTrk_v5(const TMutMCTrk* base_ptr):
  ...
  _parent_track_id(base_ptr->get_parent_track_id()),
  _grandparent_track_id(base_ptr->get_grandparent_track_id()){;}

TMutMCTrk_v5::TMutMCTrk_v5(const TMutMCTrk& base_ref):
  TMutMCTrk(base_ref),
  _arm(base_ref.get_arm()),
  _index(base_ref.get_index()),
  _file_key(base_ref.get_file_key()),
  _track_id(base_ref.get_track_id()),
  _pid(base_ref.get_pidG4()),
  ...
  _parent_track_id(base_ref.get_parent_track_id()),
  _grandparent_track_id(base_ref.get_grandparent_track_id()){;}
  

  
void TMutMCTrk_v5::print(std::ostream& os) const {		       
  MUTOO::PRINT(os,GetName());
  os << " arm: " << _arm
     << " index: " << _index << std::endl;
  os << " track id: " << _track_id << " particle id: " << _pid << " parent track id: " << _parent_track_id<<" parent id : " << _parent_id << " grandparent track id: " << _grandparent_track_id<< std::endl;

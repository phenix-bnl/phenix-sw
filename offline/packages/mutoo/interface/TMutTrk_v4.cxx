// $Id: TMutTrk_v4.cxx,v 1.11 2011/12/29 20:19:31 slash Exp $
...
//_______________________________________
TMutTrk_v4::TMutTrk_v4(const Key& key,
  UShort_t arm,
  UShort_t octant,
  UShort_t index) :
  TMutTrk(key),
...
//_______________________________________
TMutTrk_v4::TMutTrk_v4(const TMutTrk* base_ptr) :
  TMutTrk(*base_ptr),
...
//_______________________________________
TMutTrk_v4::TMutTrk_v4(const TMutTrk& base_ref) :
  TMutTrk(base_ref),
...
//_______________________________________
void TMutTrk_v4::set_trk_par_vtx( const TMutTrkPar& trk_par )
{ _trk_par_vtx = trk_par; }

//_______________________________________
void TMutTrk_v4::print(std::ostream& os, bool max) const
{
  ...
  os << setw(5) << setprecision(3) << setiosflags(ios::showpoint) << setiosflags(ios::fixed);	  
  os << _trk_par_vtx.get_x() << ", ";
  os << _trk_par_vtx.get_y() << ", ";
  os << _trk_par_vtx.get_px() << ", ";
  os << _trk_par_vtx.get_py() << ", ";
  os << _trk_par_vtx.get_pz() << "}" << std::endl;		
  ...
  os << " n_reco_par [should be 0] - " << _reco_par_list.size() << std::endl; 
  ...
  if(max) {
    ...
    if(max)
    {
      ...
      for(;residual_iter != _w_residual_list.end(); ++residual_iter) residual_iter->print();
...
//_______________________________________
double TMutTrk_v4::get_w_chi_square_pdf() const 
{ 
  int ndf( get_ndf() );
...
//_______________________________________
double TMutTrk_v4::get_r_chi_square_pdf() const 
{ 
  int ndf( get_ndf() );

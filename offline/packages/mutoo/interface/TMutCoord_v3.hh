// $Id: TMutCoord_v3.hh,v 1.5 2011/12/29 20:19:29 slash Exp $
...
#ifndef __TMUTCOORD_V3H__
#define __TMUTCOORD_V3H__
...
class TMutCoord_v3 : public TMutCoord
{
 ...
 public:
  ...
  TMutCoord_v3(const TMutCoord& base_ref);

  TMutCoord_v3(const TMutCoord* base_ptr);

  virtual ~TMutCoord_v3(){;}
  ...
  Float_t get_q_peak() const 
  {return _q_peak;}
  
  Float_t get_q_tot() const 
  {return _q_tot;}
  
  double get_q_error() const 
  {return _q_error;}
  
  UShort_t get_peak_strip() const 
  {return _peak_strip;}
  
  Float_t get_w() const 
  {return _w;}
  
  Float_t get_cos_theta_wire() const 
  {return _cos_theta_wire;}
  ...
  void set_q_peak(Float_t q_peak) 
  { _q_peak = q_peak;}
  
  void set_q_tot(Float_t q_tot) 
  { _q_tot = q_tot;}
  
  void set_q_error(double q_error) 
  { _q_error = q_error;}
  
  void set_peak_strip(UShort_t peak_strip) 
  { _peak_strip = peak_strip;}
  
  void set_w(Float_t w) 
  { _w = w;}
  
  void set_cos_theta_wire(Float_t cos_theta) 
  { _cos_theta_wire = cos_theta;}
  ...
  void set_peak_bound() 
  {_status |= 1<<TMutCoord::PEAK_BOUND;}  
  
  bool get_peak_bound() const 
  { return (_status & 1<<TMutCoord::PEAK_BOUND) != 0; }
  
  void set_low_charge() 
  {_status |= 1<<TMutCoord::LOW_CHARGE;}  
  ...
  void set_stereo() {_status |= 1<<TMutCoord::STEREO;}  
  bool get_stereo() const { return (_status & 1<<TMutCoord::STEREO) != 0; }
...
#ifndef __CINT__
  void push_chi_sqr_inc(ULong_t track_key, double chi_square) { 
    _chi_inc.insert(std::make_pair(track_key,chi_square)); 
  }
  double get_chi_sqr_inc(ULong_t track_key) {
    std::map<ULong_t, double>::iterator iter = _chi_inc.find(track_key);
...
#ifndef __CINT__ 
  std::map<ULong_t, double> _chi_inc;
#endif

  ClassDef(TMutCoord_v3,1)
};

		
#endif /* __TMUTCOORD_V3H__*/

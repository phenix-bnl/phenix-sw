void Run_mpc(char* runnum, char* outputdir, unsigned int nevents = 0,
	      ...
	      char* cnt_list = 0, char* mwg_list = 0 )
{
  ...
  SubsysReco *mpc_tower = new MpcReco("mpc_tower.root");
  rc->set_IntFlag("MPC_RECO_MODE",0x2);
  se->registerSubsystem(mpc_tower);
  ...
  TString outfn = "condor_output/pi0_"; outfn+=runnum; outfn+=".root";

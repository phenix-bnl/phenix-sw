...

{
  ...
  PHTimeStamp TimeStart = PHTimeStamp(2000,6,1,0,0,0);
  PHTimeStamp TimeStop  = PHTimeStamp(2000,9,30,0,0,0);

  // TOF Detector Object
  TofAddressObject* TofAddress = new TofAddressObject();
  TofAddress->fetchFromFile("toffemmap.txt.year1");
  ...
  TofGeometry->fetchFromFile("tofpanelgeo.txt.year1","tofslatoffset.txt");

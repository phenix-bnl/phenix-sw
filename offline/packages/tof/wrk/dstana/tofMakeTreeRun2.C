...
void tofMakeTreeRun2(Int_t maxEvents=0, 
		     ...
		     const char *rootOFile="toftree.root")
{
  ...
  PHIODataNode<PHObject>* RunHeaderNode = 
    (PHIODataNode<PHObject>*)mainIter->findFirst("PHIODataNode","RunHeader");
  ...
  while ((maxEvents == 0) || (eventNumber < maxEvents)) 
      ...
      if (eventNumber > 1) 
	{
      ...
      // Grab the New data nodes
      if (!RunHeaderNode) 
	{
	  cout << "Dst Read: Could not find data node RunHeader" << endl;
	} 
      else 
	{
	  RunHeader* runHeader = (RunHeader*)RunHeaderNode->getData();
	}
      // Grab the data nodes
      //if (!dRunHeaderNode) 
      //{
      ...
      if(eventNumber == 1)
	{
	  //int runNumber = dRunHeader->get_run(0); // before run2_v01
	  int runNumber = runHeader->get_RunNumber(); // after run2_v02

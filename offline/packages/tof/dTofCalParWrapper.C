...
ClassImp(dTofCalParWrapper);

using namespace std;

dTofCalParWrapper::dTofCalParWrapper(const char* name, const size_t& max_rows)
  : PHTable(name,max_rows)
{
  size_t rowSize = sizeof(DTOFCALPAR_ST);
  if (max_rows > 0) { // Avoid allocating a space of zero size!
     fTableData = new DTOFCALPAR_ST[max_rows];
     SetMaxRowCount(max_rows);
  }
  else {
     fTableData = new DTOFCALPAR_ST[1];
     SetMaxRowCount(1);
  }
...
dTofCalParWrapper::dTofCalParWrapper(const dTofCalParWrapper& source)
  : PHTable(source)
{
  // The row count, the max. row count, and the name are
  // already set in the PHTable copy ctor.

  fTableData = new DTOFCALPAR_ST[source.MaxRowCount()];
  SetRowSize(sizeof(DTOFCALPAR_ST));
  SetType("dTofCalPar");
  fTableHeader->dsl_pointer  = (long)this;
  fTableHeader->data_pointer = (long)fTableData;
...
void*
dTofCalParWrapper::RawTableData()
{
...
dTofCalParWrapper::~dTofCalParWrapper()
{
  delete [] fTableData;
}

DTOFCALPAR_ST*
dTofCalParWrapper::TableData()
{
  return fTableData;
}

DTOFCALPAR_ST&
dTofCalParWrapper::operator[](const size_t& row)
{
  return fTableData[row];
}

const DTOFCALPAR_ST&
dTofCalParWrapper::operator[](const size_t& row) const
{
  return fTableData[row];
}

void
dTofCalParWrapper::Print(const size_t num_rows, const size_t first_row) const
{
  cout << "   row :";
  cout << " " << setw(11) << "option";
  cout << " qvc_corr[0]";
...
void
dTofCalParWrapper::Print(Option_t* option) const
{
...
void
dTofCalParWrapper::SetMaxRowCount(const size_t& max_rows)
{
  ...
  if (max_rows > (size_t) fTableHeader->maxlen) {
     DTOFCALPAR_ST* newData = new DTOFCALPAR_ST[max_rows];
     if (fTableData) {
...
void
dTofCalParWrapper::SetRowCount(const size_t& n)
{
...
void
dTofCalParWrapper::SetRowSize(const size_t& row_size)
{
  if (row_size > 0) {
...
void
dTofCalParWrapper::Streamer(TBuffer& b)
{
   // Stream an object of class dTofCalParWrapper.
   // What should be done on output if the table is empty?

   if (b.IsReading()) {
     b.ReadVersion();
     PHTable::Streamer(b);         // Read the table header.
     if (RowSize() != sizeof(DTOFCALPAR_ST)) {

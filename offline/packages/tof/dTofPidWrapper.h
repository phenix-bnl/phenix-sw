#ifndef __DTOFPIDWRAPPER_H__
#define __DTOFPIDWRAPPER_H__

#include <stddef.h>
#include "PHTable.hh"
#include "dTofPid.h"
class dTofPidWrapper: public PHTable
{
public:
  dTofPidWrapper(const char* name = "dTofPid", const size_t& max_rows = 1);
  dTofPidWrapper(const dTofPidWrapper& source);
  dTofPidWrapper& operator=(const dTofPidWrapper& source);

  ~dTofPidWrapper();

  void* RawTableData();
  DTOFPID_ST* TableData();

  DTOFPID_ST& operator[](const size_t& row);
  const DTOFPID_ST& operator[](const size_t& row) const;
  ...
  void set_id_assoc(size_t n, short v) {
    fTableData[n].id_assoc = v;
  }
  short get_id_assoc(size_t n) const {
    return fTableData[n].id_assoc;
  }
  ...
  void set_pid_stat(size_t n, unsigned short v) {
    fTableData[n].pid_stat = v;
  }
  unsigned short get_pid_stat(size_t n) const {
    return fTableData[n].pid_stat;
  }
...
private:
  DTOFPID_ST* fTableData;

  ClassDef(dTofPidWrapper,1)
};
#endif /*__DTOFPIDWRAPPER_H__*/

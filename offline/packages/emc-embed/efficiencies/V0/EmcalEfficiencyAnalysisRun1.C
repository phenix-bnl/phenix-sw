...
/*  To compile this macro simply do:
 ...
 .L EmcalEfficiencyAnalysisRun1.C++
...
// To run this macro do:
 ...
 gSystem->Load("libpreco.so");
 gSystem->Load("libEG.so");
 gSystem->Load("~/afs/effic2/EmcalEfficiencyAnalysisRun1_C.so");
			 ...
			 const dEmcGeaClusterTrackWrapper* dEmcGeaClusterTrack )
{
  ...
  if ( lx>= 5 && lx<=395 && ly>= 5 && ly<=190 ) // in sector 0 fiducial area
    //if ( lx>=20 && lx<=377 && ly>=10 && ly<=189 )
    {
...
{ 
  ...
  // Cent-dependent Hagedorn fit parameters for pi0 Au+Au @ 130A GeV
  // p0hag_min_bias = 1.66 ; nhag_min_bias = -10.2
  Float_t p0hag_[CENTCLASSES] = {1.72,1.72,1.72,1.72,1.72,1.72,1.72,1.72,1.72,1.72};   // Param. Hagedorn fit Au+Au->pi0
  Float_t nhag_[CENTCLASSES]  = {-12.0,-12.0,-12.0,-12.0,-12.0,-12.0,-12.0,-12.0,-12.0,-12.0};  // Param. Hagedorn fit Au+Au->pi0 
  Int_t binMinv = 0;
...
// 		    - 1.01014e-04*momentum2.Pt()*momentum2.Pt()*momentum2.Pt() ;
	  ...
	  // Asym2 + Low-energy clusters (<100 MeV) cut

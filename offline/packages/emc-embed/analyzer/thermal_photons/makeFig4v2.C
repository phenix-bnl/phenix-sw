...
void makeFig4v2(char* inasciifile = "sumvarvol_tau015_v01.dat"){
  ...
  TH2F *myframe1 = (TH2F*)frame(title, 100, 0.15, 0.35, 100, 0., 2000, "T_{eff} (GeV)", "\epsilon_{0}/(#pi^{2}/30 T_{eff}^{4})");
  myframe1->Draw();

  sprintf(title,"ET_vs_Teff");
  TCanvas *c2 = new TCanvas(title,title,700,500);
  TH2F *myframe2 = (TH2F*)frame(title, 100,0.15,0.35, 100, 0., 5000, "T_{eff} (GeV)", "E_{T}/(#pi^{2}/30 T_{eff}^{4})");
  myframe2->Draw();
  ...
  double hc = 0.197; // GeV fm

  // Steffan-Boltzmann normalization
  Double_t EinNorm1[Npoints] ;
  Double_t EtNorm1[Npoints] ;
  Double_t EinNorm2[Npoints] ;
  Double_t EtNorm2[Npoints] ;
  Double_t EinNorm3[Npoints] ;
  Double_t EtNorm3[Npoints] ;
  for(Int_t i=0;i<Npoints;i++)
    {
      EinNorm1[i]=EinVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope1V[i],4.)/3.14/3.14;
      EtNorm1[i] =EtVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope1V[i],4.)/3.14/3.14;
      EinNorm2[i]=EinVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope2V[i],4.)/3.14/3.14;
      EtNorm2[i] =EtVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope2V[i],4.)/3.14/3.14;
      EinNorm3[i]=EinVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope3V[i],4.)/3.14/3.14;
      EtNorm3[i] =EtVarVol[i]*TMath::Power(hc,3)*30./TMath::Power(slope3V[i],4.)/3.14/3.14;
    }
  

  TGraphErrors * Ein_vs_Teff1 = new TGraphErrors(Npoints,slope1V,EinNorm1,slope1ErrV,xe) ;
  TGraphErrors * ET_vs_Teff1  = new TGraphErrors(Npoints,slope1V,EtNorm1,slope1ErrV,xe) ;
  
  TGraphErrors * Ein_vs_Teff2 = new TGraphErrors(Npoints,slope2V,EinNorm2,slope2ErrV,xe) ;
  TGraphErrors * ET_vs_Teff2  = new TGraphErrors(Npoints,slope2V,EtNorm2,slope2ErrV,xe) ;
  
  TGraphErrors * Ein_vs_Teff3 = new TGraphErrors(Npoints,slope3V,EinNorm3,slope3ErrV,xe) ;
  TGraphErrors * ET_vs_Teff3  = new TGraphErrors(Npoints,slope3V,EtNorm3,slope3ErrV,xe) ;

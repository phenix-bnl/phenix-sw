...
void makeFig2( bool weightImpParam = false )
{
//Jyvaskyla group
Double_t xJ[6]={1.,  2.,   3.,     4.,     5.,       6.} ;
//Double_t yJ[6]={0.25,0.008,0.00042,0.00004,0.0000042,0.00000055} ;
Double_t yJ[6]={0.25,0.008,0.00042,0.000035,0.000003,0.0000004} ;
TGraph * gJyv = new TGraph(6,xJ,yJ) ;

//Gale et al.,
Double_t xG[7] ={0.2,0.4,0.6,1.,2.,3.,4.} ;
Double_t yQG[7]={5.5,1.5,0.6,0.105,0.0035,0.00014,0.000007} ;
Double_t yHG[7]={7.5,3.2,1.05,0.11,0.0005,0.0000024,0.} ;
for(Int_t i=0; i<7; i++)yQG[i]+=yHG[i];
TGraph * gGale = new TGraph(7,xG,yQG) ;

//Srivastava 0.2
Double_t xS[8] = {0.2,0.4,0.6,1.,2.,3.,4.,5.} ;
Double_t yQS[8]= {18.,4.,1.05,0.15,0.003,0.00015,0.00001,0.0000007} ;
Double_t yHS[8]= {13.,2.,1.,0.2,0.0025,0.00006,0.,0.,} ;
for(Int_t i=0; i<8; i++)yQS[i]+=yHS[i];
TGraph * gSriv = new TGraph(8,xS,yQS) ;

//Alam
Double_t xA[]= {0.6,1.,2.,3.} ;
Double_t yA[]= {20.,1.05,0.004,0.000034} ;
...
//  ifstream inpQCD("/afs/rhic.bnl.gov/phenix/users/enterria/thermal_photons/data/pQCD_vogelsang_pp_gamma_200GeV_cteq6_sc1.dat");
  ...
  TCanvas * cCen = new TCanvas(title,title,650,700);
  cCen->Range(-1.02574,-8.44815,6.09926,2.99261);
  ...
  hTotHydroCen->GetXaxis()->SetRangeUser(ptmin,ptmax);
  ...
  hQGPCen->SetLineColor(91);
  hQGPCen->SetLineWidth(7);//(15);
//  hQGPCen->Draw("c");

  gpQCDcen->SetLineColor(62); //(52);
  gpQCDcen->SetLineWidth(3);//(15);
//  gpQCDcen->Draw("c");
...
//  hHadrCen->Draw("psame");
  ...
  hTotHydroCen->SetLineStyle(5);
  hTotHydroCen->SetLineColor(2);
  hTotHydroCen->SetLineWidth(3); //(15);
  hTotHydroCen->Draw("c");
...
//  hQGPCen->Draw("csame");
  ...
  pQCDplusHydroCen->SetLineWidth(2);
//  pQCDplusHydroCen->Draw("c");
  ...
  //Jyvalys results
  gJyv->SetLineStyle(1);
  gJyv->SetLineColor(28) ; //91);
  gJyv->SetLineWidth(3);
  gJyv->Draw("l") ;

  //Gale
  gGale->SetLineStyle(1);
  gGale->SetLineColor(6);
  gGale->SetLineWidth(3);
  gGale->Draw("c") ;

  //Srivastava
  gSriv->SetLineColor(8) ;
  gSriv->SetLineWidth(3) ;
  gSriv->Draw("l") ;

  //Alam
  gAlam->SetLineColor(7) ;
  ...
  TLegend *l = new TLegend(0.46,0.72,0.957,0.963,"Au+Au #rightarrow #gamma+X   [0-10% central]","bNDC");
  l->SetMargin(0.2);
  l->SetFillStyle(0);
  l->SetBorderSize(4);
//  l->AddEntry(pQCDplusHydroCen,"Total #gamma: Prompt + Thermal","l");
  l->AddEntry(gpQCDcen,"Prompt: NLO pQCD #times T_{AA}[0-10%]","l");
  l->AddEntry(hTotHydroCen,"D.d'Enterria and D.Peressounko","l");
  l->AddEntry(gJyv,"S.Rasanen et al.","l");
  l->AddEntry(gGale,"C.Gale et al.","l");
  l->AddEntry(gSriv,"D.K.Srivastava","l");
  l->AddEntry(gAlam,"J.Alam et al.","l");
//  l->AddEntry(hQGPCen,"QGP","l");
//  l->AddEntry(hHadrCen,"HRG","l");
  l->AddEntry(phnx_AuAugamma_cent,"PHENIX data","p");
  l->Draw();

  cCen->Update();
return ;

  TCanvas *r = new TCanvas("ratio_gamma_tot_over_pQCD_Cen","ratio_gamma_tot_over_pQCD_Cen",650,700);
  hratioTotOverpQCDCen->Draw("AC*");
  hratioTotOverpQCDCen->Fit("pol8");//,"R","",1.,6.);
  ...
  TCanvas * cPer = new TCanvas("photon_spec_AuAu200GeV_periph","Photon rates Au+Au 200 GeV peripheral",650,700);
  cPer->Range(-1.02574,-8.44815,6.09926,2.99261);
  cPer->SetLeftMargin(0.144);
  cPer->SetRightMargin(0.014);
  cPer->SetTopMargin(0.02914);
  cPer->SetBottomMargin(0.12);
  cPer->SetLogy(1);

  ymax = 15.;
  ymin = 1.0e-10;
 
  TH2F *framePer = (TH2F*)frame("bla", 10, ptmin, ptmax, 20, ymin, ymax, "p_{T} (GeV/c)","d^{2}N/(#pi dp_{T}^{2}dy) (GeV/c)^{-2}");
  framePer->GetXaxis()->SetRangeUser(ptmin,ptmax);
  framePer->GetXaxis()->SetTitleOffset(1.);
  framePer->GetXaxis()->SetTitleSize(0.05);
  framePer->GetYaxis()->SetTitleOffset(1.2);
  framePer->GetYaxis()->SetTitleSize(0.05);
  framePer->Draw();

  ptmax =5.65;
  hQGPPer->SetMinimum(ymin);
  hQGPPer->SetMaximum(ymax);
  hQGPPer->GetXaxis()->SetRangeUser(ptmin,ptmax);
  hQGPPer->SetLineStyle(2);  
  hQGPPer->SetLineColor(91);
  hQGPPer->SetLineWidth(5);  
  hQGPPer->Draw("lsame");

  gpQCDper->SetLineColor(62);
  gpQCDper->SetLineWidth(7);
  gpQCDper->Draw("csame");

  hHadrPer->SetLineStyle(2);
  hHadrPer->SetLineColor(1);
  hHadrPer->SetLineWidth(4);
  hHadrPer->SetMarkerStyle(20);
  hHadrPer->SetMarkerSize(0.8);
  hHadrPer->SetMarkerColor(1);
  hHadrPer->Draw("psame");

  hTotHydroPer->SetLineStyle(5);
  hTotHydroPer->SetLineColor(2);
  hTotHydroPer->SetLineWidth(7);
  hTotHydroPer->SetMinimum(ymin);
  hTotHydroPer->GetXaxis()->SetRangeUser(ptmin,ptmax);
  hTotHydroPer->Draw("csame");

  hQGPPer->SetMinimum(ymin);
  hQGPPer->Draw("lsame");

  pQCDplusHydroPer->SetLineStyle(1);
  pQCDplusHydroPer->SetLineColor(6);
  pQCDplusHydroPer->SetLineWidth(3);
  pQCDplusHydroPer->Draw("csame");

  // PHENIX data
  //gStyle->SetEndErrorSize(0.8);
  phnx_AuAugamma_per->Draw("same");
  //AuAugamma_per_lowpt1->SetLineColor(6);
  AuAugamma_per_lowpt2->Draw(">");
  AuAugamma_per_lowpt1->Draw("p");
  AuAugamma_per_lowpt3->Draw("p");
  AuAugamma_per_lowpt4->Draw(">");
  phnx_AuAugamma_per->Draw("same");
  //AuAugamma_per_lowpt3->SetLineColor(2);

  TLegend *l2 = new TLegend(0.465944,0.696319,0.956656,0.932515,"Au+Au #rightarrow #gamma+X   [60-70% peripheral]","brNDC");
  l2->SetMargin(0.2);
  l2->SetFillStyle(0);
  l2->SetBorderSize(4);
  l2->AddEntry(pQCDplusHydroPer,"Total #gamma: Prompt + Thermal","l");
  l2->AddEntry(gpQCDper,"Prompt: NLO pQCD #times T_{AA}[60-70%]","l");
  l2->AddEntry(hTotHydroPer,"Thermal: QGP+HRG","l");
  l2->AddEntry(hQGPPer,"QGP","l");
  l2->AddEntry(hHadrPer,"HRG","l");
  l2->AddEntry(phnx_AuAugamma_per,"PHENIX data","p");
  l2->Draw();

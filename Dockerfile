...
RUN sed -i 's/LIBRARIES Core /LIBRARIES Core dl /g' /tmp/root_v5.34.38/montecarlo/pythia8/CMakeLists.txt
WORKDIR /tmp/build/root
RUN cmake -S /tmp/root_v5.34.38 -B /tmp/build/root -Dpython=OFF -Dminuit2=ON -DCMAKE_INSTALL_PREFIX=/usr/local
RUN cmake --build /tmp/build/root --parallel $(nproc) && cmake --install /tmp/build/root

WORKDIR /tmp/hdf5
RUN curl -sL https://github.com/HDFGroup/hdf5/archive/refs/tags/hdf5-1_10_11.tar.gz | tar -xz --strip-components 1
WORKDIR /tmp/build/hdf5
RUN cmake -S /tmp/hdf5 -B /tmp/build/hdf5 -DCMAKE_INSTALL_PREFIX=/usr/local -DHDF5_BUILD_CPP_LIB=ON
RUN cmake --build /tmp/build/hdf5 --parallel $(nproc) && cmake --install /tmp/build/hdf5

WORKDIR /tmp/clhep
RUN curl -sL https://proj-clhep.web.cern.ch/proj-clhep/dist1/clhep-2.3.2.2.tgz | tar -xz --strip-components 2
WORKDIR /tmp/build/clhep
RUN cmake -S /tmp/clhep -B /tmp/build/clhep/
RUN cmake --build /tmp/build/clhep --parallel $(nproc) && cmake --install /tmp/build/clhep
...
WORKDIR /tmp/build/hepmc
RUN cmake -S /tmp/hepmc -B /tmp/build/hepmc/ -Dmomentum=GEV -Dlength=CM -DCMAKE_INSTALL_PREFIX=/tmp/build/phenix/new/install.1/
RUN cmake --build /tmp/build/hepmc --parallel $(nproc) && cmake --install /tmp/build/hepmc/
...
RUN utils/rebuild/build.pl --source $PWD --workdir /tmp/build/phenix 2>&1 | tee build.log

RUN mkdir offline/AnalysisTrain/Run16dAuPi0Photon/build \
 && cd offline/AnalysisTrain/Run16dAuPi0Photon/build \
 && ../autogen.sh --prefix=/tmp/build/phenix/new/install/ \
 && make -j $(nproc) \
 && make install

ENV OFFLINE_MAIN=/tmp/build/phenix/new/install/
ENV LD_LIBRARY_PATH=/tmp/build/phenix/new/install/lib/:$LD_LIBRARY_PATH

#include <packet_mutc_fpgashort.h>

Packet_mutc_fpgashort::Packet_mutc_fpgashort(PACKET_ptr data)
  : Packet_w4 (data){}
  
int *Packet_mutc_fpgashort::decode ( int *nwout)
{
  int *k;
  ...
  while ( (ch = (( k[pos] >> 24) & 0xff)) <= 0x7f &&  pos < dlength -10 )
      ...
      if (pos >= dlength)
	{
	  ...
	  *nwout = 0;
      ...
      iarr[ch*4+0] = 2047 - ( ( k[pos +0 ]        ) & 0x7ff);  //sample 0
      iarr[ch*4+1] = 2047 - ( ( k[pos +1 ] >> 12  ) & 0x7ff);  //sample 1
      iarr[ch*4+2] = 2047 - ( ( k[pos +0 ] >> 12  ) & 0x7ff);  //sample 2
      iarr[ch*4+3] = 2047 - ( ( k[pos +1 ]        ) & 0x7ff);  //sample 3
...
int *Packet_mutc_fpgashort::decode_misc (int *nwout)
{
  int *p,*k;
  int i;
...
int  Packet_mutc_fpgashort::iValue(const int ich, const int is)
{
...
int  Packet_mutc_fpgashort::iValue(const int ich, const char *what)
{
...
void Packet_mutc_fpgashort::dump ( OSTREAM &os)
{
  int i,j;
...
int   Packet_mutc_fpgashort::fillIntArray (int iarr[],

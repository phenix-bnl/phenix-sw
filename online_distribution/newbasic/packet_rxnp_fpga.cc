#include <packet_rxnp_fpga.h>

Packet_rxnp_fpga::Packet_rxnp_fpga(PACKET_ptr data)
  : Packet_w4 (data)
{
  hitlength = 0;
}
  
int *Packet_rxnp_fpga::decode ( int *nwout)
{
  ...
  while (pos <= dlength) 
    {
      ...
      if ((k[pos] & 0x40000000) != 0x40000000) break; // indicates end of data words
      ...
      if (currentmaxlen > olength)
	{
      ...
      // we need to take care of 5 input words and
      // 2 channels of 5 words each

       
      // 1st channel high gain post
      temp[ch*5+1] = ( k[pos] & 0xfff) ^ 0xfff;

      // 1st channel  high gain pre
      temp[ch*5+3] = ( (k[pos] >> 16) & 0xfff) ^ 0xfff;

      // 1st channel low gain post
      temp[ch*5+2] = ( k[pos+1] & 0xfff) ^ 0xfff;
      
      // 1st channel low gain pre 
      temp[ch*5+4] = ( (k[pos+1] >>16) & 0xfff) ^ 0xfff;
      
      // 1st channel timing sample
      temp[ch*5+0] = ( k[pos+2] & 0xfff) ^ 0xfff;
      hitlist [hitlength++] = ch;

      ch++;

      // 2nd channel high gain post
      temp[ch*5+1] = ( (k[pos+2] >> 16) & 0xfff) ^ 0xfff;

      // 2nd channel  high gain pre
      temp[ch*5+3] = ( k[pos+3] & 0xfff) ^ 0xfff;

      // 2nd channel low gain post
      temp[ch*5+2] = ( (k[pos+3] >> 16) & 0xfff) ^ 0xfff;
      
      // 2nd channel low gain pre 
      temp[ch*5+4] = ( k[pos+4] & 0xfff) ^ 0xfff;
      
      // 2nd channel timing sample
      temp[ch*5+0] = ( (k[pos+4] >> 16) & 0xfff) ^ 0xfff;
      hitlist [hitlength++] = ch;
  ...
  int *p = new int[currentmaxlen];
  int *kk = p;
  for (int i =0; i<currentmaxlen; i++) 
    {
      *kk++ = temp[i];
    }
  *nwout = currentmaxlen;
...
int Packet_rxnp_fpga::decode_to_sparse ( int *p, const int nlen )
{
  int index = 0;
  int i;
  int count = 0;
...
int *Packet_rxnp_fpga::decode_amu (int *nwout)
{
  int *p,*k;
...
int *Packet_rxnp_fpga::decode_misc (int *nwout)
{
  int *p, *k;
...
int  Packet_rxnp_fpga::iValue(const int ich, const char *what)
{
...
int   Packet_rxnp_fpga::iValue(const int ich, const int iy)
{
  // now let's derefence the proxy array. If we didn't decode
  // the data until now, we do it now
...
void Packet_rxnp_fpga::dump ( OSTREAM &os) 
{

  int i,j;
  // trigger the decoding so we get all values proper
  i =    iValue(0,0);
...
int   Packet_rxnp_fpga::fillIntArray (int iarr[],

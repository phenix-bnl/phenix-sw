#include <packet_mutc_fpganew.h>

Packet_mutc_fpganew::Packet_mutc_fpganew(PACKET_ptr data)
  : Packet_w4 (data){}
  
int *Packet_mutc_fpganew::decode ( int *nwout)
{
  int *k;

#define MUTCMAXLENGTH 4*128
#define MUTCMAXCHAN     128
  ...
  while ( (ch = (( k[pos] >> 24) & 0xff)) <= 0x7f &&  pos < dlength -10 )
      ...
      if (pos >= dlength)
	{
	  ...
	  *nwout = 0;
      ...
      // the sample order in the data stream is samp 2,0,1,3
      // re-order them below to 0,1,2,3

      
      iarr[ch*4+2] = 2047 - ( ( k[pos +0 ]        ) & 0x7ff);  //sample 0
      iarr[ch*4+0] = 2047 - ( ( k[pos +0 ] >> 12  ) & 0x7ff);  //sample 2


      iarr[ch*4+3] = 2047 - ( ( k[pos +1 ] >> 12  ) & 0x7ff);  //sample 1
      iarr[ch*4+1] = 2047 - ( ( k[pos +1 ]        ) & 0x7ff);  //sample 3


      pos += 2; //advance 2 positions, that is, go to the next channel

      decoded_data3[index+0] = ch;
      decoded_data3[index+1] = ch;
      decoded_data3[index+2] = ch;
      decoded_data3[index+3] = ch;

      decoded_data4[index+0] = 0;
      decoded_data4[index+1] = 1;
      decoded_data4[index+2] = 2;
      decoded_data4[index+3] = 3;

      decoded_data5[index+0] = iarr[ch*4+0];
      decoded_data5[index+1] = iarr[ch*4+1];
      decoded_data5[index+2] = iarr[ch*4+2];
      decoded_data5[index+3] = iarr[ch*4+3];
  ...
  data3_length = count *4;
  data4_length = count *4;
  data5_length = count *4;
...
int *Packet_mutc_fpganew::decode_misc (int *nwout)
{
  int *p,*k;
  int i;
  ...
  int j;
  for (j=0; j<8; j++)   // we get 8 user words + parity + summary word, 10 words total
    p[5+j] = ( k[i+j] & 0x8fffffff);

  for (j=8; j<10; j++)   // + summary word, 10 words total
    p[5+j] =  k[i+j] ;
...
int  Packet_mutc_fpganew::iValue(const int ich, const int is)
{
  if (ich < 0 || ich > MUTCMAXCHAN) return 0;
  if (is < 0 || is > 3) return 0;
  ...
  return decoded_data1[ich*4 + is]; 
}

// ------------------------------------------------------

int  Packet_mutc_fpganew::iValue(const int ich, const char *what)
{
...
void Packet_mutc_fpganew::dump ( OSTREAM &os)
{
  int i,j;
  ...
  COUT << " channel      S0       S1       S2       S3" <<std::endl;
  COUT << " ------------------------------------------------" << std::endl;
...
int   Packet_mutc_fpganew::fillIntArray (int iarr[],

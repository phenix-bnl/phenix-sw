#include <packet_hbd_fpga3samples.h>


#define HBD_MAX_MODULES 4
//#define HBD_NSAMPLES 20

Packet_hbd_fpga3samples::Packet_hbd_fpga3samples(PACKET_ptr data)
  : Packet_w4 (data)
{
...
int *Packet_hbd_fpga3samples::decode ( int *nwout)
{
  ...
  while ( pos < dlength -1)
      ...
      while ( (k[pos] & 0xF0000000) ==  0xE0000000 )  // we have a first adc
	{
	  ...
	  // sample 0 has 16 bits
	  int adc0        =  ( k[pos] & 0xffff);  
	  iarr[slot++] = adc0;
	  pos++;
	  // sample 1 has 12 bits
	  int adc1        =  ( (k[pos]>>16) & 0xfff);  
	  iarr[slot++] = adc1;

	  // sample 2 has 12 bits
	  int adc2        =  ( (k[pos]) & 0xffff);  
	  iarr[slot++] = adc2;
	  pos++;
...
int  Packet_hbd_fpga3samples::iValue(const int ich, const int is)
{
  ...
  if ( is >=100 && is < 103) return iValue ( ich, is-100);
...
int  Packet_hbd_fpga3samples::iValue(const int ich, const char *what)
{
...
void Packet_hbd_fpga3samples::dump ( OSTREAM &os)
{
  int i,j;

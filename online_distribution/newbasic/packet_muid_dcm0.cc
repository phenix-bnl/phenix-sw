#include <packet_muid_dcm0.h>

Packet_muid_dcm0::Packet_muid_dcm0(PACKET_ptr data)
  : Packet_w4 (data){}
  
int *Packet_muid_dcm0::decode ( int *nwout)
{
  int *p,*k;
  int pos;
  ...
  while (pos <= dlength) 
    {
      ...
      roc  = ((k[pos] & 0xff000000) >> 24) -1;
      word = ((k[pos] & 0x00700000) >> 20) -1;                
      data =   k[pos] & 0x0000ffff;
      if ( word <6 && word >=0  && roc < 20 && roc >=0 ) p[roc*6 + word] = data;
      pos++;
    }
...
int *Packet_muid_dcm0::decode_misc (int *nwout)
{
...
int Packet_muid_dcm0::iValue(const int ich, const int roc)
{
...
int Packet_muid_dcm0::iValue(const int ich, const char *what)
{
...
int Packet_muid_dcm0::iValue(const int ich)
{
  if (decoded_data1 == NULL )
    {
      if ( (decoded_data1 = decode(&data1_length))==NULL)
...
int
Packet_muid_dcm0::fillIntArray (int iarr[],
  ...
  *nwout = 0;
  ...
  if (strcmp(what,"SPARSE") == 0)
      ...
      while (pos <= dlength) 
	{
	  ...
	  if (data) 
	    {
	      // now here we fix up the indices
	      data = data | ( word << 20 ) | (roc << 24);
	      iarr[i++] = data;
	    }
...
void Packet_muid_dcm0::dump ( OSTREAM &os) 
{
  int i;
  ...
  for (j=0; j<20; j++)
    {
      os << std::dec << SETW(5) << j << " |  ";
      for (l=0;l<6;l++) 	os << std::hex << SETW(5) << iValue(l,j) << " " ;
      os << std::endl;
    }

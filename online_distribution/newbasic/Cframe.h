...
**   Fields which are a full DWORD (4 bytes) long are 
**   defined by a single value, the offset of the DWORD
**   that comprises them.  The smaller fields have four
**   #defined values:
**
** 1.The offset of the DWORD the field is in
**    (which word it is in)
** 2.The offset of the field in that DWORD
**    (where it starts in that word)
** 3.The number of bits in the field
**    (how big it is)
** 4.The mask used to pull out the field from its DWORD
...
/*
**  Byte-swap an entire frame using the (swapped) length stored in the first DWORD.
**

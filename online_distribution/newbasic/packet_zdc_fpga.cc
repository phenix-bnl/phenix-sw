#include <packet_zdc_fpga.h>

Packet_zdc_fpga::Packet_zdc_fpga()
{
}
Packet_zdc_fpga::Packet_zdc_fpga(PACKET_ptr data)
  : Packet_w4 (data)
{
  no_boards = 1;
}
...
// ------------------------------------------------------
int Packet_zdc_fpga::iValue(const int ich)
{
...
int  Packet_zdc_fpga::iValue(const int ich, const char *what)
{
...
int  Packet_zdc_fpga::iValue(const int ich, const int board)
{
  if (decoded_data1 == NULL )
    {
      if ( (decoded_data1 = decode(&data1_length))==NULL)
...
int *Packet_zdc_fpga::decode (int *nwout)
{
  int *p,*k;
...
int *Packet_zdc_fpga::decode_misc (int *nwout)
{
  int *p,*k;
  int i;
  ...
  // calculate the number of boards using number of user words
...
void Packet_zdc_fpga::dump ( OSTREAM &os) 
{
  this->identify(os);
  ...
  for (board = 0; board < iValue(0,"NUM_BOARDS"); board++)
	...
	{
          int adc = iValue(channel + 8*board);
          int tdc0 = iValue(channel + 8*board,"T1");
          int tdc1 = iValue(channel + 8*board,"T2");

          if ( (adc==0) && (tdc0==0) && (tdc1==0) ) continue;

	  os << SETW(8) << adc;
	  os << SETW(8) << tdc0;
	  os << SETW(8) << tdc1 << std::dec << std::endl;
	}
    }

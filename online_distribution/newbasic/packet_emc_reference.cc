#include <packet_emc_reference.h>

Packet_emc_reference::Packet_emc_reference(PACKET_ptr data)
  : Packet_w4 (data){}
  
int *Packet_emc_reference::decode ( int *nwout)
{
  ...
  while (pos <= dlength) 
    {
      ...
      prdf_ch = (k[pos] >> 20 ) & 0xff;
      ch = (prdf_ch/24)*48 + prdf_ch%24;
      currentmaxlen = ch*5+4; // maximum array index for current channel
...
int Packet_emc_reference::decode_to_sparse ( int *p )
{
  ...
  while (pos  <  dlength -2) 
    {
      ...
      int prdf_ch = (k[pos] >> 20 ) & 0xff;
      
      p[index+0] = (prdf_ch/24)*48 + prdf_ch%24;
      p[index+2] = k[pos++ ] & 0xfff;
      p[index+4] = k[pos++ ] & 0xfff;
      p[index+3] = k[pos++ ] & 0xfff;
      p[index+5] = k[pos++ ] & 0xfff;
      p[index+1] = k[pos++ ] & 0xfff;
...
int *Packet_emc_reference::decode_amu (int *nwout)
{
  int *p,*k;
...
int *Packet_emc_reference::decode_misc (int *nwout)
{
  int *p, *k;
...
int  Packet_emc_reference::iValue(const int ich, const char *what)
{
...
int   Packet_emc_reference::iValue(const int ich, const int iy)
{
  // now let's derefence the proxy array. If we didn't decode
  // the data until now, we do it now
...
void Packet_emc_reference::dump ( OSTREAM &os) 
{
  ...
  COUT << " channel      time high-post low-post high-pre low-pre" <<std::endl;
  COUT << " -----------------------------------------------------" << std::endl;
...
int   Packet_emc_reference::fillIntArray (int iarr[],

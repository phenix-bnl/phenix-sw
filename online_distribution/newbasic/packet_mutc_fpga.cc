#include <packet_mutc_fpga.h>

Packet_mutc_fpga::Packet_mutc_fpga(PACKET_ptr data)
  : Packet_w4 (data){}
  
int *Packet_mutc_fpga::decode ( int *nwout)
{
  int *k;
  ...
  while ( (ch = (( k[pos] >> 20) & 0xff)) <= 0x7f )
      ...
      if (pos >= dlength)
	{
	  ...
	  *nwout = 0;
      ...
      iarr[ch*4+0] = 2047 - (k[pos + 1] & 0x7ff);
      iarr[ch*4+1] = 2047 - (k[pos + 2] & 0x7ff);
      iarr[ch*4+2] = 2047 - (k[pos + 0] & 0x7ff);
      iarr[ch*4+3] = 2047 - (k[pos + 3] & 0x7ff);

      pos += 4; //advance 4 positions, that is, go to the next channel
...
int *Packet_mutc_fpga::decode_misc (int *nwout)
{
  int *p,*k;
  int i;
...
int  Packet_mutc_fpga::iValue(const int ich, const int is)
{
...
int  Packet_mutc_fpga::iValue(const int ich, const char *what)
{
  ...
  if (strcmp(what,"AMU") == 0)  // user requested AMU cells info
 ...
 else if ( strcmp(what,"CHECKPARITY")==0)
    {
      ...
      if (getHitFormat() == IDMUTC_FPGA0SUP ) return 0; // can't do it
...
void Packet_mutc_fpga::dump ( OSTREAM &os)
{
  int i,j;
...
int   Packet_mutc_fpga::fillIntArray (int iarr[],

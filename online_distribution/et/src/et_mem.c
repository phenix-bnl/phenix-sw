 ...
 */
{
  ...
  num_pages  = (wantedsize + pagesize -1)/pagesize;  
  totalsize  = (size_t) (pagesize * num_pages);
  /*printf("et_mem_create: size = %d bytes, requested size = %d bytes\n",totalsize, memsize);*/
  umask(002); /* standard umask 022 prevents group writable to be set */
  ...
  first_item.totalsize = totalsize;
  first_item.usedsize  = memsize;
 ...
 */
{
  ...
  if ((fd = open(name, O_RDWR|O_CREAT|O_EXCL, S_IRWXU|S_IRWXG)) < 0) {
    /* failed cause it exists already */
    /* perror("open"); */
    printf("et_temp_create: open error %d of file %s\n", fd, name);
    return NULL;
  }
  else {
    /* set (shared mem, file) size */
    if (ftruncate(fd, (off_t) size) < 0) {
      close(fd);
    printf("et_temp_create: ftruncate error %d of size %u, name %s\n", fd, size, name);
      unlink(name);
      return NULL;
    }
  }
  ...
  if ((pmem = mmap(0, size, PROT_READ|PROT_WRITE, MAP_SHARED,
       fd, (off_t)0)) == NULL) {
    close(fd);
    printf("et_temp_create: mmap error, size %u, name %s\n", size, name);
    unlink(name);
    return NULL;
  }
...
void *et_temp_attach(const char *name, size_t size)
{
  ...
  if ((fd = open(name, O_RDWR, S_IRWXU)) < 0) {
    printf("et_temp_attach: open error %d for %s\n", fd, name);
    return NULL;
  }

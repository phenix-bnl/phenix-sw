...
int et_byteorder(void)
{
  ...
  if (sizeof(short) == 2) {
    ...
    fprintf(stderr, "et_byteorder: sizeof(short) = %u\n", sizeof(short));
    return ET_ERROR;
  }
}
...
int et_netinfo (et_ifnames *ifnames, et_ifaddrs *ifaddrs, et_subnets *nets)
{
  ...
  for (;ifi != NULL; ifi = ifi->ifi_next) {
	...
	if (hptr == NULL) {
	  int hexadd = *((int *) & addr_in->sin_addr);
	  int inum[4] = {0};
	  int i;
	  for (i = 0; i < 4; i++)
	    {
	    ...
	    fprintf(stderr, "et_netinfo: error in gethostbyaddr: h_errno: %d, %s for address %d.%d.%d.%d\n",h_errno,hstrerror(h_errno),inum[0],inum[1],inum[2],inum[3]);
	  continue;
	}
	/* if this is the default interface, mark it */

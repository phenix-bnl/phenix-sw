 ...
 *----------------------------------------------------------------------------*
 * Description:
 *      Routines dealing with stations, initializations, misc.
 *
 * Author:
 *	Carl Timmer
...
int main(int argc, char **argv)
{
  ...
  */
   ...
   */  
    ...
    /*    et_open_config_sethost(openconfig, "phoncs1c.phenix.bnl.gov"); */
    et_open_config_sethost(openconfig, argv[2]);
    et_open_config_setserverport(openconfig, 11111);

  
  /*  if (et_open(&id, argv[1], openconfig) != ET_OK) { */
  if (iret = et_open(&id, argv[1], openconfig) != ET_OK) {
    printf("et_netclient: cannot open ET system iret: %d\n",iret);
    exit(1);
  }
  ...
  et_station_config_setprescale(sconfig, 20);
  /*  et_station_config_setcue(sconfig, 100); */
  et_station_config_setcue(sconfig, 1);
  ...
  for (i=0; i < 20000; i++) {
    ...
    err = et_event_get(id, my_att, &ev, ET_SLEEP, NULL);
    if (err < ET_OK) {
	...
	et_event_getdata(ev, (void **) &pdata);
        intdata = (unsigned long int *) pdata;
        et_event_getlength(ev,&len);
	/* do we need to swap data ? */
	et_event_needtoswap(ev, &swap);
	if (swap == ET_SWAP) {
          /* swap data  here */;
	}
	/*
        printf("data: %x %x %x %x %x, len: %d\n",*intdata,*(intdata+1),*(intdata+2),*(intdata+3),*(intdata+4),len);
	*/
	/*
      }
    }
  ...
  printf("et_netclient: got chunk events in SLEEP wait\n");
  ...
  if (0) {
	...
	exit(1);
      ...
      printf("et_netclient: got chunk events in TIMED wait (1sec)\n");
    }
	...
	exit(1);
      ...
      printf("et_netclient: got chunk events in ASYNC wait\n");
    }
    ...
    printf("et_netclient: got chunk events & modified (SLEEP)\n");
	...
	exit(1);
      ...
      printf("et_netclient: got chunk events & modified (TIMED)\n");
    }    
	...
	exit(1);
      ...
      printf("et_netclient: got chunk events & modified (ASYNC)\n");
    }    
    ...
    printf("et_netclient: put 1 NEW event (SLEEP)\n");
	...
	exit(1);
      ...
      printf("et_netclient: put 1 NEW event (TIMED)\n");
    }   
	...
	exit(1);
      ...
      printf("et_netclient: put 1 NEW event (ASYNC)\n");
    }
    ...
    printf("et_netclient: put chunk NEW events in (SLEEP)\n");
	...
	exit(1);
      ...
      printf("et_netclient: put chunk NEW events in (TIMED)\n");
    }
	...
	exit(1);
      ...
      printf("et_netclient: put chunk NEW events in (ASYNC)\n");
    }
    ...
    printf("et_netclient: got 1 event singly (SLEEP)\n");
	...
	exit(1);
      ...
      printf("et_netclient: got 1 event singly (TIMED)\n");
    }
	...
	exit(1);
      ...
      printf("et_netclient: got 1 event singly (ASYNC)\n");
    }
    ...
    printf("et_netclient: got 1 event singly & modified (SLEEP)\n");
	...
	exit(1);
      ...
      printf("et_netclient: got 1 event singly & modified (TIMED)\n");
    }
	...
	exit(1);
      ...
      printf("et_netclient: got 1 event singly & modified (ASYNC)\n");   
    }

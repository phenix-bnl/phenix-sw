...
int QASummary::processMui()
{
  ...
  // the maximum number of triggers (for any datatype)
  const int MAXTRIGGERS = 2; // only BBCLL1 trigger for each arm

  // South and North MUID arms
  const int NUMARM      = 2;

  // datatypes: 0 = Run2 AuAu, 1 = Run2(&3&4) pp, 2 = Run3 dAu, 4 = Run4 AuAu 
  // dA values are set at about the mean between AA and pp for now..
  const int NDATATYPES = 6;
  ...
  const float MEANNHITS_MIN_MB[NDATATYPES] = {1., 1., 1., 1., 1., 1.};
  // const float MEANNHITS_MAX_MB[NDATATYPES] = {25., 25., 25., 25., 25., 25.}; (old set)
  const float MEANNHITS_MAX_MB[NDATATYPES] = {65., 65., 65., 65., 65., 65.};
  // const float EVTWROADS_MIN_MB[NDATATYPES] = {0.0005, 0.0005, 0.0005, 0.0005, 0.0005, 0.0005}; (old set)
  const float EVTWROADS_MIN_MB[NDATATYPES] = {0.0015, 0.0015, 0.0015, 0.0015, 0.0015, 0.0015};
  const float MEANNROADS_MAX_MB[NDATATYPES] = {5., 5., 5., 5., 5., 5.};
  const float NROADSSUP10_MAX_MB[NDATATYPES] = {0.15, 0.15, 0.15, 0.15, 0.15, 0.15};
  const float GOLDPERROAD[NDATATYPES] = {1., 1., 1., 1., 1., 1.};
  const float MEANMATCHPOS_MAX_MB[NDATATYPES] = {40., 40., 40., 40., 40., 40.};
  ...
  int datatype = (int) (muiDataType->GetBinContent(1)/muiDataType->GetEntries());
  ...
  if ( datatype == 1 )
    ...
    { // For run-4 AuAu 200 GeV data
      ntriggers = 2; // only MB trigger one per each arm
    }
  else if (datatype == 5)
    { // For run-4 AuAu 63 GeV data
      ntriggers = 4;
    }
  else
    {
  ...
  for (int arm = 0; arm < NUMARM; arm++)
	...
	{ // no min bias triggers, we can't say anything about this run
      ...
      // Muid 1d trigger is absent for run-4 AuAu data
      /*
	else if ( n1dtrig == 0 )
	...
	{ // subtract bin 1 which has zero road entries
	  evtwroadsmb =  (int) (muiNRoads[trigbitmb]->GetEntries() - muiNRoads[trigbitmb]->GetBinContent(1));
	...
	{
	  supmb += (int) (muiNRoads[trigbitmb]->GetBinContent(ibin));
	} 
	...
	{ // subtract bin 1 which has zero road entries
	  evtwgoldenroadsmb =  (int) (muiNRoads_Golden[trigbitmb]->GetEntries() - 
      ...
      if (meanroadtrackmatchmb > MEANMATCHPOS_MAX_MB[datatype]) 
	{
      ...
      */ 
      ...
      sprintf(name,"%s %s",armname,"<NHits>");
      CommitToQADatabase("Muid", name, (float)meanhitsmb, (float)muiNHits[trigbitmb]->GetRMS());

      sprintf(name,"%s %s",armname,"Fract.of Evts w/ roads");
      CommitToQADatabase("Muid", name, (float)fracevtwroadsmb, 0.0);

      sprintf(name,"%s %s",armname,"Frac.of Evts w/ goldroads");
      CommitToQADatabase("Muid", name, (float)fracevtwgoldenroadsmb, 0.0);

      sprintf(name,"%s %s",armname,"<Road-Track match>");
      CommitToQADatabase("Muid", name, (float)meanroadtrackmatchmb, (float)muiRoadTrack_MatchPos[trigbitmb]->GetRMS());

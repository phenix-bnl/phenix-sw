...
//  fin.open("crk_reject_map.txt");
  ...
  TF1 *crkf1 = new TF1("crkf1","gaus",nhit_mean-3*nhit_sigma,nhit_mean+3*nhit_sigma);
...
//  textFile << endl;
  ...
  elseif(error_pmt>255) error_in_nhits=255;
  else error_in_nhits=error_pmt;

  status_word | = (error_in_nhits << 4);
  ...
  TF1 *crkf1 = new TF1("crkf1","gaus",nhit2_mean-3*nhit2_sigma,nhit2_mean+3*nhit2_sigma);
...
//  textFile << endl;
  ...
  elseif(error_pmt>255) error_in_nhits2=255;
  else error_in_nhits2=error_pmt;
  ...
  for(i=0;i<5120;i++){
    ...
    if(toff[i]>-800 && temp_count>8){
      ...
      ana_t0mean->Fill(crkt0mean[i]);
      t0sigma[i]=temp_t0sigma;
      ana_t0sigma->Fill(t0sigma[i]);
      PMTcount++;
    }
    delete t0pmts;
//    if((i+1)%1280==0) cout << "t0 PMT_num = " << i << endl;
  }
...
//
  ...
  int status;
//  if(error_in_gain || error_in_nhits) status=1; else status=0;
  if(error_in_gain>0 || error_in_nhits>0){
    if(error_in_gain==2 && error_in_nhits2==0) status_word | = 2;
    else status_word | = 1;
  }
  else status_word | = 0;
...
//  textFile<< endl;
  ...
  statusFile << status_word << " " <<status_word << " ";
}

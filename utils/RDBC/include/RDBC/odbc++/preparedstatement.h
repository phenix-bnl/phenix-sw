...
namespace odbc {
  ...
  public:
    ...
    /** Reimplement virtual signatures in base class Statement that were 
     *  hidden by argumentless versions above.
     *  2003-05-02 rhatcher@fnal.gov
     */
    bool       execute(const ODBCXX_STRING&       /* sql */) 
      { return execute(); }
    ResultSet* executeQuery(const ODBCXX_STRING&  /* sql */) 
      { return executeQuery(); }
    int        executeUpdate(const ODBCXX_STRING& /* sql */) 

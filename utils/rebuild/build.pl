...
my $CC = "pinkenburg\@bnl.gov,hachiya\@rcf.rhic.bnl.gov,lebedev\@iastate.edu";
my @externalPackages = ();
...
printhelp:
    ...
    print "--lafiles          Use libtool *.la files.\n";
    exit(0);
  }

# Read in list of packages and contacts, drop packages for certain options
my @package = ();
my %contact = ();
...
my $OSVersion = "SL7";
my $externalPackagesDir;
if ($opt_version =~ /play/) 
{
    $externalPackagesDir = "$OPT_PHENIX/stow";
}
else
{
    $externalPackagesDir = "$OPT_PHENIX";
}
...
$workdir = $opt_workdir ? $opt_workdir : '/home/'.$USER;
...
if ($opt_tinderbox)
  {
    ...
    open(TIND,"| /phenix/WWW/offline/tinderbox/handlemail.pl /phenix/WWW/offline/tinderbox");
    print TIND "\n";
...
if ($opt_phenixinstall && !$opt_scanbuild && !$opt_coverity)
{
    if ($opt_64)
    {
	$place = '/afs/rhic.bnl.gov/phenix/software/64/'.$opt_version;
    }
    else
    {
	$place = '/afs/rhic.bnl.gov/phenix/software/'.$opt_version;
    }
...
if (-e $sourceDir)
...
else
  {
    ...
    chdir $sourceDir;
    $cvscommand = "cvs -d $CVSROOT co $opt_cvstag phuniverse";
    print LOG $cvscommand, "\n";
    goto END if &doSystemFail($cvscommand);
	...
	if (&doSystemFail($arg))
	  {
		...
		print MAIL "To: $contact{$m}\n";
                ...
                print MAIL "Cc: $CC\n";	
                print MAIL "Subject: your configure crashed the $opt_version build on $OSVersion\n\n";
		print MAIL "\n";
	        print MAIL "Hello,\n";
                print MAIL "The rebuild crashed in module $m at $date under $OSVersion.\n";
		print MAIL "\"$arg\" failed: $? \n";
		print MAIL "Please look at the rebuild log, found on: \n";
		print MAIL "http://phenix-intra.sdcc.bnl.gov/software/tinderbox\n";
	...
	if (&doSystemFail($arg))
	  {
		...
		print MAIL "To: $contact{$m}\n";
                print MAIL "From: The Phenix rebuild daemon\n";
                print MAIL "Cc: $CC\n";	
                print MAIL "Subject: your install-data crashed the $opt_version build on $OSVersion\n\n";
		print MAIL "\n";
		print MAIL "Hello,\n";
...
# MuTrigLL1Emulator does not compile with insure
	...
	if (&doSystemFail($arg))
	{
		...
		print MAIL "From: The Phenix rebuild daemon\n";
		print MAIL "Cc: $CC\n";	
		print MAIL "Subject: your code crashed the $opt_version build on $OSVersion\n\n";
		print MAIL "Hello,\n";
...
if ($opt_phenixinstall && !$opt_scanbuild && !$opt_coverity)
{
    my $releasedir = sprintf("/afs/rhic.bnl.gov/phenix/software/sys/%s/log",$afs_sysname);
...
if ($opt_tinderbox) 
  {
    ...
    my $cmd = sprintf("cat %s | /phenix/WWW/offline/tinderbox/handlemail.pl /phenix/WWW/offline/tinderbox",$logfile);
    system($cmd);
  }
...
print INFO " http://phenix-intra.sdcc.bnl.gov/software/tinderbox/showbuilds.cgi?tree=default&nocrap=1&maxdate=".$startTime."\n";
print INFO " CVS tag: \n".$opt_cvstag."\n";
...
sub check_expiration_date
{
	...
	print MAIL "Hello,\n";
	print MAIL "this mail does not indicate a problem - it is informational only\n";
	...
	print MAIL "if you have other taxi modules which depend on this on you need to reenable it\n\n";
        print MAIL "Background: Whenever you sign up for a taxi your module gets added to the nightly build. The\n";
	print MAIL "nightly build runs a few code checkers which are not part of the gatekeeprr\n";
	print MAIL "e.g. insure and coverity\n";
        print MAIL "https://phenix-intra.sdcc.bnl.gov/WWW/p/draft/phnxbld/coverity/report\n";
	print MAIL "this starts a clock and after 4 weeks the module is removed again from the nightly\n";
        print MAIL "build and with the next ana build your module expires and you get this mail\n";
	print MAIL "Yours, The Rebuild Daemon \n\n";
	close(MAIL);
    }
    $setexpired->finish();
...
#	my $covanacmd = sprintf("cov-analyze  --disable-default --enable OVERRUN --enable STRAY_SEMICOLON --dir %s",$covdir);
	print LOG "$covanacmd\n";
	...
	close(F2);
	my $covcmd = sprintf("cov-commit-defects --host coverity.rcf.bnl.gov --stream offline --user pinkenbu --dir %s",$covdir);
...
#  	        my $covanacmd = sprintf("cov-analyze  --disable-default --enable OVERRUN --enable STRAY_SEMICOLON --enable BAD_COMPARE --enable MISSING_BREAK --enable COPY_PASTE_ERROR --enable MISSING_RETURN --enable NO_EFFECT --dir %s",$covdir);
		open(F2,"$covanacmd 2>&1 |");
		while(my $line = <F2>)
		{
		    chomp $line;
		...
		if ($makehtml > 0)
		{
			...
			print F1 "<a href=\"$packages\">$packages</a> contact: $contact{$packagename} </br>\n";
	            ...
	            if (exists $contact{$packagename})
	              {
	                 $mailinglist{$packagename} = "https://phenix-intra.sdcc.bnl.gov/WWW/p/draft/phnxbld/coverity/report/$hrefentry";
	              }
		}
		else
		{
...
# now send the mails
        if ($opt_notify)
	{
		...
		my $scancc = "pinkenburg\@bnl.gov,hachiya\@rcf.rhic.bnl.gov";
#		my $scancc = "pinkenburg\@bnl.gov";
		print LOG "\nsending coverity report mail to $contact{$package}, cc $scancc\n";
		open( MAIL, "|$SENDMAIL" );
		print MAIL "To: $contact{$package}\n";
#		print MAIL "To: pinkenburg\@bnl.gov\n";
		print MAIL "From: The Phenix rebuild daemon\n";
		print MAIL "Cc: $scancc\n";	
		print MAIL "Subject: SERIOUS coverity issues in $package\n\n";
		print MAIL "Hello $contact{$package},\n";
		print MAIL "coverity has found some potentially VERY REAL problems\n";
		...
		print MAIL "https://phenix-intra.sdcc.bnl.gov/WWW/p/draft/phnxbld/coverity/report\n\n";
		print MAIL "instructions how to run coverity yourself are in our wiki\n\n";
		print MAIL "https://phenix-intra.sdcc.bnl.gov/WWW/offline/wikioff/index.php/Coverity\n\n";
...
#               print $sp1[1];
	...
	if (exists $contact{$packagename})
	{
	    $mailinglist{$packagename} = "https://phenix-intra.sdcc.bnl.gov/WWW/p/draft/phnxbld/scan-build/scan/$hrefentry";
	}
	else
	{
...
# now send the mails
	...
	for my $package (sort keys %mailinglist)
	{
	    ...
	    print MAIL "$mailinglist{$package}\n\n";
            print MAIL "All reports are available under\n\n";
            print MAIL "https://phenix-intra.sdcc.bnl.gov/WWW/p/draft/phnxbld/scan-build/scan\n\n";
	    print MAIL "instructions how to run scan-build yourself are in our wiki\n\n";
	    print MAIL "https://phenix-intra.sdcc.bnl.gov/WWW/offline/wikioff/index.php/Scan-build\n\n";
            print MAIL "Please look at the report and fix the issues found\n";
	    print MAIL "Sincerely yours, The Rebuild Daemon \n\n";
	    close(MAIL);
	}

void valgrind_pisaToDST(Int_t nEvents=1, const char *in = "dummy", char *dstout="simDST.root", 
               ...
               const Int_t debugFlag=0)
{
  ...
  gSystem->Exec("mkdir ./valgrind_pisatodst");
  ...
  /* 
     tell module to run the response, 
     i.e. create, write the Mut/Mui hits together with the MC hits
  */
...
//   se->registerSubsystem( muon_mc_filter );
  ...
  dmp->SetOutDir("./valgrind_pisatodst");

void valgrind_lpc(const int nevents=1000,
		      const char* input="/phenix/data14/phnxreco/TestBuild/prdfs/EVENTDATAxxx_P01-0000120849-0001.PRDFF")
{
  ...
  gSystem->Exec("mkdir ./valgrind_lpc");
  ...
  dmp->SetOutDir("./valgrind_lpc");

  se->registerSubsystem(head);
  se->registerSubsystem(lpc);
  se->registerSubsystem(dmp);

  Fun4AllOutputManager *out = new Fun4AllDstOutputManager("DSTOUT","valgrind_lpc.root");
